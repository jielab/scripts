...
score_one(){
  ...
  input_files=("$w" "$ROOT/f/score.sh" "$ROOT/f/combine_scores.py")
  if [[ $mode == pfile ]]; then input_files+=("$prefix.pgen" "$prefix.psam");if [[ -s $prefix.pvar ]]; then input_files+=("$prefix.pvar");else input_files+=("$prefix.pvar.zst");fi
  ...
  score_sig=$(python3 "$ROOT/f/pipeline_io.py" signature --files "${input_files[@]}")
...
make_scores(){
  ...
  for label in "${labels[@]}"; do
    pids=();status=0
    for c in "${CHRS[@]}"; do
      score_one "$label" "$c" & pids+=("$!")
      if ((${#pids[@]}>=GRID_JOBS));then
        for pid in "${pids[@]}";do wait "$pid" || status=1;done
        ((status==0)) || _grid_die "Scoring failed for $label";pids=()
      fi
    done
    for pid in "${pids[@]}";do wait "$pid" || status=1;done
