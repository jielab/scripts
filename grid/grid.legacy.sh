...
# 🚩 PRS-CSx, ancestry, and DiscoDivas workflow
set -euo pipefail
export LC_ALL=C
...
Core options:
  ...
  --data-root PATH           D-drive root under WSL [/mnt/d]
  --dir-gen PATH             target PLINK bfiles chr1..chr22 [/mnt/d/data/ukb/gen/typ]
  --dir-imp PATH             target PLINK 2 pfiles chr1..chr22 [/mnt/h/ukbGen/37/imp]
  --dir-gwas PATH            GWAS project root; reads common/TRAIT/gwas/TRAIT.POP.gz
  --output-dir PATH          trait output [/mnt/d/analysis/grid/TRAIT]
...
Evaluation options:
  ...
  --replace BOOL             recreate completed outputs [FALSE]
...
Important:
  ...
  * `grid.sh pca` runs PCA and then ancestry. A complete PCA cache is skipped, after
    which any missing ancestry outputs are built automatically.
...
Examples:
  ...
  ./grid.sh ancestry --replace TRUE  # optional ancestry-only rebuild
USAGE
}

dir0=/mnt/d
dir_gen=/mnt/d/data/ukb/gen/typ
dir_imp=/mnt/h/ukbGen/37/imp
dir_gwas=""
outdir=""
trait=height
step=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --data-root) need_value "$1" "${2-}"; dir0=${2%/}; shift 2 ;;
    --dir-gen) need_value "$1" "${2-}"; dir_gen=${2%/}; shift 2 ;;
    --dir-imp) need_value "$1" "${2-}"; dir_imp=${2%/}; shift 2 ;;
    --dir-gwas) need_value "$1" "${2-}"; dir_gwas=${2%/}; shift 2 ;;
    --output-dir) need_value "$1" "${2-}"; outdir=${2%/}; shift 2 ;;
    --phe-file) need_value "$1" "${2-}"; phe_file=$2; shift 2 ;;
    --n-gwas) need_value "$1" "${2-}"; n_gwas_spec=$2; shift 2 ;;
    ...
    --med-file) need_value "$1" "${2-}"; med_file=$2; shift 2 ;;
    --disco-a) need_value "$1" "${2-}"; disco_a=$2; shift 2 ;;
    --cov-pcs) need_value "$1" "${2-}"; cov_pcs=$2; shift 2 ;;
    --distance-pcs) need_value "$1" "${2-}"; distance_pcs=$2; shift 2 ;;
    --ancestry-file) need_value "$1" "${2-}"; ancestry_file=$2; shift 2 ;;
    --ancestry-id-col) need_value "$1" "${2-}"; ancestry_id_col=$2; shift 2 ;;
    --ancestry-col) need_value "$1" "${2-}"; ancestry_col=$2; shift 2 ;;
    --ancestry-prob-col) need_value "$1" "${2-}"; ancestry_prob_col=$2; shift 2 ;;
    --self-report-col) need_value "$1" "${2-}"; self_report_col=$2; shift 2 ;;
    --auto-ancestry) need_value "$1" "${2-}"; auto_ancestry=${2^^}; shift 2 ;;
    --auto-ancestry-file) need_value "$1" "${2-}"; auto_ancestry_file=$2; shift 2 ;;
    --ethnicity-col) need_value "$1" "${2-}"; ethnicity_col=$2; shift 2 ;;
    --ancestry-prob-threshold) need_value "$1" "${2-}"; ancestry_prob_threshold=$2; shift 2 ;;
    --anchor-max-per-group) need_value "$1" "${2-}"; anchor_max_per_group=$2; shift 2 ;;
    --anchor-margin-quantile) need_value "$1" "${2-}"; anchor_margin_quantile=$2; shift 2 ;;
    --target-ids) need_value "$1" "${2-}"; target_ids=$2; shift 2 ;;
    --exclude-ids) need_value "$1" "${2-}"; exclude_ids=$2; shift 2 ;;
    --prscsx-repeats) need_value "$1" "${2-}"; prscsx_repeats=$2; shift 2 ;;
    --prscsx-prob-min) need_value "$1" "${2-}"; prscsx_prob_min=$2; shift 2 ;;
    --validation-frac) need_value "$1" "${2-}"; validation_frac=$2; shift 2 ;;
    --disco-repeats) need_value "$1" "${2-}"; disco_repeats=$2; shift 2 ;;
    --fine-tune-n) need_value "$1" "${2-}"; fine_tune_n=$2; shift 2 ;;
    --fine-tune-prob-min) need_value "$1" "${2-}"; fine_tune_prob_min=$2; shift 2 ;;
    --require-self-match) need_value "$1" "${2-}"; require_self_match=${2^^}; shift 2 ;;
    --min-test-group) need_value "$1" "${2-}"; min_test_group=$2; shift 2 ;;
    --run-prscsx) need_value "$1" "${2-}"; run_prscsx=${2^^}; shift 2 ;;
    --run-disco) need_value "$1" "${2-}"; run_disco=${2^^}; shift 2 ;;
    --write-predictions) need_value "$1" "${2-}"; write_predictions=${2^^}; shift 2 ;;
...
for v in replace require_self_match run_prscsx run_disco write_predictions auto_ancestry; do
  eval "x=\${$v}"
  option_name=${v//_/-}
  case "$x" in TRUE|FALSE) ;; *) echo "ERROR: --$option_name must be TRUE/FALSE" >&2; exit 2 ;; esac
done
[[ -z "$ancestry_file" || -n "$ancestry_col" ]] || { echo "ERROR: --ancestry-file requires --ancestry-col" >&2; exit 2; }

[[ -n "$dir_gwas" ]] || dir_gwas="$dir0/data.BIG/gwas/main"
...
run_timed_step() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$name" "$started" "$finished" "$((end_epoch-start_epoch))" "$([[ $rc -eq 0 ]] && echo OK || echo FAILED)" "step=$step replace=$replace" >> "$audit_file"
  [[ $rc -eq 0 ]] || return "$rc"
...
step_csx() {
  ...
  for race in "${races[@]}"; do
    ...
    need_file "$dir_gwas/common/$trait/gwas/$trait.$race.gz"
  ...
  for race in "${races[@]}"; do
    ...
    if [[ "$replace" == TRUE || "$prep_complete" != TRUE ]]; then
      prep_cmds+=("Rscript '$prepare_sumstats_r' --in '$dir_gwas/common/$trait/gwas/$trait.$race.gz' --out-dir '$dir_csx/input/$race' --pop '$race' > '$dir_csx/input/log/$race.prepare.log' 2>&1")
    fi
  done
...
if [[ "$replace" == TRUE || "\$complete" != TRUE ]]; then
...
for race in ${races[*]}; do
  ...
  if [[ "$replace" == TRUE || ! -s "\$out.sscore" || ! -e "\$out.score.done" ]]; then
...
EOF2
    ...
    [[ "$replace" == TRUE || "$complete" != TRUE ]] && model_cmds+=("$cmd")
  done
...
step_ancestry() {
  ...
  if [[ "$replace" != TRUE && -s "$auto_ancestry_file" \
        && -s "$dir_pca/ancestry_auto_qc.xlsx" && -s "$dir_pca/Fig2.Ancestry_QC.png" ]]; then
    log "Automatic ancestry outputs exist: $auto_ancestry_file"
    return
  fi
...
step_pca() {
  ...
  if [[ "$replace" != TRUE ]] && pca_cache_has_required_pcs; then
    log "Complete PCA results and QC already exist; all PCA commands are skipped"
    step_ancestry
    return
  fi

  if [[ -s "$pca_file" && "$replace" != TRUE ]]; then
  ...
  # Cap PCA at four 4-GiB workers and scale down with available memory.
...
plink2 --pfile "$dir_imp/chr$chr" vzs \\
...
EOF2
    ...
    if [[ "$replace" == TRUE || ! -s "$dir_pca_raw/chr$chr.sscore" || ! -s "$dir_pca_raw/chr$chr.sscore.vars" || ! -e "$dir_pca_raw/chr$chr.pca${cov_pcs}.done" ]]; then
      cmds+=("$cmd")
    fi
  done
