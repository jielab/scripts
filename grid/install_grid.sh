...
if "$solver" env list | awk '{print $1}' | grep -qx "$ENV_NAME"; then "$solver" env update -n "$ENV_NAME" -f "$ROOT/environment.yml" --prune
...
The entry scripts automatically activate the environment; no manual activation is needed.
