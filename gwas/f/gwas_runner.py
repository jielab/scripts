...
def execute(plan):
        ...
        with open(Path(plan).parent/(job['name']+'.run.log'),'w') as log:
            ...
            try:
                while result.poll() is None:
                    if stop.is_set(): raise RuntimeError('GWAS run cancelled')
                    try: result.wait(timeout=0.2)
    ...
    def run_chromosome(chain):
        if spec.get('config', {}).get('module') == 'run_saige':
            from gwas_storage import execute_saige_chain
...
def finalize_plan(args):
    ...
    if config['module'] == 'prep_regenie':
        config = {key: config[key] for key in (
            'module', 'grch', 'imputed_dir', 'threads', 'memory', 'jobs',
            'plink2', 'global_maf', 'chromosomes', 'run', 'replace')}
