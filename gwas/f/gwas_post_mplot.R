...
render_self <- function(path) {
  ...
  plot_gwas <- if (identical(Sys.getenv("GWAS_THIN"), "TRUE") || grepl("\\.thin\\.gz$", input)) gwas else thinP0(gwas, P = 1e-3, p_col = "P")
  ...
  # Flags and hit reports use the complete associations, even when plotting a
  # capped sidecar. The display data and statistics must remain independent.
  full_input <- Sys.getenv("GWAS_FULL")
  if (nzchar(full_input) && normalizePath(full_input) != normalizePath(input) &&
      (write_sig || (!is.null(cis_rows) && nrow(cis_rows)))) {
    gwas <- read_gwas(full_input)
    raw_p <- suppressWarnings(as.numeric(gwas$P))
  }
