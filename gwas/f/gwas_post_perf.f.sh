...
std_format() {
  ...
  if [[ -n "${N_TOTAL:-}" ]]; then
    ...
    [[ "$miss_col" =~ ^[1-9][0-9]*$ ]] || { echo 'ERROR: --n-total requires an F_MISS column' >&2; return 1; }
  fi
  tmp="${out}.tmp.$$"
...
std_hm3() {
  ...
  [[ -s "$tmp" ]] || { echo "ERROR: HM3 format failed: $src" >&2; return 1; }
  mv -f -- "$tmp" "$out"
  rm -f -- "${out}.tbi" "${out}.csi"
...
# Adapter for the unchanged shared 0data.f.sh formatter API.
std_small() {
...
gwas_clean_liftover_small_to_final() {
  python3 "$LIFTOVER_HELPER" --input "$SMALL" --output "$FINAL" \
...
gwas_post_mplot() {
  ...
  GWAS_FULL="$FINAL" GWAS_THIN="${THIN_MODE:-FALSE}" Rscript "$MPLOT_R" "$GWAS_POST_MPLOT_INPUT" "$MH_PNG" "$GWAS" "$PLOT_METHOD" "$ADD_PANEL" \
