...
./ppp_download.sh download --workers 8
./ppp_download.sh rename --run-rename TRUE
./ppp_download.sh unzip --jobs 4
./ppp_download.sh rsid_map --jobs 4 --threads 4
HELP
}
...
case "$MODE" in
  ...
  download)
    ...
    export SYNAPSE_AUTH_TOKEN="$(tr -d '\r\n\t '\''\"' < "$TOKEN_FILE")"
    # Keep the full download worker count in foreground mode.
    mapfile -d '' EXTRA_ARGS < <(add_default_workers_if_missing 8 "$@")
    echo "Download log: $LOGDIR/$LOG_FILE"
    "$PYTHON_BIN" -u "$PY" \
      "${COMMON_ARGS[@]}" \
      --manifest "$MANIFEST_DOWNLOAD" \
      --retries 5 \
      "${EXTRA_ARGS[@]}" \
      2>&1 | tee "$LOGDIR/$LOG_FILE"
    ;;
  *)
