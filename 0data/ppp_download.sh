...
PY="$SCRIPT_DIR/ppp_download.py"
PYTHON_BIN="${PYTHON_BIN:-}"

TOKEN_FILE_DEFAULT="/mnt/d/data/ukb/ppp/authToken.txt"
OUT_DEFAULT="/mnt/h/gwas/ppp/raw"
LOG_DEFAULT="/mnt/h/gwas/ppp/log"
PPP_DIR_DEFAULT="/mnt/d/data.BIG/gwas/ppp"
FOLDER_DEFAULT="syn51365303"                    # UKB-PPP European discovery folder
...
Mode options:
  ...
  rename      [--run-rename TRUE|FALSE] [--force-bed TRUE|FALSE]
...
Defaults:
  ...
  PPP TSV    : $PPP_DIR_DEFAULT/ppp_3k_b38.tsv
  PPP BED    : $PPP_DIR_DEFAULT/ppp_3k_b38.bed
  folder     : $FOLDER_DEFAULT
  file regex : $NAME_REGEX_DEFAULT

Environment overrides:
  TOKEN_FILE=/path/authToken.txt OUT=/path/raw LOGDIR=/path/log PPP_FOLDER=syn... ./ppp_download.sh download --workers 8
  PPP_DIR=/path/ppp PPP_TSV=/path/ppp_3k_b38.tsv PPP_BED=/path/ppp_3k_b38.bed ./ppp_download.sh rename
  OUT=/path/raw ./ppp_download.sh unzip
  unzip writes each [prot].raw.gz next to its source [prot].tar
  rsid_map writes each [prot].gz from [prot].raw.gz, then deletes [prot].raw.gz by default
  rsid_map prefers pigz when installed; otherwise it falls back to gzip
  rsid_map caches the sorted lookup under its temp directory; set RSID_MAP_CACHE_LOOKUP=FALSE to disable
EOF
}
...
running_ppp_tasks() {
  ...
  ps -eo pid=,ppid=,args= 2>/dev/null | PPP_TASK_PATTERN='ppp_download[.]sh|ppp_download[.]py' awk -v self="$self" -v bash_self="$bash_self" -v parent="$parent" '
    BEGIN { pattern = ENVIRON["PPP_TASK_PATTERN"] }
    {
      pid = $1
      ppid = $2
      $1 = ""
...
check_existing_ppp_tasks() {
  local old pids self bash_self parent
  self="$$"
  bash_self="${BASHPID:-$$}"
  parent="${PPID:-}"
  old="$(running_ppp_tasks "$self" "$bash_self" "$parent" || true)"
  [[ -z "$old" ]] && return 0
...
generate_ppp_bed() {
  ...
  [[ -s "$tsv" ]] || { task_log "ERROR: PPP TSV not found or empty: $tsv"; return 2; }
  bed_dir="$(dirname -- "$bed")"
  mkdir -p -- "$bed_dir"
  tmp="$bed.tmp.$$"
  ...
  if (( rc != 0 )); then
    task_log "WARNING⚠ ppp_3k_b38.bed was not generated because TSV validation failed."
...
ensure_ppp_bed() {
  local force_bed="${1:-FALSE}"
  if [[ ! -s "$PPP_BED" ]] || bool_is_true "$force_bed"; then
    task_log "Preparing BED from TSV: $PPP_TSV"
    generate_ppp_bed "$PPP_TSV" "$PPP_BED"
  else
    task_log "BED already exists; validating: $PPP_BED"
  fi
  validate_ppp_bed "$PPP_BED"
}
...
run_rename_task() {
  local run_rename="FALSE" force_bed="FALSE"
  local arg
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --run-rename)
      ...
      --run-rename=*)
        ...
        ;;
      --force-bed)
        [[ $# -ge 2 ]] || { echo "ERROR: missing value after --force-bed" >&2; exit 2; }
        force_bed="$2"
        shift 2
        ;;
      --force-bed=*)
...
parse_rsid_map_args() {
  ...
  RSID_MAP_JOBS="${RSID_MAP_JOBS:-4}"
  RSID_MAP_THREADS="${RSID_MAP_THREADS:-4}"
  ...
  RSID_MAP_TMPDIR="${RSID_MAP_TMPDIR:-/tmp/ppp_rsid_map}"
  RSID_MAP_SORT_MEM="${RSID_MAP_SORT_MEM:-2G}"
  RSID_MAP_CACHE_LOOKUP="${RSID_MAP_CACHE_LOOKUP:-TRUE}"
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --overwrite)
        [[ $# -ge 2 ]] || { echo "ERROR: missing value after --overwrite" >&2; exit 2; }
...
stage_rsid_map_lookup_cache() {
  ...
  local cache_dir cache_file cache_tmp source_size cache_size
  cache_dir="$RSID_MAP_TMPDIR/lookup_cache"
  cache_file="$cache_dir/$(basename -- "$RSID_MAP_LOOKUP")"
  mkdir -p -- "$cache_dir"

  source_size="$(stat -c '%s' -- "$RSID_MAP_LOOKUP")"
  cache_size=0
  if [[ -s "$cache_file" ]]; then
    cache_size="$(stat -c '%s' -- "$cache_file")"
  fi

  if [[ "$cache_size" != "$source_size" || "$RSID_MAP_LOOKUP" -nt "$cache_file" ]]; then
...
OUT="${OUT:-$OUT_DEFAULT}"
LOGDIR="${LOGDIR:-$LOG_DEFAULT}"
PPP_DIR="${PPP_DIR:-$PPP_DIR_DEFAULT}"
PPP_TSV="${PPP_TSV:-$PPP_DIR/ppp_3k_b38.tsv}"
PPP_BED="${PPP_BED:-$(dirname -- "$PPP_TSV")/ppp_3k_b38.bed}"
CLEAN="${CLEAN:-$PPP_DIR}"
BADNAME="${BADNAME:-${OUT%/}.badName}"
PPP_FOLDER="${PPP_FOLDER:-$FOLDER_DEFAULT}"
NAME_REGEX="${NAME_REGEX:-$NAME_REGEX_DEFAULT}"
RSID_MAP_DIR="${RSID_MAP_DIR:-$PPP_DIR/rsid_map}"
...
case "$MODE" in
  ...
  unzip)
    ...
    nohup bash "$SCRIPT_DIR/ppp_download.sh" unzip-foreground "$@" >/dev/null 2>&1 &
    echo "PID: $!"
  ...
  rsid_map)
    ...
    nohup bash "$SCRIPT_DIR/ppp_download.sh" rsid_map-foreground "$@" >/dev/null 2>&1 &
    echo "PID: $!"
  ...
  test)
    ...
    if [[ -z "$PYTHON_BIN" ]]; then
      if command -v python >/dev/null 2>&1; then
        ...
        echo "ERROR: cannot find python or python3 in PATH. Set PYTHON_BIN=/path/to/python." >&2
        exit 2
      fi
    fi
