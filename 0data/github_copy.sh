...
scan_files() {
    ...
    TEMP_FILE=$(mktemp "$DST_REAL/.github_files.lst.tmp.XXXXXX")
    {
        ...
        printf '# Re-run scan if source content changes before sync.\n'
...
STRONG_ID_HEADERS = {
    ...
    "participant_id", "participantid", "eid_1", "eid_2", "iid",
}
GENERIC_ID_HEADERS = {
...
def detect_table(rows, panome=False):
    ...
    for row in rows:
        ...
        for column, header in enumerate(headers):
            if header in GENERIC_ID_HEADERS:
                generic_columns[column] = set()
        if any(UKB_FIELD.fullmatch(header) for header in headers):
            ukb_columns = {column: set() for column in range(min(3, len(row)))}
        for columns, threshold, reason in (
            (generic_columns, 3, "id_column_with_multiple_seven_digit_values"),
            (ukb_columns, 2, "ukb_field_columns_with_eid_like_values"),
        ):
