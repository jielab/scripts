...
# Hadoop sets permissions even on newly created files. A writable DrvFS mount
# can still reject chmod when its uid/gid mapping belongs to another user.
check_storage() {
  local directory=$1 probe details
  mkdir -p -- "$directory" || die "cannot create storage directory: $directory"
  probe="$(mktemp -d -- "${directory%/}/.wes-fs-check.XXXXXX")" ||
    die "cannot create storage probe in: $directory"
  if ! details="$( {
    chmod 755 -- "$probe" &&
    printf 'wes storage check\n' > "$probe/write-test" &&
    chmod 644 -- "$probe/write-test" &&
    mv -- "$probe/write-test" "$probe/rename-test" &&
    [[ "$(cat -- "$probe/rename-test")" == 'wes storage check' ]]
  } 2>&1)"; then
    rm -rf -- "$probe"
    echo "$details" >&2
    die "storage write/chmod/rename check failed: $directory. On WSL Windows drives, check the active mount uid=$(id -u),gid=$(id -g), not just /etc/fstab. Restart WSL after correcting the mount configuration. Changing --tmpdir alone does not fix an output mount with the same problem."
  fi
  rm -rf -- "$probe"
}
...
# Check the real paths before authentication, Spark, or any --overwrite cleanup.
storage_dirs=("$tmpdir" "$dir_out")
[[ "$single_variant" != TRUE ]] || storage_dirs+=("$single_dir_out")
for storage_dir in "${storage_dirs[@]}"; do
  check_storage "$storage_dir"
done
for storage_dir in "${storage_dirs[@]:1}"; do
  for existing_dir in "$storage_dir/files" "$storage_dir/.resume_batch"; do
    [[ ! -e "$existing_dir" ]] || check_storage "$existing_dir"
  done
done
echo "Storage write/chmod/rename check OK"
if [[ -z "$gcp_project" ]] && command -v gcloud >/dev/null 2>&1; then
  gcp_project="$(gcloud config get-value project 2>/dev/null || true)"
  [[ "$gcp_project" == '(unset)' ]] && gcp_project=""
fi
if [[ "$check_only" != TRUE && ( "$source_mt" == gs://* ||
...
# Spark prefers JAVA_HOME over PATH; validate the same JVM it will launch.
if [[ -n "${JAVA_HOME:-}" ]]; then
  [[ -x "$JAVA_HOME/bin/java" ]] || die "JAVA_HOME has no executable bin/java: $JAVA_HOME"
...
[[ "$java_version" == 11 ]] || die "Java 11 is required; detected: $(java -version 2>&1 | awk '/version "/ {print; exit}')"
...
# Fetch HTTPS dependencies before Spark starts. A failed JVM-side download only
# surfaces as JAVA_GATEWAY_EXITED, and Spark otherwise downloads on every run.
if [[ "$gcs_connector" == http://* || "$gcs_connector" == https://* ]]; then
  command -v curl >/dev/null 2>&1 || die "curl is required to download the GCS connector"
  connector_cache="${XDG_CACHE_HOME:-$HOME/.cache}/wes/gcs-connector"
  mkdir -p -- "$connector_cache"
  connector_key="$(printf '%s' "$gcs_connector" | sha256sum | cut -d ' ' -f 1)"
  connector_path="$connector_cache/$connector_key.jar"
  valid_connector() {
    python3 - "$1" <<'PY_CONNECTOR'
import sys
import zipfile
try:
    with zipfile.ZipFile(sys.argv[1]) as jar:
        if 'com/google/cloud/hadoop/fs/gcs/GoogleHadoopFileSystem.class' not in jar.namelist():
            raise ValueError('GCS filesystem class is missing')
...
PY_CONNECTOR
  ...
  if ! valid_connector "$connector_path"; then
    connector_part="$(mktemp "$connector_cache/.download.XXXXXX")"
    trap 'rm -f -- "$connector_part"' EXIT
    echo "Downloading GCS connector before starting Java..."
    curl_args=(--fail --location --silent --show-error --retry 2
      --connect-timeout 15 --max-time 180)
    [[ -z "$proxy_url" ]] || curl_args+=(--proxy "$proxy_url")
    curl "${curl_args[@]}" --output "$connector_part" "$gcs_connector" ||
      die "GCS connector download failed; check network/--proxy, or pass --gcs-connector /path/to/gcs-connector.jar"
    valid_connector "$connector_part" || die "Downloaded GCS connector is not a valid connector JAR"
    mv -f -- "$connector_part" "$connector_path"
    trap - EXIT
  fi
  gcs_connector="$connector_path"
fi

if [[ "$check_only" == TRUE ]]; then
  echo "Checking Java gateway and local Hail export with the configured storage..."
  WES_CHECK_CONNECTOR="$gcs_connector" WES_CHECK_MEMORY="$spark_memory" \
    WES_CHECK_TMPDIR="$tmpdir" WES_CHECK_OUT="$dir_out" \
    WES_CHECK_SINGLE="$single_variant" WES_CHECK_SINGLE_OUT="$single_dir_out" \
    python3 - <<'PY_CHECK'
import gzip
import os
import tempfile
...
connector = os.environ['WES_CHECK_CONNECTOR']
conf = {'spark.driver.memory': os.environ['WES_CHECK_MEMORY']}
conf['spark.jars' if '://' in connector or connector.endswith('.jar')
     else 'spark.jars.packages'] = connector
with tempfile.TemporaryDirectory(prefix='.wes-hail-check-',
                                 dir=Path(os.environ['WES_CHECK_TMPDIR']).resolve()) as tmp:
    try:
        hl.init(master='local[1]', tmp_dir='file://' + tmp, local_tmpdir=tmp,
                log=tmp + '/hail.log', spark_conf=conf)
        assert hl.utils.range_table(1).count() == 1
        jvm = SparkContext._active_spark_context._jvm
        jvm.java.lang.Thread.currentThread().getContextClassLoader().loadClass(
            'com.google.cloud.hadoop.fs.gcs.GoogleHadoopFileSystem')
        mt = hl.utils.range_matrix_table(2, 1, n_partitions=1)
        mt = mt.key_cols_by(_file=hl.str(mt.col_idx))
        mt = mt.annotate_entries(value=mt.row_idx + mt.col_idx)
        outputs = [os.environ['WES_CHECK_OUT']]
        if os.environ['WES_CHECK_SINGLE'] == 'TRUE':
            outputs.append(os.environ['WES_CHECK_SINGLE_OUT'])
        for output in outputs:
            with tempfile.TemporaryDirectory(prefix='.wes-hail-check-',
                                             dir=Path(output).resolve()) as staging:
                export_path = Path(staging) / 'export'
                hl.experimental.export_entries_by_col(
                    mt, export_path.as_uri(), batch_size=1, bgzip=True,
                    header_json_in_file=True, use_string_key_as_file_name=True)
                assert (export_path / 'index.tsv').is_file()
                with gzip.open(export_path / '0.tsv.bgz', 'rt') as handle:
                    lines = handle.readlines()
                assert lines[0].startswith('#') and len(lines) == 4, lines
            print(f'Hail BGZF export check OK: {output}', flush=True)
    finally:
        # hl.stop() shuts down the gateway but does not wait for its JVM to
        # exit. On DrvFS, the open hail.log can prevent directory removal.
        # Save the owned child before Hail clears SparkContext._gateway.
        gateway = SparkContext._gateway
        process = gateway.proc if gateway is not None else None
        try:
            hl.stop()
        finally:
...
PY_CHECK
  echo "Software, Java gateway and local export check OK (no GCS data read)"
  echo "Python: $(python3 --version 2>&1)"
  echo "Java:   $(java -version 2>&1 | awk '/version "/ {print; exit}')"
