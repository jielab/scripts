...
usage() {
  cat <<'USAGE'
Usage:
  ./clhls_unzip.sh [options]
  ./clhls_unzip.sh -h

Options:
  --raw-dir DIR          Archive root. Default: /mnt/d/data/clhls杜克/raw
  --clean-dir DIR        Extraction root. Default: /mnt/d/data/clhls杜克/clean
  --qc-dir DIR           QC output. Default: sibling _extract_qc directory
  --keep-tmp TRUE|FALSE  Keep temporary extraction files. Default: FALSE
...
while (( $# )); do
  case "$1" in
    --raw-dir) [[ $# -ge 2 ]] || { echo "ERROR: --raw-dir requires DIR" >&2; exit 2; }; RAW=$2; shift 2 ;;
    --clean-dir) [[ $# -ge 2 ]] || { echo "ERROR: --clean-dir requires DIR" >&2; exit 2; }; CLEAN=$2; shift 2 ;;
    --qc-dir) [[ $# -ge 2 ]] || { echo "ERROR: --qc-dir requires DIR" >&2; exit 2; }; QC=$2; shift 2 ;;
    --keep-tmp)
      [[ $# -ge 2 ]] || { echo "ERROR: --keep-tmp requires TRUE or FALSE" >&2; exit 2; }
      case "${2,,}" in true|1|yes) KEEP_TMP=1;; false|0|no) KEEP_TMP=0;; *) echo "ERROR: --keep-tmp requires TRUE or FALSE" >&2; exit 2;; esac
      shift 2
      ;;
    -h|--help) usage; exit 0 ;;
