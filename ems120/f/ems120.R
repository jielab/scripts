...
# Release the full phone copy before CV and disease inference.
rm(dat.phone.list)
invisible(gc())


# 🚩 4) dx CV: stratified 5-fold validation immediately before ml_dx.
# Reuse valid OOF predictions when available.
...
if (run_dx_cv) {
	if (all(file.exists(dx_cv_base_files))) {
		...
		# uncertainty/calibration outputs when they are missing.
...
if (!final.ready) {
	...
	for (year in years) {
		...
		# Count repeated phone values before disease eligibility filtering.
		dat <- dat %>%
...
# Build the figure from existing stratified five-fold OOF outputs.
...
# Reassemble Fig2 from the validated analytical objects.
...
# Panels test overall demand and disease-mix changes by phone-score group.
...
# Align each row independently to isolate long phenotype labels.
...
# 🚩 FigS2. EMS on-scene time and circular hourly mechanism
...
# Main Fig4 uses the two upper panels and the circular panel; dispatch-time data remain in Fig4.out.xlsx.
...
cv_sheets <- list()
# Keep FigS3 layout and workbook stable. CV diagnostics use a separate companion figure.
...
writexl::write_xlsx(list(
	...
	configuration = tibble(note = "FigS6 is the death-specific validation.")
), "FigS6.out.xlsx")
...
# 🚩 Supplementary-figure reassembly
# Capture component workbooks before final assembly overwrites their assigned names.
...
# 🚩 Final supplementary-figure assembly
# Remove component figures after their workbook content has been captured.
unlink(c(paste0("FigS", 0:13, ".png"), paste0("FigS", 0:13, ".out.xlsx")), force = TRUE)


# 🚩 FigS0. Annual AI-reconstructed EMS phenotype composition
pS0_final <- pS1 +
...
# 🚩 FigS1. EMS on-scene time and circular hourly mechanism
...
# 🚩 FigS5. Geographic phenotype validation
...
# 🚩 FigS6. March 2022 PHSM companion to main Fig5
pS5A_final <- p5A +
...
# 🚩 FigS7. Transformer phenotype overview
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(
		list(transformer = component_wb$S5),
		"FigS7 contains the transformer phenotype overview; keyword-vs-transformer concordance is workbook-only."
	),
	"FigS7.out.xlsx"
)


# 🚩 FigS8. Independent validation of MacBERT-identified Death calls
...
# Retain the keyword-composition support panel.
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(
		list(death = component_wb$S6),
		"FigS8 contains the keyword-assignment composition panel."
	),
	"FigS8.out.xlsx"
)


# 🚩 FigS9. Descriptive temporal EMS spectrum
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(list(temporal = component_wb$S7), "FigS2 contains the descriptive monthly and hourly temporal EMS spectrum."),
	"FigS2.out.xlsx"
)


# 🚩 FigS10. High-vs-low circadian and on-scene-time contrasts
if (FALSE) {
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(
		list(circadian = component_wb$S8, onsite = component_wb$S9),
		"FigS10 combines circadian contrasts with the circular on-scene-duration analysis."
	),
	"FigS10.out.xlsx"
)
}
...
# 🚩 FigS11. Robustness to phone-score definition
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(
		list(score_robustness = component_wb$S4),
...
# 🚩 FigS12. Period-level and quarterly gradient dynamics
...
writexl::write_xlsx(
	ems120_merge_xlsx_sources(list(period_gradient = component_wb$S10), "FigS10 contains period-level heatmaps and quarterly gradient dynamics."),
	"FigS10.out.xlsx"
)


# 🚩 FigS13. Dose-response, adjustment, and post-2020 robustness
pS12A_final <- pS11A +
...
figS12_wb <- ems120_merge_xlsx_sources(
	list(dose_adjustment = component_wb$S11),
...
# 🚩 Cleanup superseded TEST exports
