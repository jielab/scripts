...
# EMS120 FINAL end-to-end pipeline: TEST ML/CV front-end + final main/supplementary figure numbering.
# Default: reuse phone/geo caches, validate MacBERT with stratified 5-fold OOF CV
# when CV outputs are absent/outdated, rebuild the disease cache when its pipeline
# version is old, and regenerate main + compact supplementary figures.

START_STEP=${START_STEP:-all}
END_STEP=${END_STEP:-${STOP_AFTER_STEP:-}}
CONDA_ENV_NAME=${CONDA_ENV_NAME:-ai}
EMS120_RUN_DX_CV=${EMS120_RUN_DX_CV:-1}
...
Options:
  ...
  --skip-cv           skip MacBERT 5-fold cross-validation
...
Environment equivalents:
  START_STEP, END_STEP, CONDA_ENV_NAME, EMS120_RUN_DX_CV
EOF
}
...
# FINAL: use the normal EMS120 analysis directory by default; override externally if desired.
export EMS120_ANALYSIS_DIR="${EMS120_ANALYSIS_DIR:-/mnt/d/analysis/ems120}"
