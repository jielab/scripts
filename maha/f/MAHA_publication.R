...
forest_single <- function(d, title, xlab, ref = 1, estimate = "HR", low = "CI_low", high = "CI_high",
  ...
  if (!is.null(limits)) {
    ...
    p <- p + scale_x_continuous(expand = expansion(mult = c(.04, .38)))
  }
...
make_shared_score_figure <- function() {
  ...
  figS1 <- align_panel_rows(list(list(pu, pn, pc), list(hu, hn, hc)),
...
make_ukb <- function() {
  ...
  fig2 <- align_panel_rows(list(list(p1, p2), list(p3, p4), list(p5, p6)),
  ...
  figS2 <- align_panel_rows(list(list(pS2a, pS2b), list(pS2c, pS2d)),
                            rel_widths = c(1, 1), rel_heights = c(1, 1))
  save_pub(figS2, "FigS4.ukb.prospective_sensitivity.png", 16, 12.4)
  write_book(list(main_assoc = ukbmain, alternative_MAHA = alt,
                  ...
                  strata_counts = get_sheet(wb2, "Fig2_strata_counts")), "FigS4.ukb.prospective_sensitivity.out.xlsx")

  copy_if_exists(aux_path("ukb", "FigS4.phewas_head2head.png"), pub_path("FigS2.ukb.phewas_head2head.png"))
  copy_if_exists(aux_path("ukb", "FigS4.phewas_head2head.out.xlsx"), pub_path("FigS2.ukb.phewas_head2head.out.xlsx"))
  jr_path <- aux_path("ukb", "FigS5.joint_risk.out.xlsx")
  ...
  save_pub(fig_joint, "Figprimary.ukb.joint_risk.png", 13.5, 11.8, dpi = 400)
  write_book(list(joint_risk = jr), "Figprimary.ukb.joint_risk.out.xlsx")

  diag_png <- aux_path("ukb", "UKB_survival_diagnostics.png")
  diag_xlsx <- aux_path("ukb", "UKB_survival_diagnostics.out.xlsx")
  ...
  figprimary <- align_panel_rows(list(list(pprimarya, pprimaryb), list(pprimaryc, pprimaryd)), rel_widths = c(1, 1))
  save_pub(figprimary, "FigS6.ukb.construct_stress_tests.png", 16, 11.6)
  write_book(list(validation_summary = vs, direction_null = vn, leave_one_out = vl,
                  ...
                  convergent_validity = get_sheet(cp, "convergent_validity")), "FigS6.ukb.construct_stress_tests.out.xlsx")
...
make_nhanes <- function() {
  ...
  fig3 <- align_panel_rows(list(list(p1, p2), list(p3, p4), list(p5, p6)), rel_widths = c(1, 1))
  ...
  figS4 <- align_panel_rows(list(list(pS4a, pS4b), list(pS4c, pS4d)), rel_widths = c(1, 1))
  save_pub(figS4, "FigS7.nhanes.validation.png", 16, 11.6)
  write_book(list(summary = vs, direction_null = vn, leave_one_out = vl), "FigS7.nhanes.validation.out.xlsx")
  ...
  figS5 <- align_panel_rows(list(list(pS5a, pS5b), list(pS5c, pS5d)), rel_widths = c(1, 1))
  save_pub(figS5, "FigS8.nhanes.construct_concordance.png", 16, 12.1)
  write_book(list(decile_profile = get_sheet(cp, "decile_profile"), component_metrics = cm, convergent_validity = get_sheet(cp, "convergent_validity"),
                  score_distribution = sd, spearman_matrix = get_sheet(sc, "spearman_matrix")), "FigS8.nhanes.construct_concordance.out.xlsx")
...
make_chns <- function() {
  ...
  mort <- get_sheet(wb,"mortality") %>% mutate(Diet=normalize_chns_label(Diet))
  quart <- get_sheet(wb,"quartiles") %>% mutate(Diet=normalize_chns_label(Diet))
  prec <- get_sheet(wb,"precision") %>% mutate(Diet=normalize_chns_label(Diet))
  h2h <- get_sheet(wb,"head2head")
  ...
  main_order <- c("MAHA","DASH","MIND","MEDI","MAHA-balanced","MAHA-strict","MAHA-no dairy","MAHA-no protein")

  p1 <- forest_single(
    mort %>% filter(Diet %in% main_order),
    "a. Mortality associations","Adjusted HR per 1 SD healthier score",
    estimate="Effect",low="CI_low",high="CI_high",label="Diet",color="Diet",
    hollow=setdiff(maha_variants,"MAHA")
  )

  p2 <- forest_single(
    quart %>% filter(Diet %in% main_order),
    "b. Highest versus lowest quartile","Adjusted HR, Q4 versus Q1",
    estimate="Q4_vs_Q1",low="CI_low",high="CI_high",label="Diet",color="Diet",
    hollow=setdiff(maha_variants,"MAHA")
  )

  p3 <- forest_single(
    h2h %>% transmute(Diet=Comp2,Comparison,Ratio,LCI=LCI_ratio,UCI=UCI_ratio),
    "c. Direct coefficient comparison","HR ratio: MAHA / comparator",
    estimate="Ratio",low="LCI",high="UCI",label="Comparison",color="Diet"
  )

  p4 <- forest_single(
    prec %>% filter(Diet %in% maha_variants) %>%
      mutate(lab=paste0("TOST P=",ap_p_lab(TOST_p),"; BF01=",ap_bf_lab(BF01))),
    "d. Equivalence and Bayes evidence","HR with 90% CI",
    estimate="Effect",low="CI90_low",high="CI90_high",label="Diet",color="Diet",
    hollow=setdiff(maha_variants,"MAHA"),right_label="lab",zone=c(1/1.05,1.05)
  )

  fig4 <- align_panel_rows(list(list(p1,p2),list(p3,p4)),rel_widths=c(1,1))
  save_pub(fig4,"Fig4.chns.mortality.png",16,11.8)
  out4 <- list(mortality=mort,quartiles=quart,precision=prec,head2head=h2h)
  ...
  figS9 <- align_panel_rows(list(list(pS9a,pS9b),list(pS9c,pS9d)),rel_widths=c(1,1))
  save_pub(figS9,"FigS9.chns.validation.png",16,12.1)
  write_book(list(summary=vs,direction_null=vn,leave_one_out=vl,
             ...
             "FigS9.chns.validation.out.xlsx")
  ...
  figE <- align_panel_rows(list(list(pEa,pEb),list(pEc,pEd)),rel_widths=c(1,1))
  ...
  save_pub(figS10,"FigS10.chns.mortality_sensitivity.png",10.5,7.5)
  ...
  ),"FigS10.chns.mortality_sensitivity.out.xlsx")
