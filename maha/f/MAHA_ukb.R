...
p_map <- wrap_plots(
	ggplot() + annotate("text", .5, .5, label = "a. MAHA-DASH discordance", fontface = "bold", size = fs_title / 3) +
...
p_decomp <- ggplot(geo_plot, aes(y=Diet)) +
	...
	labs(title="b. Socioeconomic vs spatial variation", x="Variance explained (R2, %)", y=NULL, color=NULL) + theme_classic(base_size=fs_text) +
...
p_fu <- ggplot(followup_common, aes(x = ReverseKM_median, y = Outcome, color = Outcome)) +
  ...
  geom_point(size = 2.8) +
  geom_text(aes(label = Followup_label), hjust = -0.08, size = 3.1, fontface = "bold") +
...
save_plot(Fig_survdiag, "UKB_survival_diagnostics.png", width = 15.5, height = 11.0, dpi = 500, bg = "white")
write_xlsx(
  list(
  ...
  "UKB_survival_diagnostics.out.xlsx"
)
...
maha_run_step("figS2", {
step_header("Figure S4: head-to-head comparison of PheWAS")
...
p2 <- ggplot() +
	...
	geom_text_repel(data = lab_b, aes(beta.DASH, beta.MAHA, label = description, color = sig_pattern), size = 2.8, fontface = "bold", box.padding = 0.45, point.padding = 0.25, force = 1.4, segment.alpha = 0.5, max.overlaps = 12, show.legend = FALSE) +
	scale_color_manual(values = cols) + scale_shape_manual(values = c("Both" = 16, "DASH only" = 17, "MAHA only" = 15)) +
...
save_plot(FigS2, "FigS4.phewas_head2head.png", width = 16, height = 16, dpi = 500, bg = "white")
write_xlsx(list(phewas_compare = dm2, label_p = lab_p, label_beta = lab_b), "FigS4.phewas_head2head.out.xlsx")
print(dm2, n = 30)
print(lab_p)
print(lab_b)
})
...
maha_run_step("figS3", {
step_header("Figure S5: 10-year joint DASH and MAHA categories")
...
write_xlsx(tab2.risk.out, "FigS5.joint_risk.out.xlsx")
...
save_plot(FigS3, "FigS5.joint_risk.png", width = 8, height = 8, dpi = 320)
print(tab2.risk.out)
})
