...
term_result <- function(fit, term, exponentiate = TRUE) {
	tt <- tryCatch(broom::tidy(fit), error = function(e) NULL)
	if (is.null(tt)) return(NULL)
	tt <- tt %>% filter(.data$term == term)
	...
	lo <- beta - 1.96 * se
	hi <- beta + 1.96 * se
	if (exponentiate) tibble(beta = beta, se = se, estimate = exp(beta), conf.low = exp(lo), conf.high = exp(hi), p.value = p)
	else tibble(beta = beta, se = se, estimate = beta, conf.low = lo, conf.high = hi, p.value = p)
}
...
combine_day <- function(d1, d2, prefix) {
	...
	for (b in base) d[[b]] <- rowmean2(d[[paste0(b, "_1")]], d[[paste0(b, "_2")]])
...
read_cycle <- function(cycle, suffix, prefix, period_years) {
	...
	base <- base %>%
		mutate(
			...
			wt_diet = coalesce(wt_diet, wt_mec, wt_int)
		)
