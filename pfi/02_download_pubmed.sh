...
usage() {
  cat <<'HELP'
Usage: ./02_download_pubmed.sh [options]

Options:
  --analysis-root DIR       Analysis root. Default: /mnt/d/analysis/pfi.
  --raw-root DIR            Raw archive root. Default: /mnt/g/pfi.
  --python FILE             Python interpreter. Default: python.
  --download-tool TOOL      aria2 or wget. Default: aria2.
  --concurrent-downloads N  Concurrent aria2 downloads. Default: 2.
  --connections-per-server N Default: 1.
  --split N                 aria2 splits per file. Default: 1.
  --min-split-size SIZE     Default: 20M.
  --max-tries N             Default: 20.
  --retry-wait SEC          Default: 30.
  --timeout SEC             Default: 60.
  --connect-timeout SEC     Default: 30.
  --test-mode TRUE|FALSE    Download only three baseline/update files. Default: FALSE.
...
ARIA2_MAX_CONNECTION_PER_SERVER=1
ARIA2_SPLIT=1
ARIA2_MIN_SPLIT_SIZE=20M
PFI_DOWNLOAD_MAX_TRIES=20
PFI_DOWNLOAD_RETRY_WAIT=30
PFI_DOWNLOAD_TIMEOUT=60
PFI_DOWNLOAD_CONNECT_TIMEOUT=30
PFI_TEST_MODE=0

while (( $# )); do
  case "$1" in
    --analysis-root) ANALYSIS_ROOT=${2:?ERROR: --analysis-root requires DIR}; shift 2 ;;
    --raw-root) RAW_ROOT=${2:?ERROR: --raw-root requires DIR}; shift 2 ;;
    --python) PFI_PYTHON_BIN=${2:?ERROR: --python requires FILE}; shift 2 ;;
    --download-tool) PFI_DOWNLOAD_TOOL=${2:?ERROR: --download-tool requires aria2 or wget}; shift 2 ;;
    --concurrent-downloads) ARIA2_MAX_CONCURRENT_DOWNLOADS=${2:?ERROR: --concurrent-downloads requires N}; shift 2 ;;
    --connections-per-server) ARIA2_MAX_CONNECTION_PER_SERVER=${2:?ERROR: --connections-per-server requires N}; shift 2 ;;
    --split) ARIA2_SPLIT=${2:?ERROR: --split requires N}; shift 2 ;;
    --min-split-size) ARIA2_MIN_SPLIT_SIZE=${2:?ERROR: --min-split-size requires SIZE}; shift 2 ;;
    --max-tries) PFI_DOWNLOAD_MAX_TRIES=${2:?ERROR: --max-tries requires N}; shift 2 ;;
    --retry-wait) PFI_DOWNLOAD_RETRY_WAIT=${2:?ERROR: --retry-wait requires SEC}; shift 2 ;;
    --timeout) PFI_DOWNLOAD_TIMEOUT=${2:?ERROR: --timeout requires SEC}; shift 2 ;;
    --connect-timeout) PFI_DOWNLOAD_CONNECT_TIMEOUT=${2:?ERROR: --connect-timeout requires SEC}; shift 2 ;;
    --test-mode)
      case "${2,,}" in true|1|yes) PFI_TEST_MODE=1;; false|0|no) PFI_TEST_MODE=0;; *) echo "ERROR: --test-mode requires TRUE or FALSE" >&2; exit 2;; esac
      shift 2
      ;;
    -h|--help) usage; exit 0 ;;
...
case "$PFI_DOWNLOAD_TOOL" in aria2|wget) ;; *) echo "ERROR: --download-tool must be aria2 or wget" >&2; exit 2;; esac
export ANALYSIS_ROOT RAW_ROOT PFI_PYTHON_BIN PFI_TEST_MODE
...
PY
  ...
  if [[ "$PFI_DOWNLOAD_TOOL" == "aria2" && -n "$(command -v aria2c || true)" ]]; then
    ...
    echo "Using wget downloader. Select it explicitly with: ./02_download_pubmed.sh --download-tool wget"
    local wget_args=(
      --continue
...
if [[ "$PFI_TEST_MODE" == "1" ]]; then
  limit=3
  echo "Test mode: downloading only the first 3 baseline/update files."
fi
