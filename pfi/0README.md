...
bash 01_install_system_tools.sh
bash 01_create_conda_env.sh
conda activate pfi
```
...
```bash
export PFI_PYTHON_BIN=/home/huangj/anaconda3/envs/pfi/bin/python
export PFI_RSCRIPT_BIN=Rscript
```
...
```text
MIN_YEAR=2000
PFI_FULL_TEXT_ONLY=1
PFI_REVIEW_ROWS=1000
PFI_PRED_SAMPLE_ROWS=1000000
PFI_PRED_SAMPLE_SEED=1
PFI_MAX_TRAIN_ROWS=1000
PFI_PUBMEDBERT_MODEL=/mnt/d/data.BIG/AI_model_files/pubmedbert_abstract_fulltext
PFI_PUBMEDBERT_FALLBACK=microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext
PFI_ALLOW_CPU_FULL_PRED=0
```
...
# Optional if conda activation does not expose the right binaries.
export PFI_PYTHON_BIN=/home/huangj/anaconda3/envs/pfi/bin/python
export PFI_RSCRIPT_BIN=Rscript
```
...
`articles_master` by setting `PFI_ALLOW_PROXY_JOURNAL_RANK=1`. Do not use proxy
rank results for final interpretation.
...
```bash
export PFI_REVIEW_ROWS=1000
export PFI_MAX_TRAIN_ROWS=1000
export PFI_PRED_SAMPLE_ROWS=1000000
export PFI_PRED_LIMIT=0
unset PFI_ALLOW_PROXY_JOURNAL_RANK

bash 00_run_pfi_after08_auto.sh
```

If you do not yet have real JCR/JCI data and only want to test the pipeline:

```bash
export PFI_REVIEW_ROWS=100
export PFI_MAX_TRAIN_ROWS=100
export PFI_PRED_SAMPLE_ROWS=10000
export PFI_PRED_LIMIT=10000
export PFI_ALLOW_PROXY_JOURNAL_RANK=1
...
bash 08b_normalize_journal_rank.sh /mnt/d/scripts/pfi/journal_rank_real.csv
...
bash 09a_prepare_pfi_training_dataset.sh \
  /mnt/d/analysis/pfi/dat/journal_rank_normalized.parquet \
  /mnt/d/analysis/pfi/dat/paper_score_review_1000_scored.csv
...
bash 12_audit_strong_outliers.sh
```

### Useful controls

```bash
# Review sampling speed/coverage. Default is max(PFI_REVIEW_ROWS*2000, 200000).
export PFI_REVIEW_MAX_SCAN_ROWS=200000

# Prediction size. Use 0 for no hard limit.
export PFI_PRED_SAMPLE_ROWS=1000000
export PFI_PRED_LIMIT=0

# Rebuild the prediction sample instead of reusing an existing sample parquet.
export PFI_PRED_FRESH_SAMPLE=1

# Optional LLM review batch after features exist.
bash 11_sample_llm_review_batch.sh 1000
```
