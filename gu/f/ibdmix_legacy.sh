...
[[ -s $phe_sh ]] || { echo "ERROR missing shared helper file: $phe_sh" >&2; exit 1; }
source "$phe_sh"
...
case "$action" in
    ...
    ibdmix_check)
        prepare_samples
        validate_inputs
        log "CHECK PASSED: target_samples=$(count_lines "$dirout/samples/ALL.txt") populations=$(pop_list_for_ref Altai | wc -l) analysis_units=${analysis_units[*]}"
        ;;
	ibdmix_igv)
