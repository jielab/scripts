...
ARG_DIR=${GU_ARG_DIR:-${GU_TARGET_ROOT:-.}/arg.tsinfer}; TARGET_VCF_DIR=${GU_TARGET_VCF_DIR:-}
...
for value in "$GROUP_SIZE" "$JEX" "$JIN" "$JSUM"; do
  [[ $value =~ ^[1-9][0-9]*$ ]] || { echo "ERROR: TRACE group size and worker counts must be positive integers" >&2; exit 2; }
done
# One native-library thread per worker; parallelism comes from the job counts.
...
exec 9> "$OUT/.run.lock"
flock -n 9 || { echo "ERROR: another TRACE run is using $OUT" >&2; exit 1; }
...
if tree.suffix == ".tsz":
    ...
    import tskit
    ts = tskit.load(tree)
if isinstance(ts.metadata, dict) and ts.metadata.get('method') == 'singer' and ts.metadata.get('regional'):
    raise SystemExit('ERROR: regional SINGER test ARG cannot be used as full-chromosome TRACE evidence')
...
prepare_regions(){
  local c out
  if [[ $TRACE_ARG_METHOD == singer ]]; then
    for c in $CHRS; do
      out=$OUT/regions/chr$c.bed
      python3 - "$ARG_DIR/chr$c.inferred.bed" "$LOCI_FILE" "$LOCI_MODE" "$c" "$out" <<'PYMASK'
import sys
from pathlib import Path
mask,loci,mode,c,out=sys.argv[1:]
def read(path):
    rows=[]
    for line in Path(path).read_text().splitlines():
        if not line.strip() or line.startswith('#'):continue
        r=line.split()
        if r[0].removeprefix('chr')==c:rows.append((int(r[1]),int(r[2])))
    return rows
regions=read(mask)
if mode=='extract' and loci:
    regions=[(max(a,x),min(b,y)) for a,b in regions for x,y in read(loci) if max(a,x)<min(b,y)]
if not regions:raise SystemExit('ERROR: no SINGER inferred windows overlap requested regions')
Path(out).write_text(''.join(f'chr{c}\t{a}\t{b}\n' for a,b in sorted(regions)))
PYMASK
    done
    return
  fi
  [[ -n $LOCI_FILE && $LOCI_MODE == extract ]] || return 0
  [[ -s $LOCI_FILE ]] || { echo "ERROR: TRACE loci file missing: $LOCI_FILE" >&2; exit 1; }
  for c in $CHRS; do out=$OUT/regions/chr$c.bed; awk -F'\t' -v c="$c" 'BEGIN{OFS="\t"}$1==c{print "chr"c,$2,$3}' "$LOCI_FILE" > "$out"; [[ -s $out ]] || return 1; done
}
prepare_sample_map(){
  ...
  for chr in $CHRS; do
    ...
    if [[ -n $vcf ]]; then
      ...
      if [[ $TRACE_ARG_METHOD == needle || $TRACE_ARG_METHOD == threads || $TRACE_ARG_METHOD == singer ]]; then
...
check_run_provenance(){
  ...
  local candidate=$OUT/run.meta.next.tsv
  run_record > "$candidate"
  # Resource settings may change on resume; scientific settings and group
  # size may not. Historical records also contain literal \t stat lines.
  if [[ -s $current && $REPLACE == 0 ]]; then
    if ! diff -q <(head -9 "$current") <(head -9 "$candidate") >/dev/null; then
      rm -f "$candidate"
      echo "ERROR: TRACE parameters changed; use --replace-trace TRUE or restore the original settings" >&2
      return 1
    fi
  fi
...
extract_one(){
  ...
  prefix=$OUT/extract/${group}.chr${c}.p${post}; ids=$(paste -sd, "$gf"); npost=$(awk -v c="$c" 'NR>1&&$1==c{n++}END{print n+0}' "$OUT/manifest/tree_files.tsv")
  ...
  python3 "$F/trace_output.py" --prefix "$prefix" --suffix .npz -- trace-extract --tree-file "$tree" -t "$TARC" --individuals "$ids" "${opts[@]}" 2>&1 | tee "$OUT/log/extract.${group}.chr${c}.p${post}.log"
}
run_extract(){
  local status=0 gf group c post tree pid; local -a pids=()
  for gf in "$OUT"/samples/groups/*.nodes; do group=$(basename "$gf" .nodes); while IFS=$'\t' read -r c post tree; do [[ $c == chr ]] && continue; extract_one "$c" "$gf" "$group" "$post" "$tree" & pids+=("$!"); if (( ${#pids[@]} >= JEX )); then wait "${pids[0]}" || status=1; pids=("${pids[@]:1}"); fi; done < "$OUT/manifest/tree_files.tsv"; done
  for pid in "${pids[@]}"; do wait "$pid" || status=1; done; (( status == 0 )) || exit 1
...
infer_one(){
  local node=$1 group=$2 c npost gmap C D G out any_multi=0; local -a chroms=() datafiles=() npzs=() gmaps=() opt=()
  ...
  local -a suffix_args=(); for c in $CHRS; do suffix_args+=(--suffix ".chr${c}.xss.npz"); done
  C=$(IFS=,;echo "${chroms[*]}")
  ...
  python3 "$F/trace_output.py" --prefix "$out" "${suffix_args[@]}" -- trace-infer -i "$node" "${opt[@]}" --chroms "$C" 2>&1 | tee "$OUT/log/infer.hap${node}.log"
}
run_infer(){ [[ -s $OUT/samples/node_group.tsv ]] || run_extract; local status=0 node group pid; local -a pids=(); while IFS=$'\t' read -r node group; do infer_one "$node" "$group" & pids+=("$!"); if (( ${#pids[@]} >= JIN )); then wait "${pids[0]}" || status=1; pids=("${pids[@]:1}"); fi; done < "$OUT/samples/node_group.tsv"; for pid in "${pids[@]}"; do wait "$pid" || status=1; done; (( status == 0 )) || exit 1; }
summarize_one(){ local node=$1 c f out FSTR CSTR; local -a files=() chroms=(); out=$OUT/calls/hap$node; [[ -s $out.summary.txt ]] && return 0; for c in $CHRS; do f=$OUT/infer/hap${node}.chr${c}.xss.npz; [[ -s $f ]] || return 1; files+=("$f"); chroms+=("chr$c"); done; FSTR=$(IFS=,;echo "${files[*]}"); CSTR=$(IFS=,;echo "${chroms[*]}"); trace-summarize -f "$FSTR" -c "$CSTR" --posterior-threshold "$POST" --physical-length-threshold "$MINBP" --genetic-distance-threshold "$MINCM" -o "$out.part" 2>&1 | tee "$OUT/log/summarize.hap${node}.log" && [[ -s $out.part.summary.txt ]] && mv "$out.part.summary.txt" "$out.summary.txt"; }
run_segments(){
  [[ -s $OUT/samples/tree_nodes.txt ]] || run_extract; local status=0 node pid; local -a pids=(); while read -r node; do summarize_one "$node" & pids+=("$!"); if (( ${#pids[@]} >= JSUM )); then wait "${pids[0]}" || status=1; pids=("${pids[@]:1}"); fi; done < "$OUT/samples/tree_nodes.txt"; for pid in "${pids[@]}"; do wait "$pid" || status=1; done; (( status == 0 )) || exit 1
