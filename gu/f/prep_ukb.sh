#!/usr/bin/env bash
# Prepare and inspect UK Biobank raw BGEN/array inputs for GU.
set -euo pipefail
...
case "$ACTION" in
  inspect-hap)
    ...
    inspect_file="$OUT/hap.inspect.tsv"
    ...
    for c in $CHRS; do
      ...
      printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$c" "$ns" "$nv" "$h" "$b" "$s" | tee -a "$inspect_file"
    done
    echo "PASS: selected chromosomes have identical sample ID order. Inspection: $inspect_file"
    ;;
...
PY
  ...
  batches)
    : "${GU_SAMPLE_PANEL:?use --sample-panel FILE; run ./gu.sh ukb make-panel first or supply an ancestry-labelled panel}"
  ...
  hap-vcf)
    ...
    : "${UKB_KEEP:?use --ukb-keep FILE; exporting all ~487k samples to VCF is intentionally blocked}"
    [[ -s $UKB_KEEP ]] || die "missing UKB_KEEP=$UKB_KEEP"
    if [[ -z $REF_FASTA || ! -s $REF_FASTA ]]; then
      die "use --ukb-ref-fasta with a build-matched GRCh37 FASTA"
    fi
    ODIR=${UKB_VCF_OUT:-$OUT/hap_vcf}; mkdir -p "$ODIR"
    exec 8>"$ODIR/.prepare.lock"
    flock -n 8 || die "another hap-vcf run uses $ODIR"
    keep_tmp=$(mktemp "${TMPDIR:-/tmp}/gu-ukb-keep.XXXXXX")
    stage=$(mktemp -d "$ODIR/.export.XXXXXX")
    trap 'rm -f "$keep_tmp"; rm -rf -- "$stage"' EXIT
    trap 'exit 130' INT
    trap 'exit 143' TERM
    trap 'exit 129' HUP
    ...
    for c in $CHRS; do
      ...
      final="$ODIR/chr$c"
      cache=(--receipt "$final.complete.json" --input "$b" --input "$s" --input "$UKB_KEEP" --input "$REF_FASTA" --input "$0" --input "$(command -v plink2)" --value "chr=$c;GRCh=${GU_BUILD:-37};ref-from-fa;phased-biallelic-SNP" --output "$final.vcf.gz" --output "$final.vcf.gz.tbi" --output "$final.qc.tsv")
      if python3 "$F/genetic_cache.py" check "${cache[@]}"; then echo "SKIP verified hap-vcf chr$c"; continue; fi
      rm -f "$final.complete.json"
      p="$stage/chr$c"
      cmd=(plink2 --threads "$THREADS" --memory "${GU_PREP_MEMORY_MB:-2048}" --bgen "$b" ref-unknown --sample "$s" --oxford-single-chr "$c" --keep "$keep_tmp" --snps-only just-acgt --max-alleles 2)
      ...
      for suffix in vcf.gz vcf.gz.tbi qc.tsv; do mv -f -- "$p.$suffix" "$final.$suffix"; done
      python3 "$F/genetic_cache.py" record "${cache[@]}"
      echo "chr$c phased VCF: $final.vcf.gz samples=$observed variants=$nv"
    done
    ;;

  hap-arg-vcf)
    : "${UKB_KEEP:?use --ukb-keep with the joint target+anchor sample list}"
    : "${UKB_1KG_VCF_DIR:?use --ukb-1kg-vcf-dir with a build-matched 1KG VCF directory containing INFO/AA}"
    AAOUT=${UKB_ARG_VCF_OUT:-$OUT/hap_vcf.aa}
    mkdir -p "$AAOUT"
    exec 9>"$AAOUT/.prepare.lock"
    flock -n 9 || die "another hap-arg-vcf run uses $AAOUT"
    RAW=$(mktemp -d "$AAOUT/.raw.XXXXXX")
    trap 'rm -rf -- "$RAW"' EXIT
    trap 'exit 130' INT
    trap 'exit 143' TERM
    trap 'exit 129' HUP
    for c in $CHRS; do
      ...
      b=$(bgen_for "$c"); s=$(sample_for "$c")
      final="$AAOUT/chr$c"
      cache=(--receipt "$final.complete.json" --input "$b" --input "$s" --input "$UKB_KEEP" --input "$REF_FASTA" --input "$ref" --input "$0" --input "$F/add_aa_from_1kg.sh" --input "$(command -v plink2)" --value "chr=$c;GRCh=${GU_BUILD:-37};exact-AA" --output "$final.vcf.gz" --output "$final.vcf.gz.tbi" --output "$final.aa_qc.tsv")
      if python3 "$F/genetic_cache.py" check "${cache[@]}"; then echo "SKIP verified hap-arg-vcf chr$c"; continue; fi
      rm -f "$final.complete.json"
      GU_CHRS="$c" UKB_VCF_OUT="$RAW" bash "$0" hap-vcf
      bash "$F/add_aa_from_1kg.sh" --in "$RAW/chr$c.vcf.gz" --ref "$ref" --out "$final.vcf.gz" --threads "$THREADS"
      python3 "$F/genetic_cache.py" record "${cache[@]}"
