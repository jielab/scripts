...
phe_fun_file <- c("D:/scripts/0f/0phe.f.R", "/mnt/d/scripts/0f/0phe.f.R")
phe_fun_file <- phe_fun_file[file.exists(phe_fun_file)][1]
if (!is.na(phe_fun_file)) source(phe_fun_file)
# Inlined helper functions for gu.R
usage <- function() {
    stop(paste("Usage:",
        "  Rscript gu.R prep_input --dirgwas DIR [--dircojo DIR] --dirout DIR --traits trait ...",
        "  Rscript gu.R prep_input_local --dirgwas DIR --dirout DIR --dirmod DIR [--ref_pop ALL|EUR] --traits trait ...",
        "  Rscript gu.R add_positive_loci --dirgwas DIR --dirmod DIR --dirout DIR --bed BED --trait TRAIT [--ref_pop ALL|EUR]",
        "  Rscript gu.R prep_archaic kg.vcf.gz archaic.raw.vcf.gz out.vcf.gz sample_name",
        "  Rscript gu.R make_hap RES_DIR [trait | trait id | merge]",
        "  Rscript gu.R filter_hap RES_DIR sample.tsv POP MAX_COUNT",
        "  Rscript gu.R make_phy RES_DIR",
        "  Rscript gu.R make_tree RES_DIR",
        "  Rscript gu.R asnp --dirgwas DIR --dirout DIR --dirmod DIR --asnp aSNPs.haplotypes.v1.tsv --traits trait ...",
        "  Rscript gu.R extra_qc --dirout_root DIR --outdir DIR --traits trait ...",
        "  Rscript gu.R proxy_assoc make_scores|fit_models ...",
        sep = "\n"), call. = FALSE)
}
...
pvar_scan_timeout <- function() Sys.getenv("pvar_scan_timeout", "10m")

require_slim_pvar <- function() Sys.getenv("require_slim_pvar", "1") %in% c("1", "true", "TRUE", "yes", "YES")

dir_report <- Sys.getenv("GU_REPORT_DIRNAME", unset = "report")
if (!nzchar(dir_report)) dir_report <- "report"
dir_plot <- Sys.getenv("GU_PLOT_DIRNAME", unset = "plot")
if (!nzchar(dir_plot)) dir_plot <- "plot"

gu_plot_width_inch <- {
    x <- suppressWarnings(as.numeric(Sys.getenv("GU_PLOT_WIDTH_INCH", "8.2")))
    if (is.finite(x) && x > 0) x else 8.2
}

gu_scaled_height_inch <- function(width_units, height_units, plot_width = gu_plot_width_inch) {
    width_units <- suppressWarnings(as.numeric(width_units))
    height_units <- suppressWarnings(as.numeric(height_units))
    if (!is.finite(width_units) || width_units <= 0 ||
        !is.finite(height_units) || height_units <= 0) {
        return(plot_width)
    }
    plot_width * height_units / width_units
}

with_shell_timeout <- function(cmd, timeout = "") {
    if (!nzchar(timeout) || timeout %in% c("0", "none")) return(cmd)
    timeout_bin <- Sys.which("timeout")
    if (!nzchar(timeout_bin)) return(cmd)
    paste(shQuote(timeout_bin), shQuote(timeout), cmd)
}

run_shell_checked <- function(cmd, timeout = "", label = "command", intern = FALSE) {
    cmd2 <- with_shell_timeout(cmd, timeout)
    if (isTRUE(intern)) {
        out <- tryCatch(suppressWarnings(system(cmd2, intern = TRUE)), error = function(e) {
            structure(character(), status = 1L, stderr = conditionMessage(e))
        })
        status <- attr(out, "status")
        if (!is.null(status) && status != 0) {
            stop(label, " failed or timed out; status=", status, "; timeout=", ifelse(nzchar(timeout), timeout, "none"), call. = FALSE)
        }
        return(out)
    }
    rc <- system(cmd2)
    if (!identical(rc, 0L)) {
        stop(label, " failed or timed out; status=", rc, "; timeout=", ifelse(nzchar(timeout), timeout, "none"), call. = FALSE)
    }
    invisible(rc)
}

check_pvar_format <- function(f, label = "pvar") {
    if (!require_slim_pvar()) return(invisible(TRUE))
    cmd <- sprintf("awk '/^#CHROM/{print NF; exit}' %s", shQuote(f))
    z <- run_shell_checked(cmd, timeout = pvar_scan_timeout(), label = paste(label, "header scan", f), intern = TRUE)
    nf <- suppressWarnings(as.integer(z[1]))
    if (is.na(nf)) stop("Malformed pvar header: ", f, call. = FALSE)
    if (nf > 5L) {
        stop("Fat pvar detected: ", f, " has ", nf,
             " columns. Regenerate a 5-column pvar (#CHROM POS ID REF ALT) or set require_slim_pvar=0.",
             call. = FALSE)
    }
    invisible(TRUE)
}
...
run_prep_input <- function(script_args) {
    ...
    if(is.null(dirgwas) || is.null(dirout) || is.null(traits)) stop("usage: Rscript gu.R prep_input --dirgwas DIR [--dircojo DIR] --dirout DIR --traits trait ...")
    ...
    one <- function(tr){
# Local layout used by gu.sh: DIR/cojo/<trait>/<trait>.jma.cojo and DIR/clean/<trait>/<trait>.gz.
# Older COJO layouts are kept as fallbacks when --dircojo is supplied.
    	...
    	leadtsvf <- if (length(traits) == 1L && basename(normalizePath(dirout, winslash = "/", mustWork = FALSE)) == tr) file.path(lead0, "lead.tsv") else file.path(lead0, paste0(tr, ".lead.tsv"))
    	fwrite(lead, leadtsvf, sep = "\t")
    	...
    	if(nrow(bad)) {
    		failf <- if (length(traits) == 1L && basename(normalizePath(dirout, winslash = "/", mustWork = FALSE)) == tr) file.path(lead0, "lead.fail.tsv") else file.path(lead0, paste0(tr, ".lead.fail.tsv"))
    		fwrite(bad, failf, sep = "\t")
    	...
    	assocf <- if (length(traits) == 1L && basename(normalizePath(dirout, winslash = "/", mustWork = FALSE)) == tr) file.path(lead0, "lead.assoc") else file.path(lead0, paste0(tr, ".lead.assoc"))
    	fwrite(out, assocf, sep = "\t")
    	out
    }
...
run_prep_input_local <- function(script_args) {
    ...
    need_pkg("data.table")
    fphe <- c("/mnt/d/scripts/0f/0phe.f.R", "D:/scripts/0f/0phe.f.R")
    fphe <- fphe[file.exists(fphe)][1]
    if (!is.na(fphe))
        source(fphe)
    args <- commandArgs(TRUE)
    arg <- function(k, multi = FALSE) {
    ...
    pvar_files <- function(dirmod, chr) {
        if (chr == 23L) {
# Return all plausible chrX pvar shards.  The shell chooses PAR/nonPAR
# by lead position; here we read both so ID/BP alignment is consistent
# regardless of whether the lead is in PAR or nonPAR.
            if (ref_pop == "ALL") unique(c(
                paste0(dirmod, "/ALL.male.chrX.par.pvar"),
                paste0(dirmod, "/ALL.male.chrX.nonPar.pvar"),
                paste0(dirmod, "/ALL.chrX.par.pvar"),
                paste0(dirmod, "/ALL.chrX.nonPar.pvar"),
                paste0(dirmod, "/ALL.chrX.pvar"),
                paste0(dirmod, "/chrX.pvar")
            ))
            else unique(c(paste0(dirmod, "/", ref_pop, ".male.chrX.par.pvar"), paste0(dirmod, "/", ref_pop, ".male.chrX.nonPar.pvar"),
                paste0(dirmod, "/", ref_pop, ".chrX.par.pvar"), paste0(dirmod, "/", ref_pop, ".chrX.nonPar.pvar"),
                paste0(dirmod, "/", ref_pop, ".chrX.pvar")))
        } else {
            if (ref_pop == "ALL") unique(c(paste0(dirmod, "/ALL.chr", chr, ".pvar"), paste0(dirmod, "/chr", chr, ".pvar")))
    ...
    pvar_ids <- function(f, n = 2000) {
        if (!file.exists(f))
            return(character())
        check_pvar_format(f, "pvar ID-mode")
        cmd <- sprintf("awk '/^#/{next} $3!=\".\" && $3!=\"\"{print $3; if(++n==%d) exit}' %s",
            as.integer(n), shQuote(f))
        run_shell_checked(cmd, timeout = pvar_scan_timeout(), label = paste("pvar ID-mode scan", f), intern = TRUE)
    }
    ...
    rd_pvar <- function(dirmod, lead) {
        ...
        for (chr in sort(unique(lead$lead_chr))) {
            ...
            for (f in fs) {
                check_pvar_format(f, "pvar lookup")
                awk_script <- paste0(
                  "BEGIN{FS=OFS=\"\\t\"} ",
                ...
                out_tmp <- tempfile()
                cmd <- sprintf("awk %s %s %s > %s", shQuote(awk_script),
                  shQuote(tmp), shQuote(f), shQuote(out_tmp))
                run_shell_checked(cmd, timeout = pvar_scan_timeout(), label = paste("pvar lookup scan", f))
                x <- fread(out_tmp, header = TRUE, showProgress = FALSE)
                unlink(out_tmp)
                if (nrow(x) <= 0)
                  next
    ...
    align_1kg <- function(z, dirmod, lead0, tr) {
        ...
        fwrite(data.table(source = c("COJO_lead", "1000G_pvar"), id_mode = c(cojo_mode, mode),
            ...
            lead_out_file(tr, "id_sanity.tsv"), sep = "\t")
    ...
    if (is.null(dirgwas) || is.null(dirout) || is.null(traits))
        stop("Usage: Rscript gu.R prep_input --dirgwas DIR --dirout DIR --dirmod 1KG_DIR --traits bald bald12 ...")
    ...
    dir.create(lead0, recursive = TRUE, showWarnings = FALSE)
    single_trait_out <- length(traits) == 1L && basename(normalizePath(dirout, winslash = "/", mustWork = FALSE)) == traits[1]
    lead_out_file <- function(tr, stem) {
        if (single_trait_out) file.path(lead0, stem) else file.path(lead0, paste0(tr, ".", stem))
    }
    all_match <- list()
    ...
    for (tr in traits) {
        ...
        z <- z[!is.na(pvar_id)][, .(trait = tr, lead_chr, lead_snp, lead_bp, effect_allele = EA,
            ...
            match_type_1kg)]
        fwrite(z[, .(lead_chr, lead_snp, lead_bp)], lead_out_file(tr, "lead.3col"), sep = "\t")
        if (nrow(fail))
            fwrite(fail, lead_out_file(tr, "lead.fail.tsv"), sep = "\t")
        else unlink(lead_out_file(tr, "lead.fail.tsv"))
        fwrite(z, lead_out_file(tr, "lead.assoc"), sep = "\t")
    }
    fwrite(rbindlist(lapply(traits, function(tr) fread(lead_out_file(tr, "lead.assoc"))),
...
run_make_hap <- function(script_args) {
    ...
    root_arg <- if (has_root) args0[1] else Sys.getenv("BALD_RES", "")
    if (!nzchar(root_arg))
        stop("make_hap requires RES_DIR as the first argument or BALD_RES to be set")
    root <- normalizePath(root_arg, winslash = "/", mustWork = FALSE)
    args <- if (has_root)
        args0[-1]
    else args0
    ...
    report0 <- file.path(root, dir_report)
    single_trait_root <- function(tr) basename(root) == tr && exists("risk") && "trait" %in% names(risk) && tr %in% risk$trait
    trait_dir <- function(base, tr) {
        d <- file.path(base, tr)
        if (dir.exists(d)) d else if (single_trait_root(tr)) base else d
    }
    ...
    base_set <- c("A", "C", "G", "T")
    pick_lineage_method <- Sys.getenv("PICK_LINEAGE_METHOD", "avcf")
    hap_filter_method <- Sys.getenv("HAP_FILTER_METHOD", "avcf")
    ...
    # Strict-style diagnostics are now calculated inside the avcf workflow.
    # They are reported as columns in region_summary.tsv and inherited_region.tsv;
    # they are not used to drop loci unless the user filters the TSV afterward.
    strict_p_ils_th <- suppressWarnings(as.numeric(Sys.getenv("STRICT_P_ILS_TH", "0.1")))
    strict_min_compared_risk <- suppressWarnings(as.integer(Sys.getenv("STRICT_MIN_COMPARED_RISK", "2")))
    strict_min_match_risk <- suppressWarnings(as.integer(Sys.getenv("STRICT_MIN_MATCH_RISK", "2")))
    strict_prop_match_risk_th <- suppressWarnings(as.numeric(Sys.getenv("STRICT_PROP_MATCH_RISK_TH", "0.5")))
    strict_hap_min_n <- suppressWarnings(as.integer(Sys.getenv("STRICT_HAP_MIN_N", "10")))
    strict_require_carry_risk <- Sys.getenv("STRICT_REQUIRE_CARRY_RISK", "1") %in% c("1", "true", "TRUE", "yes", "YES")
    strict_yri_freq_th <- suppressWarnings(as.numeric(Sys.getenv("STRICT_YRI_FREQ_TH", as.character(yri_freq_th))))
    if (!is.finite(strict_p_ils_th)) strict_p_ils_th <- 0.1
    if (is.na(strict_min_compared_risk)) strict_min_compared_risk <- 2L
    if (is.na(strict_min_match_risk)) strict_min_match_risk <- 2L
    if (!is.finite(strict_prop_match_risk_th)) strict_prop_match_risk_th <- 0.5
    if (is.na(strict_hap_min_n)) strict_hap_min_n <- 10L
    if (!is.finite(strict_yri_freq_th)) strict_yri_freq_th <- yri_freq_th
    if (pick_lineage_method == "hpc") pick_lineage_method <- "avcf"
    if (hap_filter_method == "hpc") hap_filter_method <- "avcf"
    ...
    read_panel <- function() {
        ...
        if (!length(env_panel))
            stop("SAMPLE_FILE is required for YRI filter; gu.sh should export sample_file")
        f <- env_panel[1]
        if (!file.exists(f))
            stop("SAMPLE_FILE not found for YRI filter: ", f)
        x <- fread(f, header = TRUE, fill = TRUE, check.names = FALSE, showProgress = FALSE)
        setnames(x, trimws(sub("^\ufeff", "", names(x))))
        if (!all(c("sample", "pop", "super_pop") %in% names(x))) {
            stop("SAMPLE_FILE must contain columns: sample,pop,super_pop; columns read: ",
                paste(names(x), collapse = ","))
        }
        x <- x[, .(sample = trimws(as.character(sample)), pop = trimws(as.character(pop)),
            super_pop = trimws(as.character(super_pop)))]
        x <- x[!is.na(sample) & sample != ""]
    ...
    pick_lineage_avcf <- function(arch_stat, p, p_cut = 0.1) {
        if (!is.finite(p) || p >= p_cut || !nrow(arch_stat))
            return(list(best_lineage = NA_character_, keep_arch = character(0)))
        z <- copy(arch_stat)
    ...
    pick_lineage <- function(arch_stat, p) {
        if (pick_lineage_method == "avcf") pick_lineage_avcf(arch_stat, p) else pick_lineage_local(arch_stat,
            p)
    }
    ...
    process_one <- function(tr0, id0) {
        ...
        snp0 <- p0$snp
        bp0 <- p0$bp
        ldf <- file.path(trait_dir(ld0, tr0), "ld.tsv")
        bdf <- file.path(trait_dir(ld0, tr0), "block.tsv")
        ...
        d0 <- file.path(trait_dir(mat0, tr0), id0)
        ...
        vcf_samples <- read_vcf_samples(file.path(trait_dir(corevcf0, tr0), id0, "kg.vcf.gz"))
    ...
    merge_all <- function() {
        ...
        out_files <- c("hap_match.csv", "site_count.csv", "archaic_match.csv", "region_summary.csv",
            "risk_core.csv", "candidate_inherited_region.csv", "hap_match.tsv", "site_count.tsv", "archaic_match.tsv",
            "region_summary.tsv", "risk_core.tsv", "candidate_inherited_region.tsv", "hap_site_count.tsv",
            "core_archaic_match.tsv", "core_risk.tsv", "inherited_region.tsv", "inherited_region.strict.tsv", "yri_filter.tsv")
        ...
        if (hap_filter_method == "avcf") {
            ...
            strict_summary_cols <- c(
                "strict_p_ils_th", "strict_min_compared_risk", "strict_min_match_risk",
                "strict_prop_match_risk_th", "strict_hap_min_n", "strict_require_carry_risk",
                "strict_yri_freq_th", "strict_p_ils_pass", "strict_lineage_pass",
                "strict_hap_pass", "strict_yri_pass", "strict_filter_pass",
                "strict_filter_reason", "strict_lineage_n_compared_risk",
                "strict_lineage_n_match_risk", "strict_lineage_prop_match_risk",
                "strict_lineage_max_prop_match_risk", "strict_best_archaic",
                "strict_best_archaic_prop_match_risk", "strict_best_archaic_n_compared_risk",
                "strict_best_archaic_n_match_risk", "strict_hap_n_total",
                "strict_hap_n_gt_min", "strict_hap_carry_risk",
                "strict_hap_carry_risk_n_gt_min", "strict_hap_carry_risk_unknown",
                "strict_hap_max_n", "strict_best_carry_risk_hap_n")
            summary_cols <- c("trait", "id", "lead_chr", "lead_snp", "lead_bp", "core_start",
                ...
                afr_summary_cols, strict_summary_cols)
            ...
            for (nm in c(afr_summary_cols, strict_summary_cols)) empty_summary[, (nm) := NA]
            if (nrow(region_dt)) {
                ...
                # Strict-style diagnostic statistics. These capture the main exclusion reasons
                # while keeping the avcf workflow as the single source of truth. Users can filter
                # inherited_region.tsv by strict_filter_pass or any individual component below.
                if (nrow(arch_dt)) {
                  arch_tmp <- copy(arch_dt)
                  if (!"lineage_group" %in% names(arch_tmp) && "archaic" %in% names(arch_tmp))
                    arch_tmp[, lineage_group := lineage_group_fun(archaic)]
                  for (nm in c("n_compared_risk", "n_match_risk", "prop_match_risk"))
                    if (!nm %in% names(arch_tmp)) arch_tmp[, (nm) := NA_real_]
                  arch_lineage <- arch_tmp[!is.na(lineage_group), .(
                    strict_lineage_n_compared_risk = sum(as.numeric(n_compared_risk), na.rm = TRUE),
                    strict_lineage_n_match_risk = sum(as.numeric(n_match_risk), na.rm = TRUE),
                    strict_lineage_max_prop_match_risk = suppressWarnings(max(as.numeric(prop_match_risk), na.rm = TRUE))
                  ), by = .(trait, id, lineage_group)]
                  arch_lineage[!is.finite(strict_lineage_max_prop_match_risk), strict_lineage_max_prop_match_risk := NA_real_]
                  arch_lineage[, strict_lineage_prop_match_risk := fifelse(strict_lineage_n_compared_risk > 0,
                    strict_lineage_n_match_risk / strict_lineage_n_compared_risk, NA_real_)]
                  arch_best <- arch_tmp[order(trait, id, -as.numeric(prop_match_risk), -as.numeric(n_match_risk)),
                    .SD[1], by = .(trait, id)]
                  arch_best <- arch_best[, .(trait, id,
                    strict_best_archaic = as.character(archaic),
                    strict_best_archaic_prop_match_risk = as.numeric(prop_match_risk),
                    strict_best_archaic_n_compared_risk = as.numeric(n_compared_risk),
                    strict_best_archaic_n_match_risk = as.numeric(n_match_risk))]
                  tmp_key <- reg[, .(trait, id, lineage_group = best_lineage)]
                  strict_arch <- merge(tmp_key, arch_lineage, by = c("trait", "id", "lineage_group"),
                    all.x = TRUE, sort = FALSE)
                  strict_arch[, lineage_group := NULL]
                  strict_arch <- merge(strict_arch, arch_best, by = c("trait", "id"), all.x = TRUE, sort = FALSE)
                  reg <- merge(reg, unique(strict_arch, by = c("trait", "id")), by = c("trait", "id"),
                    all.x = TRUE, sort = FALSE)
                }
                if (nrow(hap_dt)) {
                  hap_tmp <- copy(hap_dt)
                  if (!"n" %in% names(hap_tmp)) hap_tmp[, n := NA_real_]
                  if (!"carry_risk" %in% names(hap_tmp)) hap_tmp[, carry_risk := NA]
                  hap_tmp[, n_num := suppressWarnings(as.numeric(n))]
                  hap_tmp[, carry_risk_bool := as.logical(as.character(carry_risk))]
                  hap_stats <- hap_tmp[, .(
                    strict_hap_n_total = .N,
                    strict_hap_n_gt_min = sum(n_num > strict_hap_min_n, na.rm = TRUE),
                    strict_hap_carry_risk = sum(carry_risk_bool == TRUE, na.rm = TRUE),
                    strict_hap_carry_risk_n_gt_min = sum(carry_risk_bool == TRUE & n_num > strict_hap_min_n, na.rm = TRUE),
                    strict_hap_carry_risk_unknown = sum(is.na(carry_risk_bool)),
                    strict_hap_max_n = suppressWarnings(max(n_num, na.rm = TRUE)),
                    strict_best_carry_risk_hap_n = suppressWarnings(max(n_num[carry_risk_bool == TRUE], na.rm = TRUE))
                  ), by = .(trait, id)]
                  hap_stats[!is.finite(strict_hap_max_n), strict_hap_max_n := NA_real_]
                  hap_stats[!is.finite(strict_best_carry_risk_hap_n), strict_best_carry_risk_hap_n := NA_real_]
                  reg <- merge(reg, hap_stats, by = c("trait", "id"), all.x = TRUE, sort = FALSE)
                }
                for (nm in c("strict_lineage_n_compared_risk", "strict_lineage_n_match_risk",
                  "strict_lineage_prop_match_risk", "strict_lineage_max_prop_match_risk",
                  "strict_best_archaic", "strict_best_archaic_prop_match_risk",
                  "strict_best_archaic_n_compared_risk", "strict_best_archaic_n_match_risk",
                  "strict_hap_n_total", "strict_hap_n_gt_min", "strict_hap_carry_risk",
                  "strict_hap_carry_risk_n_gt_min", "strict_hap_carry_risk_unknown",
                  "strict_hap_max_n", "strict_best_carry_risk_hap_n")) {
                  if (!nm %in% names(reg)) reg[, (nm) := NA]
                }
                for (nm in c("strict_hap_n_total", "strict_hap_n_gt_min", "strict_hap_carry_risk",
                  "strict_hap_carry_risk_n_gt_min", "strict_hap_carry_risk_unknown"))
                  reg[is.na(get(nm)), (nm) := 0L]
                reg[, `:=`(
                  strict_p_ils_th = strict_p_ils_th,
                  strict_min_compared_risk = strict_min_compared_risk,
                  strict_min_match_risk = strict_min_match_risk,
                  strict_prop_match_risk_th = strict_prop_match_risk_th,
                  strict_hap_min_n = strict_hap_min_n,
                  strict_require_carry_risk = strict_require_carry_risk,
                  strict_yri_freq_th = strict_yri_freq_th,
                  strict_p_ils_pass = is.finite(p_ils) & p_ils < strict_p_ils_th,
                  strict_lineage_pass = !is.na(best_lineage) & nzchar(best_lineage) &
                    !is.na(strict_lineage_n_compared_risk) &
                    strict_lineage_n_compared_risk >= strict_min_compared_risk &
                    !is.na(strict_lineage_n_match_risk) &
                    strict_lineage_n_match_risk >= strict_min_match_risk &
                    !is.na(strict_lineage_prop_match_risk) &
                    strict_lineage_prop_match_risk >= strict_prop_match_risk_th,
                  strict_hap_pass = if (strict_require_carry_risk)
                    strict_hap_carry_risk_n_gt_min > 0 else strict_hap_n_gt_min > 0,
                  strict_yri_pass = (!have_yri) | strict_yri_freq_th >= 1 |
                    (!is.na(YRI_max_freq) & YRI_max_freq <= strict_yri_freq_th)
                )]
                for (nm in c("strict_p_ils_pass", "strict_lineage_pass", "strict_hap_pass", "strict_yri_pass"))
                  reg[is.na(get(nm)), (nm) := FALSE]
                reg[, strict_filter_pass := strict_p_ils_pass & strict_lineage_pass & strict_hap_pass & strict_yri_pass]
                reg[, strict_filter_reason := vapply(seq_len(.N), function(ii) {
                  reasons <- character(0)
                  if (!strict_p_ils_pass[ii]) reasons <- c(reasons, paste0("p_ils>=", strict_p_ils_th[ii]))
                  if (!strict_lineage_pass[ii]) reasons <- c(reasons, paste0("lineage_match<", strict_prop_match_risk_th[ii],
                    " or n_match<", strict_min_match_risk[ii]))
                  if (!strict_hap_pass[ii]) reasons <- c(reasons, if (strict_require_carry_risk[ii])
                    paste0("no carry_risk TRUE haplotype with n>", strict_hap_min_n[ii]) else
                    paste0("no haplotype with n>", strict_hap_min_n[ii]))
                  if (!strict_yri_pass[ii]) reasons <- c(reasons, paste0("YRI_max_freq>", strict_yri_freq_th[ii], " or missing"))
                ...
                yri_filter <- data.table(
                  ...
                  strict_pass_loci_in_selected = if (nrow(selected) && "strict_filter_pass" %in% names(selected)) sum(selected$strict_filter_pass == TRUE, na.rm = TRUE) else 0L
                )
                ...
                fwrite(selected[, ..summary_cols], file.path(report0, "inherited_region.tsv"), sep = "\t")
                strict_selected <- selected[strict_filter_pass == TRUE]
                fwrite(strict_selected[, ..summary_cols], file.path(report0, "inherited_region.strict.tsv"), sep = "\t")
                fwrite(yri_filter, file.path(report0, "yri_filter.tsv"), sep = "\t")
            } else {
                fwrite(empty_summary, file.path(report0, "region_summary.tsv"), sep = "\t")
                fwrite(empty_summary, file.path(report0, "inherited_region.tsv"), sep = "\t")
                fwrite(empty_summary, file.path(report0, "inherited_region.strict.tsv"), sep = "\t")
                fwrite(data.table(yri_freq_th = yri_freq_th, yri_filter_enabled = FALSE,
                  ...
                  matched_loci = 0L, selected_loci = 0L,
                  strict_pass_loci_in_selected = 0L), file.path(report0, "yri_filter.tsv"), sep = "\t")
            }
        }
        else {
            write_report(hap_dt, "hap_match")
            ...
            if (nrow(region_dt)) {
                ...
                write_report(inherited, "candidate_inherited_region")
            }
        }
    }
    run_trait <- function(tr0) {
        td <- trait_dir(mat0, tr0)
        if (!dir.exists(td))
    ...
    run_all <- function() {
        if (single_trait_root(basename(root))) run_trait(basename(root))
        else for (tr0 in list.dirs(mat0, recursive = FALSE, full.names = FALSE)) run_trait(tr0)
        merge_all()
    }
    if (!length(args))
        run_all()
    ...
    else stop("usage: Rscript gu.R make_hap <res_dir> [trait | trait id | merge]")
}
...
run_filter_hap <- function(script_args) {
    ...
    need_pkg(c("data.table", "writexl"))
    args <- commandArgs(TRUE)
    if (length(args) < 2L)
        stop("usage: Rscript gu.R filter_hap DIR_OUT SAMPLE_FILE [FILTER_POP] [FILTER_MAX_COUNT]")
    root <- normalizePath(args[1], winslash = "/", mustWork = FALSE)
    sample_file <- normalizePath(args[2], winslash = "/", mustWork = FALSE)
    ...
    single_trait_root <- function(tr) basename(root) == tr
    trait_dir <- function(base, tr) {
        d <- file.path(base, tr)
        if (dir.exists(d)) d else if (single_trait_root(tr)) base else d
    }
    ...
    segf <- file.path(report0, "candidate_inherited_region.tsv")
    if (!file.exists(hapf))
        stop("missing hap_match.tsv: ", hapf)
    if (!file.exists(segf))
        stop("missing candidate_inherited_region.tsv: ", segf)
    if (!file.exists(sample_file))
        stop("missing sample file: ", sample_file)
    hap <- fread(hapf)
    seg <- fread(segf)
    sample_meta <- fread(sample_file, header = TRUE, fill = TRUE, check.names = FALSE, showProgress = FALSE)
    setnames(sample_meta, trimws(sub("^\ufeff", "", names(sample_meta))))
    if (!all(c("sample", "pop") %in% names(sample_meta)))
        stop("sample file must contain columns: sample,pop; columns read: ", paste(names(sample_meta), collapse = ","))
    if (!"super_pop" %in% names(sample_meta))
        sample_meta[, `:=`(super_pop, NA_character_)]
    sample_meta[, `:=`(sample = trimws(as.character(sample)), pop = trimws(as.character(pop)),
        super_pop = trimws(as.character(super_pop)))]
    sample_meta <- sample_meta[!is.na(sample) & sample != ""]
    ...
    get_sample_map <- function(trait, id) {
        ...
        f <- file.path(trait_dir(mat0, trait), id, "kg.samples.tsv")
        if (!file.exists(f))
    ...
    all_sheets <- list(inherited_loci_counts = locus_summary, inherited_haplotype_counts = hap_counts,
        candidate_inherited_region = seg, archaic_match = read_optional("archaic_match"), risk_core = read_optional("risk_core"),
    ...
    filtered_sheets <- list(filtered_loci = filtered_loci, filtered_haplotypes = hap_counts[filtered_keys,
        on = .(trait, id), nomatch = 0], inherited_region = filtered_seg, filtered_archaic_match = filter_dt(read_optional("archaic_match")),
    ...
    fwrite(filtered_seg, file.path(report0, "inherited_region.tsv"), sep = "\t")
    write_xlsx(all_sheets, file.path(report0, "all.xlsx"))
    write_xlsx(filtered_sheets, file.path(report0, "filtered.xlsx"))
    write_xlsx(selected_sheets, file.path(report0, "selected.xlsx"))
...
run_make_phy <- function(script_args) {
    ...
    root_arg <- if (has_root) args0[1] else Sys.getenv("BALD_RES", "")
    if (!nzchar(root_arg))
        stop("make_phy requires RES_DIR as the first argument or BALD_RES to be set")
    root <- normalizePath(root_arg, winslash = "/", mustWork = FALSE)
    args <- if (has_root)
        args0[-1]
    else args0
    ...
    make_phy_method <- Sys.getenv("MAKE_PHY_METHOD", "avcf")
    report0 <- file.path(root, dir_report)
    if (make_phy_method == "hpc") make_phy_method <- "avcf"
    hapf <- if (make_phy_method == "avcf") file.path(report0, "hap_match.tsv") else file.path(report0,
        "filtered_hap_match.tsv")
    if (!file.exists(hapf)) hapf <- file.path(report0, "hap_match.tsv")
    if (!file.exists(hapf))
        hapf <- file.path(root, "hap_match.tsv")
    selectedf <- file.path(report0, "inherited_region.tsv")
    ...
    base_set <- c("A", "C", "G", "T")
    hap <- fread(hapf)
    single_trait_root <- function(tr) basename(root) == tr && "trait" %in% names(hap) && tr %in% hap$trait
    trait_dir <- function(base, tr) {
        d <- file.path(base, tr)
        if (dir.exists(d)) d else if (single_trait_root(tr)) base else d
    }
    if (make_phy_method == "avcf") {
    ...
    old_root <- if (is.na(target_trait))
        phy0
    else trait_dir(phy0, target_trait)
    ...
    for (i in seq_len(nrow(keys))) {
        ...
        make_one <- function(sub, tag) {
            ...
            if (length(len) != 1L)
                return(invisible(NULL))
            lab0 <- names(seqs)
            lab <- sprintf("S%09d", seq_along(seqs))
        ...
        make_one(h, "full")
        make_one(if (make_phy_method == "avcf") h[n > 10] else h, "main")
    }
}
...
run_make_tree <- function(script_args) {
    ...
    need_pkg(c("data.table", "ape"))
    args <- commandArgs(TRUE)
    root_arg <- if (length(args)) args[1] else Sys.getenv("BALD_RES", "")
    if (!nzchar(root_arg))
        stop("make_tree requires RES_DIR as the first argument or BALD_RES to be set")
    root <- normalizePath(root_arg, winslash = "/", mustWork = FALSE)
    phy0 <- file.path(root, "phy")
    out0 <- file.path(root, dir_plot)
    dir.create(out0, recursive = TRUE, showWarnings = FALSE)

    keep_existing <- Sys.getenv("MAKE_TREE_KEEP_EXISTING", "0") == "1"
    tags <- unlist(strsplit(Sys.getenv("MAKE_TREE_TAGS", "main"), "[,;[:space:]]+"))
    tags <- intersect(tags[nzchar(tags)], c("main", "full"))
    if (!length(tags)) tags <- "main"
    write_pdf <- Sys.getenv("MAKE_TREE_PDF", "0") == "1"
    min_boot <- suppressWarnings(as.numeric(Sys.getenv("MAKE_TREE_MIN_BOOT", "70")))
    if (!is.finite(min_boot)) min_boot <- 70
    scale_bar_len <- suppressWarnings(as.numeric(Sys.getenv("MAKE_TREE_SCALE_BAR", "0.005")))
    if (!is.finite(scale_bar_len)) scale_bar_len <- 0.005

    if (!keep_existing)
        unlink(list.files(out0, pattern = "^s8_tree_.*\\.(png|pdf)$", full.names = TRUE), force = TRUE)
    ...
    style <- function(tag) {
        if (tag == "main") {
            list(width = gu_plot_width_inch, height = 5.2, cex_tip = 0.80,
                 cex_node = 0.62, lwd_tree = 1.0, label_offset = 0.0008,
                 dot_cex = 0.75)
        } else {
            list(width = gu_plot_width_inch, height = 6.6, cex_tip = 0.48,
                 cex_node = 0.48, lwd_tree = 0.65, label_offset = 0.0008,
                 dot_cex = 0.45)
        }
    }
    ...
    tip_info <- function(meta, labs) {
        original <- labs
        type <- rep("modern", length(labs))
        if (!is.null(meta) && all(c("label", "original_label", "type") %in% names(meta))) {
            idx <- match(labs, meta$label)
            ok <- !is.na(idx)
            original[ok] <- as.character(meta$original_label[idx[ok]])
            type[ok] <- as.character(meta$type[idx[ok]])
        }
        type[grepl("vindija|altai|chagyr|denisova|neand|archaic", original, ignore.case = TRUE)] <- "archaic"
        type[original == "Ancestral"] <- "ancestral"
        list(label = original, type = type)
    }

    node_support <- function(tr, node) {
        x <- suppressWarnings(as.numeric(tr$node.label[node - Ntip(tr)]))
        if (length(x) != 1L || !is.finite(x)) return(NA_real_)
        x
    }

    highlight_node <- function(tr, type) {
        nt <- Ntip(tr)
        arch_i <- which(type == "archaic")
        mod_i <- which(type == "modern")
        if (!length(arch_i) || !length(mod_i)) return(NA_integer_)
        nodes <- (nt + 1):(nt + tr$Nnode)
        z <- rbindlist(lapply(nodes, function(node) {
            tips <- desc_tips(tr, node)
            support <- node_support(tr, node)
            if (!all(arch_i %in% tips) || !any(mod_i %in% tips) || any(type[tips] == "ancestral")) return(NULL)
            data.table(node = node, support = support, n_tips = length(tips), n_modern = sum(type[tips] == "modern"))
        }), fill = TRUE)
        if (!nrow(z)) return(NA_integer_)
        z <- z[is.na(support) | support >= min_boot]
        if (!nrow(z)) return(NA_integer_)
        setorder(z, n_tips, -n_modern, -support)
        z$node[1]
    }

    draw_tree <- function(tr, meta, tag, main_title = NULL) {
        st <- style(tag)
        tr$tip.label <- trimws(tr$tip.label)
        info0 <- tip_info(meta, tr$tip.label)
        if ("Ancestral" %in% info0$label) {
            out_lab <- tr$tip.label[info0$label == "Ancestral"][1]
            tr <- tryCatch(root(tr, outgroup = out_lab, resolve.root = TRUE), error = function(e) tr)
        }
        tr <- ladderize(tr)
        info <- tip_info(meta, tr$tip.label)
        tr$tip.label <- info$label
        type <- info$type
        tip_col <- rep(modern_col, length(type))
        tip_col[type == "archaic"] <- ifelse(grepl("denisova", tr$tip.label[type == "archaic"], ignore.case = TRUE), den_col, arch_col)
        tip_col[type == "ancestral"] <- anc_col
        hi <- highlight_node(tr, type)

        par(mar = c(3.2, 1.0, 2.2, 1.0), xpd = NA)
        plot.phylo(tr, type = "phylogram", direction = "rightwards", show.tip.label = TRUE,
                   cex = st$cex_tip, font = 1, tip.color = tip_col,
                   edge.width = st$lwd_tree, label.offset = st$label_offset,
                   no.margin = FALSE, main = main_title)
        tiplabels(pch = 21, bg = tip_col, col = tip_col, cex = st$dot_cex, adj = 0.5)
        if (!is.na(hi)) {
            nodelabels(node = hi, pch = 21, bg = adjustcolor(hi_col, alpha.f = 0.75),
                       col = hi_col, cex = 1.0)
            boot <- node_support(tr, hi)
            if (!is.na(boot))
                nodelabels(text = boot, node = hi, frame = "none", cex = st$cex_node,
                           adj = c(1.15, -0.2), col = hi_col)
        }
        add.scale.bar(length = scale_bar_len, lwd = 1.1, cex = 0.75)
        legend("bottomleft", inset = 0.01, bty = "n", cex = 0.78,
               legend = c("modern haplotypes", "Neanderthal", "Denisovan", "ancestral", "candidate clade"),
               pch = c(21, 21, 21, 21, 21), pt.bg = c(modern_col, arch_col, den_col, anc_col, hi_col),
               col = c(modern_col, arch_col, den_col, anc_col, hi_col))
    }

    files <- list.files(phy0, pattern = "\\.(full|main)\\.phy_phyml_tree\\.txt$", recursive = TRUE,
                        full.names = TRUE)
    if (!length(files)) quit(save = "no", status = 0)
    files <- files[vapply(files, function(f) {
        base <- sub("\\.phy_phyml_tree\\.txt$", "", basename(f))
    ...
    for (treef in files) {
        ...
        if (trait == "phy") trait <- basename(root)
        title <- paste0(trait, " | ", sub(paste0("\\.", tag, "$"), "", base))
        pngf <- file.path(out0, paste0("s8_tree_", tag, "_", trait, "_", base, ".png"))
        if (!keep_existing || !file.exists(pngf) || file.info(pngf)$size == 0) {
            png(pngf, width = st$width, height = st$height, units = "in", res = 300)
            draw_tree(tr, meta, tag, title)
            dev.off()
        }
        if (write_pdf) {
            pdff <- sub("\\.png$", ".pdf", pngf)
            if (!keep_existing || !file.exists(pdff) || file.info(pdff)$size == 0) {
                pdf(pdff, width = st$width, height = st$height, useDingbats = FALSE)
                draw_tree(tr, meta, tag, title)
                dev.off()
            }
        }
...
run_add_positive_loci <- function(script_args){
    ...
    if(is.null(dirgwas) || is.null(dirout) || is.null(bed) || is.null(trait)) stop('usage: Rscript gu.R add_positive_loci --dirgwas DIR --dirmod DIR --dirout DIR --bed BED --trait TRAIT [--ref_pop ALL|EUR]')
    ...
    single_trait_out <- basename(normalizePath(dirout, winslash = "/", mustWork = FALSE)) == trait
    assocf <- if (single_trait_out) file.path(lead0, 'lead.assoc') else file.path(lead0, paste0(trait, '.lead.assoc'))
    threef <- if (single_trait_out) file.path(lead0, 'lead.3col') else file.path(lead0, paste0(trait, '.lead.3col'))
...
# Optional columns avoid scanning huge 1KG pvar files:
# chr start end snp [lead_bp effect_allele other_allele beta risk_allele] [# comment]
    bed_lines <- readLines(bed, warn = FALSE)
    bed_lines <- sub('[[:space:]]+#.*$', '', bed_lines)
    ...
    get_col <- function(z, i) if(length(z) >= i) z[i] else NA_character_
    loc <- rbindlist(lapply(sp, function(z) data.table(
        chr=z[1], start=z[2], end=z[3], snp=z[4],
        bed_bp=get_col(z, 5), bed_ea=get_col(z, 6), bed_oa=get_col(z, 7),
        bed_beta=get_col(z, 8), bed_risk=get_col(z, 9)
    )), fill=TRUE)
    loc[, chr := toupper(sub('^CHR','',as.character(chr)))]
    loc[, lead_chr := fifelse(chr == 'X', 23L, as.integer(chr))]
    loc[, `:=`(
        start=as.integer(start), end=as.integer(end), snp=as.character(snp),
        bed_bp=suppressWarnings(as.integer(bed_bp)),
        bed_ea=toupper(as.character(bed_ea)),
        bed_oa=toupper(as.character(bed_oa)),
        bed_beta=suppressWarnings(as.numeric(bed_beta)),
        bed_risk=toupper(as.character(bed_risk))
    )]
    loc[bed_ea %in% c('', '.', 'NA', 'NAN'), bed_ea := NA_character_]
    loc[bed_oa %in% c('', '.', 'NA', 'NAN'), bed_oa := NA_character_]
    loc[bed_risk %in% c('', '.', 'NA', 'NAN'), bed_risk := NA_character_]
    ...
    x_chr_part <- function(bp){
        build <- Sys.getenv('genome_build', 'b37')
        env_int <- function(stem, default){
            z <- Sys.getenv(paste0(stem, '_', build), '')
            if(nzchar(z)) suppressWarnings(as.integer(z)) else default
        }
        p1s <- env_int('X_PAR1_START', 60001L); p1e <- env_int('X_PAR1_END', 2699520L)
        p2s <- env_int('X_PAR2_START', 154931044L); p2e <- env_int('X_PAR2_END', 155260560L)
        if(!is.na(bp) && ((bp >= p1s && bp <= p1e) || (bp >= p2s && bp <= p2e))) 'par' else 'nonPar'
    }
    pvar_files <- function(chr, bp){
        if(chr == 23L){
            ...
            if(ref_pop == 'ALL') {
                if(par == 'par') unique(c(
                    file.path(dirmod, 'ALL.male.chrX.par.pvar'),
                    file.path(dirmod, 'ALL.chrX.par.pvar'),
                    file.path(dirmod, 'ALL.chrX.pvar'),
                    file.path(dirmod, 'chrX.pvar')
                )) else unique(c(
                    file.path(dirmod, 'ALL.male.chrX.nonPar.pvar'),
                    file.path(dirmod, 'ALL.chrX.nonPar.pvar'),
                    file.path(dirmod, 'ALL.chrX.pvar'),
                    file.path(dirmod, 'chrX.pvar')
                ))
            } else {
                unique(c(file.path(dirmod, paste0(ref_pop,'.male.chrX.',par,'.pvar')),
                         file.path(dirmod, paste0(ref_pop,'.chrX.',par,'.pvar')),
                         file.path(dirmod, paste0(ref_pop,'.chrX.pvar'))))
            }
        } else {
            if(ref_pop == 'ALL') {
                unique(c(file.path(dirmod, paste0('ALL.chr',chr,'.pvar')), file.path(dirmod, paste0('chr',chr,'.pvar'))))
            } else {
    ...
    query_pvar <- function(chr, snp, bp){
        ...
        for(f in fs){
            check_pvar_format(f, "positive-locus pvar ID")
            cmd <- sprintf("awk 'BEGIN{FS=OFS=\"\\t\"} $1!~/^#/ && $3==%s{print $2,$3,$4,$5; exit}' %s", shQuote(snp), shQuote(f))
            z <- run_shell_checked(cmd, timeout = pvar_scan_timeout(), label = paste("positive-locus pvar ID scan", f), intern = TRUE)
        ...
        for(f in fs){
            check_pvar_format(f, "positive-locus pvar BP")
            cmd <- sprintf("awk 'BEGIN{FS=OFS=\"\\t\"} $1!~/^#/ && $2==%s{print $2,$3,$4,$5; exit}' %s", shQuote(as.character(bp)), shQuote(f))
            z <- run_shell_checked(cmd, timeout = pvar_scan_timeout(), label = paste("positive-locus pvar BP scan", f), intern = TRUE)
            if(length(z) && nzchar(z[1])){
                sp <- strsplit(z[1], '\\t')[[1]]
...
# Positive-control loci are intentionally injected as sanity checks.  Do not
# read the full trait GWAS here: those files can be very large, and this
# step only needs REF/ALT from the matching 1KG pvar record.
    g <- data.table()
    rows <- list(); k <- 0L
    for(i in seq_len(nrow(loc))){
        snp <- loc$snp[i]
        if(any(as.character(x$lead_snp) == snp)) next
        have_bed_alleles <- !is.na(loc$bed_bp[i]) && !is.na(loc$bed_ea[i]) && !is.na(loc$bed_oa[i])
        pv <- if(have_bed_alleles) {
            list(bp=loc$bed_bp[i], id=snp, ref=loc$bed_oa[i], alt=loc$bed_ea[i], pvar=NA_character_)
        } else {
            query_pvar(loc$lead_chr[i], snp, if(!is.na(loc$bed_bp[i])) loc$bed_bp[i] else loc$end[i])
        }
        gg <- if(nrow(g)) g[lead_snp == snp][1] else data.table()
        ea <- if(!is.na(loc$bed_ea[i])) loc$bed_ea[i] else if(nrow(gg)) gg$effect_allele[1] else pv$alt
        oa <- if(!is.na(loc$bed_oa[i])) loc$bed_oa[i] else if(nrow(gg)) gg$other_allele[1] else pv$ref
        be <- if(!is.na(loc$bed_beta[i])) loc$bed_beta[i] else if(nrow(gg)) gg$beta[1] else 1
        rk <- if(!is.na(loc$bed_risk[i])) loc$bed_risk[i] else if(!is.na(be) && be >= 0) ea else oa
        k <- k + 1L
...
run_hap_sample_map <- function(script_args) {
    ...
    args <- script_args
    if (length(args) < 3) stop("usage: Rscript gu.R hap_sample_map DIR_OUT SAMPLE_FILE OUT_TSV")
    root <- args[1]
    sample_file <- args[2]
    outf <- args[3]
    hapf <- file.path(root, dir_report, "hap_match.tsv")
    if (!file.exists(sample_file))
        stop("missing sample file: ", sample_file)
    hap <- fread(hapf, select = c("trait", "id", "hap_id", "n", "copies"))
    single_trait_root <- function(tr) basename(normalizePath(root, winslash = "/", mustWork = FALSE)) == tr && tr %in% hap$trait
    trait_dir <- function(base, tr) {
        d <- file.path(base, tr)
        if (dir.exists(d)) d else if (single_trait_root(tr)) base else d
    }
    panel <- fread(sample_file, header = TRUE, fill = TRUE, check.names = FALSE, showProgress = FALSE)
    setnames(panel, trimws(sub("^\ufeff", "", names(panel))))
    if (!all(c("sample", "pop", "super_pop") %in% names(panel))) {
        stop("sample file must contain columns: sample,pop,super_pop; columns read: ",
            paste(names(panel), collapse = ","))
    }
    panel <- panel[, .(sample = trimws(as.character(sample)), pop = trimws(as.character(pop)),
        super_pop = trimws(as.character(super_pop)))]
    ...
    get_samples <- function(tr, id) {
        ...
        f <- file.path(trait_dir(file.path(root, "mat"), tr), id, "kg.samples.tsv")
        x <- if (file.exists(f)) fread(f, header = FALSE)[[1]] else character()
        if (!length(x)) {
            vcf <- file.path(trait_dir(file.path(root, "coreVcf"), tr), id, "kg.vcf.gz")
            if (file.exists(vcf)) {
                con <- gzfile(vcf, "rt")
                repeat {
...
run_positive_loci_fate <- function(script_args) {
    suppressPackageStartupMessages(library(data.table))
    args <- script_args
    if (length(args) < 3) stop("usage: Rscript gu.R positive_loci_fate DIR_OUT BED OUT_TSV")
    root <- args[1]
    bedf <- args[2]
    outf <- args[3]
    ...
    region <- read_tsv(file.path(root, dir_report, "region_summary.tsv"))
    selected <- read_tsv(file.path(root, dir_report, "inherited_region.tsv"))
    phy <- read_tsv(file.path(root, dir_report, "phy_region.tsv"))
    core <- read_tsv(file.path(root, dir_report, "core_risk.tsv"))
    arch <- read_tsv(file.path(root, dir_report, "core_archaic_match.tsv"))
    mkid <- function(x) {
        if (!nrow(x)) return(x)
    ...
    yri_freq_th <- suppressWarnings(as.numeric(Sys.getenv("YRI_FREQ_TH", "0.05")))
    yri_msg <- if (is.finite(yri_freq_th) && yri_freq_th >= 1) "s6 avcf YRI filter disabled" else
        paste0("s6 avcf YRI filter: archaic-like region found but YRI_max_freq > ",
...
run_asnp <- function(script_args) {
    ...
    parse_variant_id <- function(x) {
        ...
        x0 <- gsub("[|/;]", ":", x0)
# Accept chr:pos:ref:alt, chr_pos_ref_alt, and chr-pos-ref-alt style IDs.
    ...
    read_gwas_asnp <- function(f) {
        ...
        x[!is.na(CHR) & !is.na(POS) & !is.na(P) & nzchar(REF) & nzchar(ALT),
          `:=`(varid_chr = norm_variant_id(CHR, POS, REF, ALT, TRUE),
               ...
               varid_chr_flip = norm_variant_id(CHR, POS, ALT, REF, TRUE),
               varid_nochr_flip = norm_variant_id(CHR, POS, ALT, REF, FALSE))]
        x
    }
    standardize_asnp <- function(f) {
        ...
        a[, `:=`(asnp_id_chr = norm_variant_id(var_chr, var_pos, var_ref, var_alt, TRUE),
                 ...
                 asnp_id_chr_flip = norm_variant_id(var_chr, var_pos, var_alt, var_ref, TRUE),
                 asnp_id_nochr_flip = norm_variant_id(var_chr, var_pos, var_alt, var_ref, FALSE))]
        if (is.na(hap_col)) a[, asnp_haplotype := paste0("hap_", asnp_id_chr)] else a[, asnp_haplotype := as.character(get(hap_col))]
        if (is.na(sup_col)) a[, SupPop_asnp := NA_character_] else a[, SupPop_asnp := as.character(get(sup_col))]
        if (is.na(freq_col)) a[, asnp_freq := NA_real_] else a[, asnp_freq := suppressWarnings(as.numeric(get(freq_col)))]
        if (is.na(lin_col)) a[, asnp_lineage := NA_character_] else a[, asnp_lineage := as.character(get(lin_col))]
        if (is.na(n_col)) {
            n_by_hap <- a[!is.na(var_pos), .(asnp_n_asnp_haplotype = uniqueN(asnp_id_chr)), by = asnp_haplotype]
            a <- merge(a, n_by_hap, by = "asnp_haplotype", all.x = TRUE, sort = FALSE)
        } else {
            a[, asnp_n_asnp_haplotype := suppressWarnings(as.integer(get(n_col)))]
        }
        a[is.na(asnp_n_asnp_haplotype), asnp_n_asnp_haplotype := 1L]
        a
    }
    read_existing_ld <- function(root, tr) {
        dirs <- unique(c(file.path(root, "ld", tr), file.path(root, "ld")))
        fs <- unique(unlist(lapply(dirs[dir.exists(dirs)], function(d) list.files(d, pattern = "ld\\.tsv$", recursive = TRUE, full.names = TRUE))))
    ...
    attach_ld <- function(hit, ld) {
        hit[, asnp_ld_r2_with_lead := NA_real_]
        hit[, asnp_ld_source := "not_available"]
        ...
        hit <- merge(hit, z[, .(lead_snp, POS, asnp_ld_r2_with_lead_tmp = R2)], by = c("lead_snp", "POS"), all.x = TRUE, sort = FALSE)
        hit[!is.na(asnp_ld_r2_with_lead_tmp), `:=`(asnp_ld_r2_with_lead = asnp_ld_r2_with_lead_tmp, asnp_ld_source = "existing_locus_ld")]
        hit[, asnp_ld_r2_with_lead_tmp := NULL]
        hit
    }
    write_empty <- function(report0) {
        empty_hits <- data.table(trait=character(), lead_chr=integer(), lead_snp=character(), lead_bp=integer(), SNP=character(), CHR=integer(), POS=integer(), REF=character(), ALT=character(), BETA=numeric(), P=numeric(), AF=numeric(), asnp_id=character(), asnp_haplotype=character(), asnp_n_asnp_haplotype=integer(), asnp_freq=numeric(), SupPop_asnp=character(), asnp_lineage=character(), asnp_ld_r2_with_lead=numeric(), asnp_ld_source=character())
        fwrite(empty_hits, file.path(report0, "asnp_aSNP_hits.tsv"), sep = "\t")
        fwrite(empty_hits, file.path(report0, "asnp_inherited_segments.tsv"), sep = "\t")
        fwrite(data.table(), file.path(report0, "asnp_region_summary.tsv"), sep = "\t")
    }
    make_network_one <- function(root, dirmod, row, a_sub, ld_r2) {
        ...
        dir.create(file.path(root, "asnp"), recursive = TRUE, showWarnings = FALSE)
        prefix <- file.path(root, "asnp", paste(row$trait[1], row$lead_chr[1], row$lead_snp[1], row$lead_bp[1], sep = "."))
        gt_file <- paste0(prefix, ".network_genotypes.tsv")
        contigs <- tryCatch(system2("bcftools", c("index", "-s", vcf), stdout = TRUE, stderr = FALSE), error = function(e) character())
        contig_names <- sub("\\t.*$", "", contigs)
        region_chr <- if (paste0("chr", chr_lab) %in% contig_names) paste0("chr", chr_lab) else chr_lab
        cmd <- sprintf("bcftools view -v snps -r %s:%d-%d %s | bcftools query -f '%%CHROM\\t%%POS\\t%%REF\\t%%ALT[\\t%%GT]\\n' > %s", shQuote(region_chr), st, en, shQuote(vcf), shQuote(gt_file))
        ...
        tryCatch({
            grDevices::pdf(pdf_file, width = gu_plot_width_inch, height = gu_scaled_height_inch(7, 6))
    ...
    if (is.null(dirgwas) || is.null(dirout) || is.null(asnp_file) || is.null(traits)) stop("usage: Rscript gu.R asnp --dirgwas DIR --dirout DIR --dirmod DIR --asnp FILE --traits trait ...")
    traits <- unlist(strsplit(paste(traits, collapse = " "), "[, ]+")); traits <- traits[nzchar(traits)]
    report0 <- file.path(dirout, dir_report)
    dir.create(report0, recursive = TRUE, showWarnings = FALSE)
    dir.create(file.path(dirout, "asnp"), recursive = TRUE, showWarnings = FALSE)
    if (!file.exists(asnp_file)) stop("asnp aSNP map not found: ", asnp_file)
    lead_file <- file.path(dirout, "lead", "lead.assoc")
    if (!file.exists(lead_file)) stop("asnp requires prepared lead.assoc: ", lead_file)
    ...
    a_key <- unique(a[!is.na(var_chr) & !is.na(var_pos), .(asnp_id_chr, asnp_id_nochr, asnp_id_chr_flip, asnp_id_nochr_flip, asnp_haplotype, asnp_n_asnp_haplotype, asnp_freq, SupPop_asnp, asnp_lineage, var_chr, var_pos, var_ref, var_alt)])
    ...
    for (tr in traits) {
        ...
        message("asnp: reading GWAS ", tr, " from ", gf)
        g <- read_gwas_asnp(gf)
        ...
        for (ii in seq_len(nrow(leads_tr))) {
            ...
            if (nrow(h1)) h1[, allele_orientation := "as_is"]
            h2 <- merge(reg, a_key, by.x = "varid_nochr", by.y = "asnp_id_nochr", all = FALSE, sort = FALSE, allow.cartesian = TRUE)
            if (nrow(h2)) h2[, allele_orientation := "as_is"]
            h3 <- merge(reg, a_key, by.x = "varid_chr_flip", by.y = "asnp_id_chr_flip", all = FALSE, sort = FALSE, allow.cartesian = TRUE)
            if (nrow(h3)) h3[, allele_orientation := "flipped"]
            h4 <- merge(reg, a_key, by.x = "varid_nochr_flip", by.y = "asnp_id_nochr_flip", all = FALSE, sort = FALSE, allow.cartesian = TRUE)
            if (nrow(h4)) h4[, allele_orientation := "flipped"]
            hit <- unique(rbindlist(list(h1, h2, h3, h4), fill = TRUE), by = c("SNP", "CHR", "POS", "REF", "ALT", "asnp_haplotype", "SupPop_asnp", "allele_orientation"))
            if (nrow(hit)) {
                hit[, `:=`(trait = tr, lead_chr = l$lead_chr, lead_snp = l$lead_snp, lead_bp = l$lead_bp,
                           ...
                           asnp_test_freq = fifelse(!is.na(asnp_freq), asnp_freq, AF),
                           pass_p = P <= p_th,
                           pass_freq = is.na(asnp_test_freq) | asnp_test_freq >= freq_th,
                           pass_min_asnp = asnp_n_asnp_haplotype >= min_asnp)]
                hit <- attach_ld(hit, ld)
                hit[, pass_asnp := pass_p & pass_freq & pass_min_asnp]
                asnp_cols <- c("trait","lead_chr","lead_snp","lead_bp","region_start","region_end","SNP","CHR","POS","REF","ALT","BETA","P","AF","asnp_id","asnp_haplotype","asnp_n_asnp_haplotype","asnp_freq","asnp_test_freq","SupPop_asnp","asnp_lineage","asnp_ld_r2_with_lead","allele_orientation","asnp_ld_source","pass_p","pass_freq","pass_min_asnp","pass_asnp")
                setcolorder(hit, c(intersect(asnp_cols, names(hit)), setdiff(names(hit), asnp_cols)))
                all_hits[[length(all_hits)+1L]] <- hit
                best <- hit[order(P)][1]
                sig <- hit[pass_asnp == TRUE]
                region_sum[[length(region_sum)+1L]] <- data.table(trait=tr, lead_chr=l$lead_chr, lead_snp=l$lead_snp, lead_bp=l$lead_bp, start=st, end=en,
                    n_gwas=nrow(reg), n_asnp_hits=nrow(hit), n_significant_asnp=nrow(sig), best_p=best$P, best_asnp=best$asnp_id, best_haplotype=best$asnp_haplotype,
                    best_lineage=best$asnp_lineage, best_freq=best$asnp_freq, best_n_asnp_haplotype=best$asnp_n_asnp_haplotype,
                    max_ld_r2_with_lead=suppressWarnings(max(hit$asnp_ld_r2_with_lead, na.rm = TRUE)), status=ifelse(nrow(sig)>0, "asnp_candidate", "asnp_overlap_no_pass"))
                if (make_network == 1L && nrow(sig)) {
                    a_sub <- a[asnp_haplotype %chin% unique(sig$asnp_haplotype)]
    ...
    if (!nrow(hits)) {
        ...
        fwrite(hits, file.path(report0, "asnp_aSNP_hits.tsv"), sep = "\t")
        fwrite(hits[pass_asnp == TRUE], file.path(report0, "asnp_inherited_segments.tsv"), sep = "\t")
    }
    ...
    fwrite(rs, file.path(report0, "asnp_region_summary.tsv"), sep = "\t")
    nl <- rbindlist(network_log, fill = TRUE)
    fwrite(nl, file.path(report0, "asnp_network_log.tsv"), sep = "\t")
    message("asnp done: ", nrow(hits), " aSNP overlap rows; ", if (nrow(hits)) nrow(hits[pass_asnp == TRUE]) else 0L, " rows pass asnp filters")
    invisible(NULL)
}
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 🚩 Extra QC / sensitivity analyses for reported archaic-like loci
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
run_extra_qc <- function(script_args) {
    need_pkg(c("data.table", "ggplot2", "scales"))
    args <- script_args
    ...
    split_vec <- function(x, type = c("character", "numeric", "integer")) {
        type <- match.arg(type)
        if (is.null(x) || !nzchar(x)) return(vector(type, 0))
        z <- unlist(strsplit(x, "[, ]+"))
        z <- z[nzchar(z)]
        if (type == "numeric") return(as.numeric(z))
        if (type == "integer") return(as.integer(z))
        z
    }
    root <- normalizePath(get_arg("--dirout_root", "/mnt/d/analysis/gu/avcf"), winslash = "/", mustWork = FALSE)
    traits <- split_vec(get_arg("--traits", "bald bald12 bald13 bald14"), "character")
    outdir <- normalizePath(get_arg("--outdir", file.path(root, "extra")), winslash = "/", mustWork = FALSE)
    dir.create(outdir, recursive = TRUE, showWarnings = FALSE)
    pils_grid <- split_vec(get_arg("--pils_grid", "0.1,0.01,0.001,0.0001"), "numeric")
    freq_grid <- split_vec(get_arg("--freq_grid", "0.01,0.025,0.05,0.10"), "numeric")
    diag_n_grid <- split_vec(get_arg("--diag_n_grid", "1,3,5,10"), "integer")
    lineage_prop_grid <- split_vec(get_arg("--lineage_prop_grid", "0.5,0.7,0.8,0.9"), "numeric")
    hap_n_grid <- split_vec(get_arg("--hap_n_grid", "5,10,20"), "integer")

    afr_max_cols <- c("YRI_max_freq", "LWK_max_freq", "GWD_max_freq", "MSL_max_freq", "ESN_max_freq")
    read_tsv <- function(f) {
        if (!file.exists(f) || file.info(f)$size == 0) return(data.table())
        tryCatch(fread(f, sep = "\t", quote = "", fill = TRUE, showProgress = FALSE), error = function(e) data.table())
    }
    add_trait <- function(x, tr) {
        if (!nrow(x)) return(x)
        if (!"trait" %in% names(x)) x[, trait := tr]
        x
    }
    read_all <- function(rel) {
        rbindlist(lapply(traits, function(tr) {
            f <- file.path(root, tr, rel)
            add_trait(read_tsv(f), tr)
        }), fill = TRUE)
    }

    pick <- read_all(file.path("lead", "pick.tsv"))
    region <- read_all(file.path("report", "region_summary.tsv"))
    inherited <- read_all(file.path("report", "inherited_region.tsv"))
    strict <- read_all(file.path("report", "inherited_region.strict.tsv"))
    yri_filter <- read_all(file.path("report", "yri_filter.tsv"))
    core <- read_all(file.path("report", "core_risk.tsv"))
    arch <- read_all(file.path("report", "core_archaic_match.tsv"))
    hap <- read_all(file.path("report", "hap_match.tsv"))

    if (!nrow(region)) stop("No report/region_summary.tsv files found under: ", root, call. = FALSE)

    num_cols <- c("lead_chr", "lead_bp", "core_start", "core_end", "n_ld_snp", "core_size_bp", "p_ils",
                  "yri_freq", "yri_freq_all", "n_yri_filter_sites", afr_max_cols,
                  "strict_lineage_prop_match_risk", "strict_lineage_n_compared_risk",
                  "strict_lineage_n_match_risk", "strict_best_archaic_prop_match_risk",
                  "strict_hap_n_gt_min", "strict_hap_carry_risk_n_gt_min", "strict_best_carry_risk_hap_n")
    for (nm in intersect(num_cols, names(region))) region[, (nm) := suppressWarnings(as.numeric(get(nm)))]
    for (nm in intersect(num_cols, names(inherited))) inherited[, (nm) := suppressWarnings(as.numeric(get(nm)))]

    calc_afrmax <- function(dt) {
        for (nm in afr_max_cols) if (!nm %in% names(dt)) dt[, (nm) := NA_real_]
        dt[, AFR5_max_freq := do.call(pmax, c(.SD, list(na.rm = TRUE))), .SDcols = afr_max_cols]
        dt[!is.finite(AFR5_max_freq), AFR5_max_freq := NA_real_]
        dt
    }
    region <- calc_afrmax(region)
    inherited <- calc_afrmax(inherited)

    count_unique <- function(dt, cols = c("trait", "id")) {
        if (!nrow(dt) || !all(cols %in% names(dt))) return(0L)
        nrow(unique(dt[, ..cols]))
    }
    trait_levels <- unique(c(traits, "ALL"))
    summary_by_trait <- rbindlist(lapply(trait_levels, function(tr) {
        pp <- if (tr == "ALL") pick else pick[trait == tr]
        rr <- if (tr == "ALL") region else region[trait == tr]
        ii <- if (tr == "ALL") inherited else inherited[trait == tr]
        ss <- if (tr == "ALL") strict else strict[trait == tr]
        data.table(
            trait = tr,
            n_cojo_pick = count_unique(pp),
            n_archaic_matched_before_yri = count_unique(rr),
            n_yri_retained = count_unique(ii),
            n_strict_retained = count_unique(ss),
            n_yri_retained_with_AFR5_max_le_0.05 = count_unique(ii[AFR5_max_freq <= 0.05]),
            n_yri_retained_with_AFR5_max_le_0.01 = count_unique(ii[AFR5_max_freq <= 0.01])
        )
    }), fill = TRUE)
    fwrite(summary_by_trait, file.path(outdir, "locus_screening_summary.tsv"), sep = "\t")

    flag_cols <- c("strict_p_ils_pass", "strict_lineage_pass", "strict_hap_pass", "strict_yri_pass", "strict_filter_pass")
    for (nm in flag_cols) if (!nm %in% names(inherited)) inherited[, (nm) := NA]
    keep_cols <- c("trait", "id", "lead_chr", "lead_snp", "lead_bp", "core_start", "core_end", "n_ld_snp",
                   "core_size_bp", "p_ils", "best_lineage", "matched_archaics", "n_yri_filter_sites",
                   "YRI_max_freq", "LWK_max_freq", "GWD_max_freq", "MSL_max_freq", "ESN_max_freq", "AFR5_max_freq",
                   "strict_lineage_n_compared_risk", "strict_lineage_n_match_risk", "strict_lineage_prop_match_risk",
                   "strict_best_archaic", "strict_best_archaic_prop_match_risk", "strict_hap_n_gt_min",
                   "strict_hap_carry_risk_n_gt_min", "strict_best_carry_risk_hap_n", flag_cols, "strict_filter_reason")
    for (nm in setdiff(keep_cols, names(inherited))) inherited[, (nm) := NA]
    quality <- inherited[, ..keep_cols]
    setorder(quality, lead_chr, lead_bp, trait, id)
    fwrite(quality, file.path(outdir, "candidate_quality_table.tsv"), sep = "\t")

    if (nrow(core)) {
        for (nm in c("is_diagnostic_archaic", "risk_core_allele", "risk_core_side", "pos", "ref", "alt", "YRI_diagnostic_freq", "risk_freq_delta")) if (!nm %in% names(core)) core[, (nm) := NA]
        selected_key <- unique(quality[, .(trait, id)])
        diag <- merge(core, selected_key, by = c("trait", "id"), nomatch = 0)
        diag <- diag[is_diagnostic_archaic == TRUE | as.character(is_diagnostic_archaic) == "TRUE"]
        if (nrow(diag)) {
            diag[, chr_label := fifelse(lead_chr %in% c(23, "23", "X"), "X", as.character(lead_chr))]
            diag[, variant_uid := paste(chr_label, pos, ref, alt, sep = ":")]
            diag_keep <- unique(c("trait", "id", "lead_chr", "lead_snp", "lead_bp", "variant_uid", "pos", "ref", "alt",
                                  "risk_core_allele", "risk_core_side", "risk_freq_delta",
                                  intersect(names(diag), c("yri_diagnostic_freq", "YRI_diagnostic_freq", "YRI_max_freq", "diagnostic_archaic_allele"))))
            fwrite(unique(diag[, ..diag_keep]), file.path(outdir, "diagnostic_variants_for_haplotype_proxy.tsv"), sep = "\t")
        } else {
            fwrite(data.table(), file.path(outdir, "diagnostic_variants_for_haplotype_proxy.tsv"), sep = "\t")
        }
    }

    for (nm in c("p_ils", "n_yri_filter_sites", "YRI_max_freq", "AFR5_max_freq",
                 "strict_lineage_prop_match_risk", "strict_hap_carry_risk_n_gt_min")) {
        if (!nm %in% names(region)) region[, (nm) := NA_real_]
    }
    region[, locus_key := paste(trait, id, sep = "|")]
    grid <- CJ(p_ils_th = pils_grid,
               freq_panel = c("YRI", "AFR5"),
               freq_th = freq_grid,
               min_diag_sites = diag_n_grid,
               min_lineage_prop = lineage_prop_grid,
               min_common_risk_hap = hap_n_grid,
               unique = TRUE)
    score_one <- function(p_ils_th, freq_panel, freq_th, min_diag_sites, min_lineage_prop, min_common_risk_hap) {
        freq_col <- if (freq_panel == "AFR5") "AFR5_max_freq" else "YRI_max_freq"
        z <- region[
            is.finite(p_ils) & p_ils < p_ils_th &
              is.finite(get(freq_col)) & get(freq_col) <= freq_th &
              is.finite(n_yri_filter_sites) & n_yri_filter_sites >= min_diag_sites &
              is.finite(strict_lineage_prop_match_risk) & strict_lineage_prop_match_risk >= min_lineage_prop &
              is.finite(strict_hap_carry_risk_n_gt_min) & strict_hap_carry_risk_n_gt_min >= min_common_risk_hap
        ]
        data.table(n_loci = uniqueN(z$locus_key), loci = paste(unique(z$locus_key), collapse = ";"))
    }
    sens <- grid[, score_one(p_ils_th, freq_panel, freq_th, min_diag_sites, min_lineage_prop, min_common_risk_hap),
                 by = .(p_ils_th, freq_panel, freq_th, min_diag_sites, min_lineage_prop, min_common_risk_hap)]
    fwrite(sens, file.path(outdir, "filter_sensitivity_grid.tsv"), sep = "\t")

    metric_specs <- data.table(
        metric = c("p_ils", "YRI_max_freq", "AFR5_max_freq", "n_yri_filter_sites", "strict_lineage_prop_match_risk", "strict_best_archaic_prop_match_risk", "strict_hap_carry_risk_n_gt_min"),
        direction = c("lower", "lower", "lower", "higher", "higher", "higher", "higher")
    )
    emp <- rbindlist(lapply(seq_len(nrow(quality)), function(i) {
        cand <- quality[i]
        bg <- region[!(trait == cand$trait & id == cand$id)]
        if (is.finite(cand$n_ld_snp) && is.finite(cand$core_size_bp)) {
            bgm <- bg[n_ld_snp >= 0.5 * cand$n_ld_snp & n_ld_snp <= 2 * cand$n_ld_snp &
                        core_size_bp >= 0.5 * cand$core_size_bp & core_size_bp <= 2 * cand$core_size_bp]
            if (nrow(bgm) >= 20) bg <- bgm
        }
        rbindlist(lapply(seq_len(nrow(metric_specs)), function(j) {
            m <- metric_specs$metric[j]
            d <- metric_specs$direction[j]
            if (!m %in% names(cand) || !m %in% names(bg)) return(NULL)
            v <- suppressWarnings(as.numeric(cand[[m]]))
            x <- suppressWarnings(as.numeric(bg[[m]]))
            x <- x[is.finite(x)]
            if (!is.finite(v) || !length(x)) return(data.table(trait = cand$trait, id = cand$id, lead_snp = cand$lead_snp, metric = m, value = v, background_n = length(x), empirical_p = NA_real_, direction = d))
            p <- if (d == "lower") (1 + sum(x <= v, na.rm = TRUE)) / (1 + length(x)) else (1 + sum(x >= v, na.rm = TRUE)) / (1 + length(x))
            data.table(trait = cand$trait, id = cand$id, lead_snp = cand$lead_snp, metric = m, value = v, background_n = length(x), empirical_p = p, direction = d)
        }), fill = TRUE)
    }), fill = TRUE)
    fwrite(emp, file.path(outdir, "empirical_background.tsv"), sep = "\t")

    base_sens <- sens[min_diag_sites == 1 & min_lineage_prop == 0.5 & min_common_risk_hap == 5]
    if (nrow(base_sens)) {
        p1 <- ggplot(base_sens, aes(x = factor(freq_th), y = factor(p_ils_th), fill = n_loci)) +
            geom_tile(color = "white") + geom_text(aes(label = n_loci), size = 3) +
            facet_wrap(~freq_panel) +
            labs(x = "African-panel maximum frequency threshold", y = "p-ILS threshold",
                 fill = "Retained loci", title = "Sensitivity of candidate archaic-like MPB loci") +
            theme_minimal(base_size = 10)
        ggsave(file.path(outdir, "fig_filter_sensitivity_yri.pdf"), p1, width = 7.2, height = 4.2)
    }

    plot_dt <- copy(region)
    plot_dt[, selected := paste(trait, id, sep = "|") %in% paste(quality$trait, quality$id, sep = "|")]
    plot_dt[, label := ifelse(selected, paste0(lead_snp, "\n", trait), "")]
    if (nrow(plot_dt)) {
        p2 <- ggplot(plot_dt, aes(x = AFR5_max_freq, y = -log10(p_ils), size = n_yri_filter_sites)) +
            geom_point(aes(shape = selected), alpha = 0.7) +
            geom_text(aes(label = label), hjust = 0, vjust = 0, size = 2.8, check_overlap = TRUE) +
            scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0, NA)) +
            labs(x = "Maximum diagnostic allele frequency across YRI/LWK/GWD/MSL/ESN",
                 y = "-log10(p-ILS)", size = "Diagnostic sites", shape = "Reported") +
            theme_minimal(base_size = 10)
        ggsave(file.path(outdir, "fig_candidate_quality.pdf"), p2, width = 7.2, height = 4.5)
    }
    message("Wrote extra GU analysis outputs to: ", outdir)
    invisible(NULL)
}

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 🚩 UKB haplotype-proxy association from diagnostic archaic-like variants
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
run_proxy_assoc <- function(script_args) {
    need_pkg("data.table")
    args <- script_args
    ...
    split_vec <- function(x) {
        ...
        z <- unlist(strsplit(x, "[, ]+")); z[nzchar(z)]
    }
    make_scores <- function() {
        diag_tsv <- get_arg("--diag_tsv")
        outdir <- get_arg("--outdir")
        if (is.null(diag_tsv) || !file.exists(diag_tsv)) stop("missing --diag_tsv: ", diag_tsv, call. = FALSE)
        if (is.null(outdir)) stop("missing --outdir", call. = FALSE)
        dir.create(outdir, recursive = TRUE, showWarnings = FALSE)
        x <- fread(diag_tsv)
        need <- c("trait", "id", "lead_chr", "lead_snp", "lead_bp", "variant_uid", "risk_core_allele")
        miss <- setdiff(need, names(x))
        if (length(miss)) stop("diagnostic table missing columns: ", paste(miss, collapse = ", "), call. = FALSE)
        x <- x[!is.na(risk_core_allele) & nzchar(risk_core_allele) & !is.na(variant_uid) & nzchar(variant_uid)]
        if (!nrow(x)) stop("diagnostic table has no usable variants: ", diag_tsv, call. = FALSE)
        x[, score := 1]
        x[, chr_label := ifelse(lead_chr %in% c(23, "23", "X"), "X", as.character(lead_chr))]
        manifest <- x[, {
            safe_id <- gsub("[^A-Za-z0-9_.-]", "_", paste0(trait[1], "_", id[1]))
            f <- file.path(outdir, paste0(safe_id, ".score.tsv"))
            sc <- unique(.SD[, .(variant_uid, risk_core_allele, score)])
            setnames(sc, c("variant_uid", "risk_core_allele", "score"), c("ID", "A1", "SCORE"))
            fwrite(sc, f, sep = "\t")
            .(chr = chr_label[1], score_file = f, lead_snp = lead_snp[1], lead_bp = lead_bp[1], n_score_variants = nrow(sc))
        }, by = .(trait, id)]
        setcolorder(manifest, c("trait", "id", "chr", "score_file", "lead_snp", "lead_bp", "n_score_variants"))
        fwrite(manifest, file.path(outdir, "score_manifest.tsv"), sep = "\t")
        message("Wrote proxy score manifest: ", file.path(outdir, "score_manifest.tsv"))
    }
    read_sscore <- function(f) {
        z <- fread(f)
        idc <- intersect(c("IID", "eid", "#FID", "FID"), names(z))[1]
        scorec <- intersect(c("SCORE1_AVG", "SCORE1_SUM", "SCORE1"), names(z))[1]
        if (is.na(idc) || is.na(scorec)) stop("Cannot identify ID/score columns in ", f, call. = FALSE)
        z <- z[, .(eid = get(idc), hap_score = as.numeric(get(scorec)))]
        z
    }
    fit_models <- function() {
        mf <- get_arg("--sscore_manifest")
        pheno_cov <- get_arg("--pheno_cov")
        id_col <- get_arg("--id_col", "eid")
        outcomes <- split_vec(get_arg("--outcomes", "bald bald12 bald13 bald14"))
        binary <- split_vec(get_arg("--binary_outcomes", "bald12 bald13 bald14"))
        covars <- split_vec(get_arg("--covars", "age PC1 PC2 PC3 PC4 PC5 PC6 PC7 PC8 PC9 PC10"))
        outdir <- get_arg("--outdir")
        if (is.null(mf) || !file.exists(mf)) stop("missing --sscore_manifest: ", mf, call. = FALSE)
        if (is.null(pheno_cov) || !file.exists(pheno_cov)) stop("missing --pheno_cov: ", pheno_cov, call. = FALSE)
        if (is.null(outdir)) stop("missing --outdir", call. = FALSE)
        dir.create(outdir, recursive = TRUE, showWarnings = FALSE)
        man <- fread(mf)
        if (!"sscore" %in% names(man)) {
            man <- fread(mf, header = FALSE, col.names = c("trait", "id", "chr", "sscore", "lead_snp", "lead_bp"))
        }
        ph <- fread(pheno_cov)
        if (!id_col %in% names(ph)) stop("id_col not found in pheno_cov: ", id_col, call. = FALSE)
        if (id_col != "eid") setnames(ph, id_col, "eid")
        covars <- covars[covars %in% names(ph)]
        missing_out <- setdiff(outcomes, names(ph))
        if (length(missing_out)) stop("outcome columns missing from pheno_cov: ", paste(missing_out, collapse = ", "), call. = FALSE)
        res <- rbindlist(lapply(seq_len(nrow(man)), function(i) {
            if (!file.exists(man$sscore[i])) return(NULL)
            z <- read_sscore(man$sscore[i])
            d <- merge(ph, z, by = "eid", all = FALSE)
            d <- d[is.finite(hap_score)]
            if (!nrow(d) || length(unique(d$hap_score)) < 2) return(NULL)
            d[, hap_score_z := as.numeric(scale(hap_score))]
            rbindlist(lapply(outcomes, function(y) {
                dd <- d[!is.na(get(y)) & is.finite(hap_score_z)]
                if (nrow(dd) < 100) return(NULL)
                rhs <- paste(c("hap_score_z", covars), collapse = " + ")
                form <- as.formula(paste(y, "~", rhs))
                fam <- if (y %in% binary) stats::binomial() else stats::gaussian()
                fit <- tryCatch(stats::glm(form, data = dd, family = fam), error = function(e) NULL)
                if (is.null(fit) || !"hap_score_z" %in% rownames(summary(fit)$coef)) return(NULL)
                cc <- summary(fit)$coef["hap_score_z", ]
                beta <- unname(cc["Estimate"]); se <- unname(cc["Std. Error"])
                p <- unname(cc[ncol(summary(fit)$coef)])
                data.table(trait_region = man$trait[i], id = man$id[i], lead_chr = man$chr[i], lead_snp = man$lead_snp[i], lead_bp = man$lead_bp[i], outcome = y,
                           n = nrow(dd), beta = beta, se = se, z = beta / se, p = p,
                           model = if (y %in% binary) "logistic" else "linear",
                           covars = paste(covars, collapse = ";"))
            }), fill = TRUE)
        }), fill = TRUE)
        if (nrow(res)) {
            res[, p_fdr := p.adjust(p, method = "BH"), by = outcome]
            fwrite(res, file.path(outdir, "haplotype_proxy_assoc.tsv"), sep = "\t")
        } else {
            fwrite(data.table(), file.path(outdir, "haplotype_proxy_assoc.tsv"), sep = "\t")
        }
        message("Wrote proxy association table: ", file.path(outdir, "haplotype_proxy_assoc.tsv"))
    }
    if (cmd == "make_scores") make_scores()
    else if (cmd == "fit_models") fit_models()
    else stop("Usage: Rscript gu.R proxy_assoc make_scores|fit_models ...", call. = FALSE)
    invisible(NULL)
}
...
switch(cmd,
    ...
    asnp = run_asnp(cmd_args),
    extra_qc = run_extra_qc(cmd_args),
    proxy_assoc = run_proxy_assoc(cmd_args),
