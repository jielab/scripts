...
This is deliberately the first interpretation layer after ``phyml.py`` has
...
* ``region_matches.unfiltered.tsv``: every modern H type x archaic reference;
* ``region_match_segments.unfiltered.tsv``: every local high-similarity tract;
* ``region_candidate_families.unfiltered.tsv``: overlap-clustered tract families.
...
filters are applied later by ``phyml_evidence.py``.
"""
...
def prepare(args: argparse.Namespace) -> None:
    ...
    for locus_dir in dirs:
        ...
        # in region_matches.unfiltered.tsv.
    ...
    final = out / "final"
    write_tsv(
        final / "region_haplotypes.unfiltered.tsv",
        [
    ...
    write_tsv(
        final / "region_archaic_references.unfiltered.tsv",
        [
    ...
    write_tsv(
        final / "region_matches.unfiltered.tsv",
        [
            "locus_id", "chr", "core_start", "core_end", "hap_id", "hap_n",
    ...
    write_tsv(
        final / "region_haplotype_candidates.unfiltered.tsv",
        [
            "locus_id", "chr", "core_start", "core_end", "hap_id", "hap_n",
            "total_haplotype_copies", "hap_frequency", "copies",
            "archaic", "lineage", "n_compared", "n_match", "n_mismatch", "prop_match",
    ...
    write_tsv(
        final / "region_match_segments.unfiltered.tsv",
        [
    ...
    write_tsv(
        final / "region_candidate_families.unfiltered.tsv",
        [
    ...
    write_tsv(
        final / "region_scan_loci.unfiltered.tsv",
        [
    ...
    parameters = {
        ...
        "outputs": {
            "all_modern_haplotypes": "region_haplotypes.unfiltered.tsv",
            "all_archaic_references": "region_archaic_references.unfiltered.tsv",
            "all_haplotype_reference_pairs": "region_matches.unfiltered.tsv",
            "permissive_similarity_candidates": "region_haplotype_candidates.unfiltered.tsv",
            "local_similarity_segments": "region_match_segments.unfiltered.tsv",
            "overlap_clustered_families": "region_candidate_families.unfiltered.tsv",
...
def summarize(args: argparse.Namespace) -> None:
    ...
    family_rows = read_tsv(out / "final" / "region_candidate_families.unfiltered.tsv")
    ...
    write_tsv(
        final / "region_tree_clades.unfiltered.tsv",
        [
    ...
    if family_rows:
        ...
        for extra in (
            ...
            if extra not in fields:
                fields.append(extra)
        write_tsv(final / "region_candidate_families.unfiltered.tsv", fields, updated_families)
    print(
