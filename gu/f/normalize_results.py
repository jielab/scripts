...
CORE_FILE_PATTERNS={
    ...
    'as3':(),
    'phyml':(
        '**/final/*.tsv','**/final/gwas_parameters.json','**/final/evidence_parameters.json','**/final/*unfiltered.tsv.gz','**/loci/**/sites.tsv','**/loci/**/search_sites.tsv',
...
def stable_phyml_run(run_dir):
    ...
    try:
        ...
        if (final/'gwas_loci.tsv').exists():
            required=('gwas_parameters.json','gwas_haplotypes.tsv','gwas_copies.tsv',
                      'loci.tsv','trees.tsv','evidence_trees.tsv','haplotypes.tsv','haplotype_samples.tsv')
...
def portable_paths(df,package_root,columns):
    ...
    for column in columns:
        ...
        # Millions of rows commonly share one provenance path. Resolve each
        # distinct path once, especially on mounted Windows filesystems.
        mapped={value:convert(value) for value in df[column].dropna().unique()}
        df[column]=df[column].map(lambda value: mapped.get(value,value))
    return df
...
def iter_ibdmix(root,build,completed_runs=None,filter_output=None,daf_mask=None,audits=None):
    ...
    for f in files:
        ...
        if not meta.get('pipeline_version'):
            print(f'IBDMIX legacy result excluded; rerun required: {f}',flush=True)
            continue  # a corrected chromosome must never union with old calls
        ...
        for d in pd.read_csv(filtered,sep='\t',compression='infer',chunksize=200000):
            ...
            if meta.get('coordinate_system')!='0-based-half-open':
                # Legacy wrapper retained native 1-based half-open coordinates.
                d['start']=pd.to_numeric(d['start'],errors='coerce')-1
                d['end']=pd.to_numeric(d['end'],errors='coerce')-1
...
def discover_method_runs(root,fallback_build):
    ...
    def add(method,path,build,chrom,status='complete'):
        ...
        if method=='ibdmix':
            ...
            if not meta.get('pipeline_version'):
                status='legacy_invalid'; eligible=0; note='legacy IBDmix: invalid SNP-only archaic preparation / pooled populations; rerun required'
            else:
        ...
        if key in rows and rows[key][4]=='complete' and status!='complete':return
    ...
    for hp in sorted((root/'phyml').glob('**/final/haplotypes.tsv')):
        ...
        if trees.is_file() and trees.stat().st_size:
            try:
                ...
                tree_tips={};tree_lineages={};tree_pass={}
        # Keep carriers from every passing candidate-focused lineage tree.
        # trees.tsv is only the representative topology for the locus display.
        hap_lineages={}
        evidence_trees=hp.parent/'evidence_trees.tsv'
        if evidence_trees.is_file() and evidence_trees.stat().st_size:
            ed=pd.read_csv(evidence_trees,sep='\t',dtype=str,keep_default_na=False)
            for lid in ed['locus_id'].astype(str).unique():
                tree_tips[lid]=set(); tree_pass[lid]=0
            for _,row in ed.iterrows():
                if not truth_int(row.get('candidate_clade_pass',0)):continue
                lid=str(row.get('locus_id','')); lin=str(row.get('expected_lineage',''))
                tips={x for x in str(row.get('candidate_tips_in_clade','')).split(',') if x.startswith('H')}
                tree_tips.setdefault(lid,set()).update(tips)
                tree_pass[lid]=1
                for hid in tips:hap_lineages.setdefault((lid,hid),set()).add(lin)
        buffer=[]
        for _,hap in haps.iterrows():
            ...
            for copy in copies.split(';'):
                ...
                buffer.append({
                    ...
                    'candidate_lineage':','.join(sorted(hap_lineages.get((lid,hap_id),set()))) or tree_lineages.get(lid,''),
...
def build_locus_support(con,trajectory_bins=120):
      ...
      WITH tested AS (
        SELECT pc.dataset_id,pc.genome_build,pc.locus_id,l.chr,l.source_class,pc.sample_id,
               ...
               GROUP_CONCAT(DISTINCT CASE WHEN pc.tree_candidate_pass=1 THEN
                 CASE
                   WHEN LOWER(COALESCE(pc.candidate_lineage,'')) LIKE '%neander%' AND LOWER(COALESCE(pc.candidate_lineage,'')) LIKE '%denis%' THEN 'Neanderthal,Denisovan'
    ...
    for d,b,lid,chrom,src,pop,n,phy,ibd,trc,a3,phy_ibd,phy_trc,phy_a3 in con.execute(aggregate_sql):
        ...
        for method,(carriers,intersection) in values.items():
            ...
            elif method=='ibdmix':eligible,note=0,'IBDmix completion/provenance unavailable; excluded from GU evidence score'
...
def build_locus_evidence(con):
    ...
    loci=con.execute('''
      SELECT dataset_id,genome_build,locus_id,chr,source_class,start,end,input_start,input_end,
             ...
             candidate_clade_pass,candidate_clade_bootstrap,candidate_clade_specificity,candidate_clade_rule
      FROM loci WHERE method='phyml' ORDER BY dataset_id,genome_build,locus_id
    ''').fetchall()
    for d,b,lid,chrom,src,start,end,input_start,input_end,best_archaic,best_lineage,n_compared,n_match,prop_match,tree_status,candidate_lineage,has_outgroup,clade_pass,clade_boot,clade_specificity,clade_rule in loci:
        ...
        elif int(clade_pass or 0)!=1 and clade_rule=='supported_mixed_archaic_edge_lineage_unresolved':phyml_qc='archaic_lineage_unresolved'
        ...
        elif phyml_qc=='archaic_lineage_unresolved':state='mixed_archaic_tree_support_lineage_unresolved'
...
def main():
    ...
    ap.add_argument('--database',type=Path,help='SQLite destination; defaults to <analysis-root>/final/gu.sqlite')
    ...
    db=(args.database or (args.analysis_root/'final'/'gu.sqlite')).resolve(); db.parent.mkdir(parents=True,exist_ok=True)
