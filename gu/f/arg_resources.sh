...
arg_run_guarded() {
  ...
  local value state status features_set=0 method=needle previous= option
  for option in "$@"; do
    if [[ $previous == --method ]]; then method=$option; fi
    previous=$option
  done
  local -a args=()
  while (($#)); do
    case "$1" in
      ...
      --jobs|--arg-jobs)
        ...
        if [[ $action == build && $method != singer ]] && ((value>1)); then
      ...
      --features) features_set=1; args+=("$1"); shift;;
      *) args+=("$1"); shift;;
    esac
  done
  # Preparation does not require GRID feature anchors; explicit options still win.
  if [[ $action == prep_gen && $features_set == 0 ]]; then args+=(--features FALSE); fi
  ...
  export ARG_MEMORY_LIMIT_GB=$ram
  ...
  if [[ $action == build && $method != singer ]]; then
