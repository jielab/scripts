...
# Keep Needle's autosome discovery; X always uses the dedicated male PFILE.
arg_needle_pfile_for(){
  if [[ $1 != X ]]; then grid_pfile_for "$1"; return; fi
  local p; p=$(arg_source_prefix "$GRID_ARG_HAP_DIR" X)
  [[ -s $p.pgen && ( -s $p.pvar || -s $p.pvar.zst ) && -s $p.psam ]] && printf '%s\n' "$p"
}
arg_needle_sample_for(){
  if [[ $1 != X ]]; then grid_sample_for "$1"; return; fi
  local p; p=$(arg_source_prefix "$GRID_ARG_HAP_DIR" X)
  [[ -s $p.psam ]] && printf '%s.psam\n' "$p"
}
arg_needle_bgen_for(){
  [[ $1 != X ]] || return 0
  grid_bgen_for "$1"
...
prepared_record(){
  local c=$1 in index
  in=$(arg_source_prefix "$IN_DIR" "$c").vcf.gz
  ...
  if [[ $c == X ]]; then
    ...
    [[ ! -s ${REFGEN_PFILE_DIR}/chrX.male.psam ]] || python3 "$HERE/arg.py" cache --text-file "${REFGEN_PFILE_DIR}/chrX.male.psam"
  fi
}
prep_one(){
  local c=$1 in out meta expected replace_word=FALSE
  in=$(arg_source_prefix "$IN_DIR" "$c").vcf.gz
  ...
  local -a x_args=()
  if [[ $c == X && -s ${REFGEN_PFILE_DIR}/chrX.male.psam ]]; then x_args+=(--x-psam "${REFGEN_PFILE_DIR}/chrX.male.psam"); fi
  ...
  if [[ $c == X ]]; then
    ( source "$HERE/comm.sh"; gu_validate_chrx_male_vcf "$out" "$REFGEN_GRCH" "$REFGEN_SAMPLE_FILE" )
  fi
...
set -euo pipefail
local -a singer_args=("$@")
local singer_requested=0
for ((i=0;i<$#;i++)); do
  if [[ ${singer_args[i]} == --method && ${singer_args[i+1]:-} == singer ]]; then singer_requested=1; fi
done
if ((singer_requested)); then
  local runtime=${GU_SINGER_ENV:-$HOME/.venvs/gu-singer}
  [[ -x $runtime/bin/python ]] || { echo "ERROR: run bash $HERE/install_singer.sh" >&2; exit 2; }
  export PYTHONNOUSERSITE=1
  exec "$runtime/bin/python" -u "$HERE/arg_singer.py" "${singer_args[@]}"
fi
HERE=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
...
case "$method" in needle|tsinfer|threads|singer) ;; *) die "--method must be needle, tsinfer, threads or singer";; esac
...
if [[ -z $gen4arg_dir ]]; then
  ...
  elif [[ $method == needle ]]; then gen4arg_dir=${dir_pfile}.4arg.needle; else gen4arg_dir=${dir_vcf}.4arg.tsinfer; fi
fi
...
if [[ " ${CHRS[*]} " == *" X "* ]]; then
  python3 "$HERE/arg_input.py" --psam "$(arg_source_prefix "$GRID_ARG_HAP_DIR" X).psam"
fi
...
PY
  if [[ $c == X ]]; then
    python3 "$HERE/arg_input.py" --psam "$(arg_needle_sample_for X)" --sample-map "$GRID_ARG_TREES_DIR/chrX.sample_map.tsv"
    python3 "$HERE/arg.py" needle-x check --tree "$GRID_ARG_TREES_DIR/chrX.trees" --identities "$PREP/chrX.haplotypes.tsv" --output "$GRID_ARG_TREES_DIR/chrX.sample_map.tsv" --psam "$(arg_needle_sample_for X)" --keep "$PREP/panel/X/arg.keep.txt"
  fi
}
...
make_panel_one(){
  ...
  local sample; sample=$(arg_needle_sample_for "$c" || true)
...
prepare_record(){
  local c=$1 pf s m
  select_panel "$c"
  pf=$(arg_needle_pfile_for "$c" || true); s=$(arg_needle_sample_for "$c" || true); m=$(grid_map_for "$c" || true)
  ...
  if [[ -n $pf ]]; then
    args+=(--file "$pf.pgen")
    if [[ ! -s $pf.pvar && -s $pf.pvar.zst ]]; then args+=(--file "$pf.pvar.zst"); else args+=(--file "$pf.pvar"); fi
  else args+=(--file "$(arg_needle_bgen_for "$c")"); fi
...
export_chr() (
  ...
  select_panel "$c"
  b=$(arg_needle_bgen_for "$c" || true); pf=$(arg_needle_pfile_for "$c" || true)
  s=$(arg_needle_sample_for "$c" || true); srcmap=$(grid_map_for "$c" || true)
  ...
  cleanup_export(){
    rm -f -- "$subset.pgen" "$subset.pvar" "$subset.pvar.zst" "$subset.psam" "$subset.log" "$prefix.haps" "$prefix.haps.gz.x.next" "$prefix.haps.gz.unique.next"
    if [[ ! -s $prefix.outputs.json ]]; then rm -f -- "$prefix.haps.gz"; fi
  }
  ...
  local -a cmd
  if [[ -n $pf ]]; then
    cmd=(plink2 --pfile "$pf"); [[ -s $pf.pvar || ! -s $pf.pvar.zst ]] || cmd+=(vzs)
  ...
  cmd+=(--keep "$KEEP" --indiv-sort f "$ORDER" --snps-only just-acgt --max-alleles 2 --maf "$GRID_ARG_MAF_MIN" --geno "$GRID_ARG_GENO_MAX" --sort-vars --make-pgen --out "$subset")
  grid_run_logged "$PREP/log/arg.prepare.chr$c.log" "${cmd[@]}"
  grid_run_logged "$PREP/log/arg.prepare.chr$c.log" plink2 --pfile "$subset" --threads "$GRID_ARG_THREADS" --export haps bgz --out "$prefix"
  if [[ $c == X ]]; then
    grid_run python3 "$HERE/arg.py" needle-x pack --prefix "$prefix" --psam "$subset.psam" --keep "$KEEP" --build "$REFGEN_GRCH"
  fi
  grid_run python3 "$HERE/arg.py" unique-haps --haps "$prefix.haps.gz" --chr "$c" --out "$prefix.positions.qc.json"
...
infer_chr() (
  ...
  local temporary
  temporary=$(mktemp -d "$WORK/chr${c}.run.XXXXXX")
  trap 'rm -rf -- "$temporary"; rm -f -- "$GRID_ARG_OUT/argn/chr$c.argn.next"' EXIT
  trap 'exit 130' INT
  trap 'exit 143' TERM
  trap 'exit 129' HUP
  local WORK=$temporary INF=$temporary/infer
  mkdir -p "$INF"
  select_panel "$c"
  prefix=$PREP/chr$c
  require_prepared "$c"
  if [[ $GRID_REPLACE == FALSE && -s $GRID_ARG_OUT/argn/chr$c.input.json && -s $GRID_ARG_OUT/argn/chr$c.argn ]]; then
    inferred_record "$c" > "$temporary/request.json"
    if cmp -s "$temporary/request.json" "$GRID_ARG_OUT/argn/chr$c.input.json"; then
      echo "SKIP verified ARG: chr$c"; return
    fi
  fi
  mkdir -p "$WORK/input"
...
check_one(){
  ...
  if [[ $c == X ]]; then
    ( source "$HERE/comm.sh"; gu_validate_chrx_male_vcf "$(prepared_vcf "$c")" "$REFGEN_GRCH" "$REFGEN_SAMPLE_FILE" )
...
cleanup() { rm -rf -- "$work"; if [[ -n ${out_part:-} ]]; then rm -f -- "$out_part" "$out_part.tbi" "$out_part.csi"; fi; }
trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
...
rm -f -- "$tagged_bcf" "$tagged_bcf.csi"
...
rm -f -- "$filtered_bcf" "$filtered_bcf.csi"
...
case "${backend,,}" in
  ...
  singer) method=singer; ref_action=check;;
  *) echo "ERROR: GU ARG method must be needle, tsinfer, threads or singer" >&2; exit 2;;
esac
...
args=(check --method "$method" --format "$format" --action "$ref_action"
      --dir-gen "${GU_TARGET_ROOT:?}" --arg-dir "${GU_ARG_DIR:-$GU_TARGET_ROOT/arg.$method}"
...
if [[ ( $method == needle || $method == threads ) && -n ${GU_TARGET_GEN_PREFIX:-} ]]; then
...
method=${method,,}
case "$method" in needle|tsinfer|threads|singer) ;; *) echo 'ERROR: --method must be needle, tsinfer, threads or singer' >&2; exit 2;; esac
if [[ -z $arg_dir ]]; then arg_dir=$gen_root/arg.$method; fi
log_dir=$arg_dir/log
mkdir -p "$log_dir"
