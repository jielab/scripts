...
def find_anchor_row(vcf: Path, contig: str, locus: dict, aliases=(), genome_build="") -> tuple[list[str], list[str] | None]:
    samples, rows = query_rows(vcf, f"{contig}:{locus['start'] + 1}-{locus['end']}")
    exact = [row for row in rows if len(row) >= 5 and locus["name"] in row[2].split(";")]
    if not exact and genome_build:
        # A verified, build-specific position AND REF/ALT match is mandatory.
        # Never infer an rsID from proximity or from the BED interval endpoint.
        keys = {(str(a['pos']), a['ref'], a['alt']) for a in aliases
                if a['genome_build'] == genome_build and a['marker_id'] == locus['name']
                and a['chr'] == locus['chrom']}
        exact = [row for row in rows if len(row) >= 5 and (row[1], row[3], row[4]) in keys]
    if len(exact) > 1:
...
def main() -> None:
    ...
    parser.add_argument("--genome-build", choices=("GRCh37", "GRCh38"), default="")
    parser.add_argument("--marker-map", type=Path, default=Path(__file__).with_name("phyml_markers.tsv"))
    ...
    aliases = read_tsv(args.marker_map)
    ...
    for locus in loci:
        ...
        if provided:
            samples, anchor = find_anchor_row(modern, vcf_contig(modern, locus["chrom"]), locus, aliases, args.genome_build)
        else:
            samples, anchor = [], None
