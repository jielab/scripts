    ...
    def test_five_reference_masks_intersect_common_exclusions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with ExitStack() as stack:
                ...
                cpg_cache = stack.enter_context(patch.object(ibd,'cached_cpg_sites',return_value=cpg.copy()))
                ibd.prepare_masks(root/'resources','1',modern,fasta,root,root/'out',axt_root=root/'axt',refs=list(phy.REFS))
            self.assertEqual(cpg_cache.call_args.args[-1], root/'derived')
            self.assertTrue((root/'derived/combined/chr1').is_dir())
            self.assertFalse((root/'resources').exists())
            self.assertTrue((root/'out/Altai.exclude.bed').resolve().is_relative_to(root/'derived'))
