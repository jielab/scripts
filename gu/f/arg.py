...
def needle_x_pack(prefix, psam, build, keep):
    ...
    haps = Path(prefix + '.haps.gz')
    if not haps.exists(): haps = Path(prefix + '.haps')
    tmp = haps.with_name(haps.name + '.x.next')
    compressed = haps.suffix == '.gz'
    retained = removed = 0
    try:
        with (gzip.open(haps, 'rt') if compressed else haps.open()) as src, (gzip.open(tmp, 'wt', compresslevel=1) if compressed else tmp.open('w')) as dest:
            for line in src:
                z = line.split()
...
def unique_clean(path, chromosome):
    ...
    try:
        ...
        with (gzip.open(path, 'rt') if compressed else path.open()) as source, (gzip.open(temporary, 'wt', compresslevel=1) if compressed else temporary.open('w', newline='\n')) as output:
