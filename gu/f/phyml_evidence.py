...
from phyml_thresholds import PROFILES, apply_profile, ils_probability
...
def best_contiguous_cluster(positions: list[int], max_gap: int) -> list[int]:
    ...
    for pos in positions[1:]:
        if max_gap > 0 and pos - clusters[-1][-1] > max_gap:
            clusters.append([pos])
        else:
...
def prepare(args: argparse.Namespace) -> None:
    ...
    for locus_dir in dirs:
        ...
        anchor_names = {locus_id, meta.get("name", ""), anchor_summary.get("anchor_id", "")}
        ...
        for row in hap_rows:
            ...
            for lineage, payload in lineage_payload.items():
                ...
                candidate_ils_p = ils_probability(int(candidate_bp)) if candidate_bp else 1.0
                ...
                reasons: list[str] = []
                if args.max_ils_probability < 1 and candidate_ils_p >= args.max_ils_probability:
                    reasons.append("ils_probability_above_threshold")
                ...
                score = {
                    ...
                    "candidate_ils_probability": candidate_ils_p,
    ...
    params = {
        ...
        "compatibility_limits": "region-first SNP set and derived-state definition retained; not an exact replay of risk-LD-selected legacy SNPs",
        "ils_model": {"recombination_cm_mb": 0.53, "split_years": 550000,
                      "archaic_age_years": 50000, "generation_years": 29,
                      "length_basis": "observed_candidate_diagnostic_span", "comparison": "p < cutoff"},
        "schema_version": 3,
        ...
        "thresholds": {
            "max_ils_probability": args.max_ils_probability,
...
def add_common_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--threshold-profile", choices=tuple(PROFILES), default="legacy_compatible")
    parser.add_argument("--max-ils-probability", type=float, default=None)
    ...
    parser.add_argument("--min-diagnostic-sites", type=int, default=None)
    parser.add_argument("--min-diagnostic-match-prop", type=float, default=None)
    parser.add_argument("--min-candidate-copies", type=int, default=None)
    parser.add_argument("--max-diagnostic-gap-bp", type=int, default=None, help="0 disables the added GU gap cutoff")
    ...
    parser.add_argument("--min-candidate-purity", type=float, default=None)
    parser.add_argument("--min-candidate-sensitivity", type=float, default=None)
...
def validate_args(args: argparse.Namespace) -> None:
    apply_profile(args)
    if not 0 < args.max_ils_probability <= 1:
        fail("--max-ils-probability must be in (0, 1]; 1 disables the ILS gate")
    if args.max_diagnostic_gap_bp < 0:
        fail("--max-diagnostic-gap-bp must be non-negative")
    ...
    for name in (
        ...
        "min_neanderthal_refs", "min_other_lineage_refs",
