...
GU v2 uses the explicit generate_ancestors -> match_ancestors -> match_samples
...
The input should be produced by ``f/arg_vcf_prep.sh``. It must contain only
...
def check_phasing(vcf_path: Path, max_records: int = 2000) -> dict[str, int]:
    """Check a representative set of records for unphased heterozygotes."""
    import pysam
...
def write_sample_map(tree_sequence: Any, sample_ids: list[str], output_path: Path) -> None:
    """Write actual tree node IDs and map diploid haplotypes to VCF samples."""
    seen: dict[str, int] = {}
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for node_id in tree_sequence.samples():
            ...
            sample = next((x for x in candidates if x in sample_ids), None)
...
def main() -> None:
    ...
    # A deterministic directory makes expensive chromosome-scale stages resumable.
    # Include input identity and inference settings so stale checkpoints cannot be
    # silently reused for a different VCF or parameter set.
    work_identity = json.dumps(
        {
            "vcf": str(args.vcf),
            "vcf_size": args.vcf.stat().st_size,
            "vcf_mtime_ns": args.vcf.stat().st_mtime_ns,
    ...
    work_tag = hashlib.sha256(work_identity.encode()).hexdigest()[:12]
    work = args.tmp_root / f"gu_arg_{args.out.stem}_{work_tag}"
    ...
    try:
        ...
        # Variational gamma estimates its own node-time distribution and tsdate
        # explicitly rejects a population size for this method. The older
        # inside-outside and maximization methods still require/use it.
        ...
        if dated_ts.num_samples != 2 * len(sample_ids):
            print(
                "WARNING: ARG sample-node count is not exactly twice the VCF sample "
                f"count ({dated_ts.num_samples} vs {2 * len(sample_ids)}).",
                flush=True,
            )
        ...
        if completed and not args.keep_work:
            ...
            print(
                f"[ARG] Incomplete run; keeping work directory: {work}",
                flush=True,
            )
