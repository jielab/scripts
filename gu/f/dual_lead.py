...
def apply_loci_file(authoritative,path,coordinate_format='bed',allow_missing=False):
    ...
    if missing and not allow_missing:raise ValueError('Requested regions lack an exact saved PhyML coordinate match: '+str(sorted(missing)))
    if not result and not allow_missing:raise ValueError('No loci in input file')
    return result
