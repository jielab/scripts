...
from arg_input import pfile_prefix, male_sample_ids, validate_x_map
...
def prepare(chrom, prep, work, grid, replace, require=False):
    p = prep / ('chr' + chrom)
    source = pfile_prefix(env('PFILE_DIR'), chrom)
    if chrom == 'X':
        male_sample_ids(str(source) + '.psam')
    ...
    if chrom == 'X':
        inputs.append(HERE/'arg_input.py')
    ...
    with tempfile.TemporaryDirectory(prefix=f'prepare.chr{chrom}.', dir=work) as tmp:
        ...
        try:
            ...
            for ext in extensions:Path(str(p)+'.'+ext+'.next').unlink(missing_ok=True)
...
def build(chrom, p, out, work, model, replace, action):
    try:
        with tempfile.TemporaryDirectory(prefix=f'build.chr{chrom}.', dir=work) as scratch:
            return _build(chrom, p, out, Path(scratch), model, replace, action)
    finally:
        if out is not None:
            for pending in (out/'threads'/f'chr{chrom}.threads.next', out/'trees'/f'chr{chrom}.trees.next'):
                pending.unlink(missing_ok=True)


def _build(chrom, p, out, work, model, replace, action):
...
def check(chrom, out):
    tree = out/'trees'/f'chr{chrom}.trees'; smap = tree.with_suffix('.sample_map.tsv')
    if chrom == 'X':
        validate_x_map(smap, str(pfile_prefix(env('PFILE_DIR'), chrom)) + '.psam')
...
def write_build_metadata(out, chrs, model):
    """A chrX-only rebuild must retain the completed autosomes in the manifest."""
    path = out/'ARG_THREADS_BUILD.json'
    previous = json.loads(path.read_text()).get('chromosomes', []) if path.is_file() else []
    completed = []
    for chrom in dict.fromkeys(previous + chrs):
        inference = out/'threads'/f'chr{chrom}.infer.json'
        converted = out/'trees'/f'chr{chrom}.converted.json'
        if (verified(inference) and verified(converted)
                and json.loads(converted.read_text())['request']['inference'] == stamp(inference)):
            completed.append(chrom)
    atomic_json(path, dict(method='threads', chromosomes=completed, model=model))
    if env('ARG_FORMAT') == 'trace':
        (out/'trace/threads/ARG_TRACE_BUILD.tsv').write_text(
            f'method\tthreads\nformat\ttrace\nbuild\tGRCh{env("GRCH")}\nchromosomes\t{" ".join(completed)}\n')
...
def main():
    ...
    if action not in ('prepare','infer'):
        write_build_metadata(out, chrs, model)
