...
function(input, output, session) {
  ...
  if (length(schema_errors)) {
    ...
    stop("GU SQLite schema is older than this Shiny app; rerun ./gu.sh normalize (missing ",paste(schema_errors,collapse=", "),").")
  }
  ...
  output$locus_interpretation <- renderUI({
    d <- selected_evidence(); if (!nrow(d)) return(tags$div(class="alert alert-warning", "No evidence row; rerun ./gu.sh normalize"))
  ...
  output$locus_evidence_table <- renderDT({
    d <- selected_evidence()
    if (!nrow(d)) return(datatable(data.frame(Message="No evidence row; rerun ./gu.sh normalize"),rownames=FALSE,options=list(dom="t")))
  ...
  output$locus_support_table <- renderDT({
    ...
    if (!nrow(d)) return(datatable(data.frame(Message = "No normalized support data; rerun ./gu.sh normalize"), rownames = FALSE, options = list(dom = "t")))
    ...
    d$phyml_carriers_supported <- paste0(d$n_phyml_supported, " / ", d$n_phyml_carriers)
