...
gu_local_igv_references <- function(root) {
  ...
  for(genome in c("hg19","hg38")) {
    ...
    fasta_url<-paste0("gu_reference/",name)
    index_url<-paste0("gu_reference/",genome,".fai")
...
gu_igv_reference_response <- function(req,resources) {
  route<-req$PATH_INFO
