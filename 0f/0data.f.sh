...
gwas_clean_need_file(){ [[ -s "$1" ]] || { echo "ERROR: missing or empty file: $1" >&2; exit 1; }; }
gwas_clean_has_data_rows(){ [[ -s "$1" ]] && awk 'NR>1{found=1; exit} END{exit !found}' "$1"; }
gwas_clean_zcat(){
  local f="$1"
  case "$f" in
    *.gz|*.bgz)
...
gwas_clean_detect_fs(){
  local f="$1" line
  line=$(set +o pipefail; gwas_clean_zcat "$f" | head -n 1 | tr -d '\r' || true)
  case "$line" in
    *$'\t'*) printf '\t';;
    *) printf '[ \t]+';;
  esac
...
gwas_clean_header_line(){
  local f="$1"
  (set +o pipefail; gwas_clean_zcat "$f" | head -n 1 | tr -d '\r' || true)
}
...
std_small(){
  ...
  awk -v FS="$input_fs" -v OFS='\t' \
    ...
    }' "$cand_uniq" | gwas_clean_compress > "$tmp"
...
gwas_clean_copy_small_to_final(){
  if [[ "$FORCE" != "TRUE" ]] && gwas_clean_gzip_ok "$FINAL"; then
...
gwas_clean_liftover_small_to_final(){
  ...
  command -v "$LIFTOVER_BIN" >/dev/null 2>&1 || { echo "ERROR: liftOver not found: $LIFTOVER_BIN" >&2; exit 1; }
  ...
  gzip -c "$work/final.tsv" > "${FINAL}.tmp.$$"
  gzip -t "${FINAL}.tmp.$$"
...
gwas_clean_make_cis(){
  ...
  if [[ "$FORCE" != "TRUE" ]] && gwas_clean_gzip_ok "$CIS_OUT"; then
...
prep_inputs_chr(){
  local chr="$1" assoc="$2" ma="$3" assoc_tmp ma_tmp
  gwas_clean_need_file "$FINAL"

  assoc_tmp="${assoc}.tmp.$$"
  ma_tmp="${ma}.tmp.$$"
  gwas_clean_zcat "$FINAL" | awk -v FS='\t' -v OFS='\t' -v chr="$chr" -v assoc="$assoc_tmp" -v ma="$ma_tmp" '
    function isnum(x){return x ~ /^[-+]?([0-9]*[.])?[0-9]+([eE][-+]?[0-9]+)?$/}
    function normchr(x){gsub(/^chr/,"",x); if(x=="X") x="23"; if(x=="Y") x="24"; if(x=="MT"||x=="M") x="25"; return x}
    NR==1{
      for(i=1;i<=NF;i++) c[$i]=i
      print "SNP","P" > assoc
      print "SNP","A1","A2","freq","b","se","p","N" > ma
      next
    }
    NR>1 && ("SNP" in c) && ("CHR" in c) && ("P" in c) && normchr($(c["CHR"]))==chr && isnum($(c["P"])) {
      print $(c["SNP"]),$(c["P"]) > assoc
    }
    NR>1 && ("SNP" in c) && ("CHR" in c) && ("EA" in c) && ("NEA" in c) && ("BETA" in c) && ("SE" in c) && ("P" in c) && ("N" in c) && normchr($(c["CHR"]))==chr {
      if(!isnum($(c["BETA"])) || !isnum($(c["SE"])) || !isnum($(c["P"])) || !isnum($(c["N"]))) next
      freq=(("EAF" in c) ? $(c["EAF"]) : "")
      if(freq=="" || !isnum(freq)) next
      print $(c["SNP"]),$(c["EA"]),$(c["NEA"]),freq,$(c["BETA"]),$(c["SE"]),$(c["P"]),$(c["N"]) > ma
    }
  '
  mv -f "$assoc_tmp" "$assoc"
  mv -f "$ma_tmp" "$ma"
}

prep_inputs_all_chr(){
  local refs="$1" suffix assoc ma
  gwas_clean_need_file "$FINAL"
  if [[ ! -s "$refs" ]]; then
    gwas_clean_log "no selected reference chromosomes; skip lead input preparation"
    return 0
  fi

  suffix=".tmp.$$"
  while IFS=$'\t' read -r _ _ _ assoc ma _; do
    [[ -n "$assoc" && -n "$ma" ]] || continue
    rm -f "${assoc}${suffix}" "${ma}${suffix}"
  done < "$refs"

  gwas_clean_zcat "$FINAL" | awk -v FS='\t' -v OFS='\t' -v refs="$refs" -v suffix="$suffix" '
    function isnum(x){return x ~ /^[-+]?([0-9]*[.])?[0-9]+([eE][-+]?[0-9]+)?$/}
    function normchr(x){gsub(/^chr/,"",x); if(x=="X") x="23"; if(x=="Y") x="24"; if(x=="MT"||x=="M") x="25"; return x}
    BEGIN{
      while((getline line < refs)>0){
        split(line,a,"\t")
        chr=a[3]
        if(chr=="") continue
        assoc[chr]=a[4] suffix
        ma[chr]=a[5] suffix
        want[chr]=1
        print "SNP","P" > assoc[chr]
        print "SNP","A1","A2","freq","b","se","p","N" > ma[chr]
      }
      close(refs)
    }
    NR==1{
    ...
    NR>1 && ("SNP" in c) && ("CHR" in c) {
      chr=normchr($(c["CHR"]))
      if(!(chr in want)) next
      if(("P" in c) && isnum($(c["P"]))){
        print $(c["SNP"]),$(c["P"]) > assoc[chr]
      }
      if(("EA" in c) && ("NEA" in c) && ("BETA" in c) && ("SE" in c) && ("P" in c) && ("N" in c)){
        if(!isnum($(c["BETA"])) || !isnum($(c["SE"])) || !isnum($(c["P"])) || !isnum($(c["N"]))) next
        freq=(("EAF" in c) ? $(c["EAF"]) : "")
        if(freq=="" || !isnum(freq)) next
        print $(c["SNP"]),$(c["EA"]),$(c["NEA"]),freq,$(c["BETA"]),$(c["SE"]),$(c["P"]),$(c["N"]) > ma[chr]
      }
    }
  '
...
ref_clump_pfiles(){
  ...
  if [[ -f "${ref}.pgen" && -f "${ref}.pvar" && -f "${ref}.psam" ]]; then
    label=$(basename "$ref")
    chr=$(ref_chr "$label") || { echo "ERROR: cannot infer chromosome from refGen_clump prefix: $ref" >&2; return 1; }
  ...
  if [[ -d "$ref" ]]; then
    while read -r f; do
      prefix=${f%.pgen}
      [[ -f "${prefix}.pvar" && -f "${prefix}.psam" ]] || continue
      label=$(basename "$prefix")
      chr=$(ref_chr "$label") || { echo "ERROR: cannot infer chromosome from refGen_clump prefix: $prefix" >&2; return 1; }
...
gwas_clean_run_core(){
  ...
  case "$DO_STEP" in
    ...
    cis) run_cis=TRUE;;
    lead) ;;
    all) run_small=TRUE; [[ "$DO_LIFTOVER" == "TRUE" ]] && run_liftover=TRUE; [[ -n "${CIS_BED:-}" ]] && run_cis=TRUE;;
  esac

  if [[ "$run_small" == "TRUE" ]]; then
    gwas_clean_need_file "$RAW"
    if [[ "$FORCE" != "TRUE" ]] && gwas_clean_gzip_ok "$SMALL"; then
