...
Usage:
  ...
  ls -1v *.png *.jpeg abc/*.png | img2pdf.v2.sh [-o OUTPUT.pdf]

  # Or pass files directly (order is whatever the shell supplies)
  img2pdf.v2.sh [-o OUTPUT.pdf] IMAGE [IMAGE ...]

Examples:
  ls -1v *.png | img2pdf.v2.sh -o figures.pdf
  ls -1v *.png *.jpeg abc/*.png | img2pdf.v2.sh -o all.pdf
  img2pdf.v2.sh -o figures.pdf Fig1.png Fig2.png abc/Fig3.png
...
TMPDIR_WORK="$(mktemp -d "${TMPDIR:-/tmp}/img2pdf.v2.XXXXXX")"
cleanup() {
    rm -rf "$TMPDIR_WORK"
}
trap cleanup EXIT
