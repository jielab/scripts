...
# Shared default adjustment set; project-specific covariates are selected by callers.
...
# 🚩 P-value thinning and display compression
...
# 🚩 Vector summaries and code mapping
...
pattern = c('^snp$|^rsid$|variant_id', '^chr$|^chrom', '^bp$|^pos$|^position|^base_pair', '^ea$|^alt$|^a1$|^allele1$|^effect_allele$', '^nea$|^ref|^allele0$|^a2$|^other_allele', '^eaf$|^freq$|a1freq|effect_allele_freq', '^n$|Neff', '^beta$|^effect$', '^se$|standard_error', '^p$|^pval$|^p_bolt_lmm$')
...
# 🚩 Genomic locus collapse
# Distance-only loci: strongest remaining SNP first; fixed maximum width.
# Already assigned windows clip new windows so all loci remain non-overlapping.
# POS is 1-based; returned BED start/end are 0-based, half-open.
loci_collapse <- function(file, collapse_size = "1Mb", CHR = "Chr", POS = "bp",
                          P = "pJ", SNP = "SNP", output = NULL) {
  size <- function(x) {
    if (is.numeric(x) && length(x)==1L) return(x)
    m <- regmatches(tolower(x), regexec("^([0-9.]+)\\s*(bp|kb|mb)?$", tolower(x)))[[1]]
    if (!length(m)) stop("collapse_size must be bp, kb or Mb")
    as.numeric(m[2]) * switch(m[3], kb=1000, mb=1000000, 1)
  }
  width <- size(collapse_size)
  if (length(width)!=1L || !is.finite(width) || width<1 || width!=floor(width)) stop("Invalid collapse_size")
  d <- if (is.data.frame(file)) file else read.table(file, header=TRUE, sep="", quote="", comment.char="", stringsAsFactors=FALSE, check.names=FALSE)
  if (!nrow(d) || !all(c(CHR,POS,P,SNP) %in% names(d))) stop("Empty input or missing CHR/POS/P/SNP columns")
  chr <- toupper(sub("^chr", "", as.character(d[[CHR]]), ignore.case=TRUE)); chr[chr=="23"]<-"X";chr[chr=="24"]<-"Y";chr[chr=="MT"]<-"M"
  pos <- suppressWarnings(as.numeric(d[[POS]])); p <- suppressWarnings(as.numeric(d[[P]])); snp<-as.character(d[[SNP]])
  bad <- !chr %in% c(as.character(1:22),"X","Y","M") | !is.finite(pos) | pos<1 | pos!=floor(pos) | !is.finite(p) | p<0 | p>1 | is.na(snp) | !nzchar(snp)
  bad <- bad | toupper(snp) %in% c("NA","NAN",".")
  for(i in which(!bad & grepl(":",snp))) {
    m<-regmatches(snp[i],regexec("^(?:chr)?([^:]+):([0-9]+):([ACGT]+):([ACGT]+)$",snp[i],ignore.case=TRUE,perl=TRUE))[[1]]
    if(!length(m)) {bad[i]<-TRUE;next}
    id_chr<-toupper(m[2]);if(id_chr=="23")id_chr<-"X"
    if(id_chr!=chr[i] || as.numeric(m[3])!=pos[i])bad[i]<-TRUE
  }
  source_rows<-seq_len(nrow(d))
  if(any(bad)) {
    for(i in which(bad))cat(sprintf("[loci_collapse] SKIP input_row=%d SNP=%s reason=invalid_record_or_coordinate_conflict\n",i,snp[i]))
    source_rows<-source_rows[!bad];d<-d[!bad,,drop=FALSE]
    chr<-chr[!bad];pos<-pos[!bad];p<-p[!bad];snp<-snp[!bad]
  }
  if(!nrow(d))stop("No valid SNP records remain")
  log_p<-log(p)
  # COJO rounds extremely small joint P values to zero. Recover their ordering
  # from the joint Wald statistic, without changing the reported P column.
  if(P=="pJ" && all(c("bJ","bJ_se") %in% names(d))) {
    beta<-suppressWarnings(as.numeric(d$bJ));se<-suppressWarnings(as.numeric(d$bJ_se))
    use<-p==0 & is.finite(beta) & is.finite(se) & se>0
    log_p[use]<-log(2)+stats::pnorm(-abs(beta[use]/se[use]),log.p=TRUE)
  }
  z<-data.frame(row=seq_len(nrow(d)),chr=chr,pos=pos,p=p,log_p=log_p,snp=snp,start=pmax(0,pos-1-floor(width/2)),end=pos-1-floor(width/2)+width)
  z<-z[order(match(z$chr,c(as.character(1:22),"X","Y","M")),z$pos,z$p,z$snp,z$row),]
  group<-integer(nrow(z));g<-0L;windows<-list()
  for(i in order(z$log_p,z$pos,z$snp,z$row)) {
    if(group[i]!=0L) next
    lo<-z$start[i];hi<-z$end[i]
    for(w in windows) if(w$chr==z$chr[i]) {
      if(w$end<z$pos[i]) lo<-max(lo,w$end)
      if(w$start>=z$pos[i]) hi<-min(hi,w$start)
    }
    take<-which(group==0L & z$chr==z$chr[i] & z$pos>lo & z$pos<=hi)
    g<-g+1L;group[take]<-g
    windows[[g]]<-data.frame(chr=z$chr[i],start=lo,end=hi,lead_snp=z$snp[i],
      lead_pos=z$pos[i],lead_p=z$p[i],lead_log_p=z$log_p[i],n_snps=length(take),old_group=g)
  }
  loci<-do.call(rbind,windows)
  loci<-loci[order(match(loci$chr,c(as.character(1:22),"X","Y","M")),loci$start),]
  loci$locus_id<-sprintf("cojo_%04d",seq_len(nrow(loci)))
  z$locus_id<-loci$locus_id[match(group,loci$old_group)]
  loci$old_group<-NULL
  rownames(loci)<-NULL
  members<-cbind(data.frame(input_row=source_rows[z$row],locus_id=z$locus_id),d[z$row,,drop=FALSE]);rownames(members)<-NULL
  if(!is.null(output)) {
    dir.create(output,recursive=TRUE,showWarnings=FALSE)
    write.table(loci,file.path(output,"collapsed.tsv"),sep="\t",quote=FALSE,row.names=FALSE)
    write.table(loci[,c("chr","start","end","lead_snp")],file.path(output,"collapsed.bed"),sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)
    write.table(members,file.path(output,"members.tsv"),sep="\t",quote=FALSE,row.names=FALSE)
  }
  list(loci=loci,members=members,collapse_size=width,p_column=P)
}
