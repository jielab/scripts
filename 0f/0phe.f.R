...
save_plot <- function(p, file, width = 8, height = 6, w = width, h = height,
                      ...
                      limitsize = FALSE, ...) {
	ggplot2::ggsave(filename = file, plot = p, width = w, height = h,
...
edit_dat <- function(x, year1, disease1, ...){
	i <- which(x$year == year1 & x$disease == disease1)
	if (!length(i)) return(x)
	dots <- list(...)
	for (nm in names(dots)) x[[nm]][i] <- dots[[nm]]
	x
}
...
edit_dat <- function(dat1, yr, dx, lo=NULL, hi=NULL){
	i <- dat1$year==yr & dat1$disease==dx
	if(!is.null(lo)) dat1$lo[i] <- lo
	if(!is.null(hi)) dat1$hi[i] <- hi
	dat1
}
...
t2e <- function(dat, domain, Y_date, birth_date, date_attend, date_lost, date_death,
	...
	if (!is.na(prefix) && paste0(prefix, ".Yt2e") %in% names(dat)) return(dat)
	...
	dat <- dat %>% mutate(
	  ...
	  f_dt = case_when(is_aft ~ Yd, !is.na(.data[[date_lost]]) ~ .data[[date_lost]], !is.na(.data[[date_death]]) ~ .data[[date_death]], TRUE ~ date_end0),
	...
	) %>% { if (time_unit == "year") mutate(., t2e = t2e/365.25, r2e = r2e/365.25, b2e = b2e/365.25) else . }
	if (!is.na(prefix)) dat <- dat %>% rename_with(~ paste0(prefix, ".", .), c(Yt2e, Yr2e, t2e, r2e, b2e))
	dat %>% dplyr::select(-Yd, -is_aft, -is_bef, -is_nev, -d_happ, -is_dty, -f_dt, -r_dt)
}
...
get_icd <- function(ver, dat.code = NA, dat.date = NA, dat.this, code.this, label.this,
	...
	# Remove stale old versions before merge, otherwise base::merge may make .x/.y columns.
...
# Fast all-trait ICD helper.
# This is used by phe.R's icd step to avoid calling get_icd() once per trait
# over the full N x slot matrix. It creates a sparse long event table once per
# source, matches trait definitions against unique codes, and then writes wide
# fod/lod/cnt/aodtxt columns. By default, only aodtxt_* is saved; list-column
# aod_* can be requested but makes all.rds much larger.
