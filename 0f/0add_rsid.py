...
def write_output(
    ...
    with open_any(in_path, 'rt') as fin, open_any(out_path, 'wt') as fout:
        ...
        for line in fin:
            ...
            # If CHR/POS are needed and user-named columns do not exist, try SNP->POS.
            ...
            # If SNP is needed and no user-named SNP column exists, try POS->SNP.
            if need_add_snp and out_snp_name not in header:
...
def main():
    ...
    # Derive coordinates from SNP or SNP from exact CHR/POS columns.
    rsids: Set[str] = set()
    pos_keys: Set[Tuple[str,int]] = set()
