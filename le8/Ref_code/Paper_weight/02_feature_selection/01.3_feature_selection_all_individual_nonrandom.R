####################################################################
####      Individual domain non-random feature selection        ####
####               Kamil Demircan October 1 2024                ####
####################################################################
#!/usr/bin/env Rscript
...
for (group_name in names(category_groups)) {
  ...
  fwrite(feat.sel, paste0("output/overweight.feature.selection.nonrandom.", group_name, ".", outc, ".txt"), 
  ...
  top.sel <- feat.sel[1:nrow(var.sel)]
  
  cat("1: Performing opt & test with top selected features\n")
  feature_top_name <- paste0("feature.nonrandom.", group_name)
  ...
  save(list=feature_top_name, file=paste0("output/overweight.model.feature.nonrandom.", group_name, ".", outc, ".RData"))
}
