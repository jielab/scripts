####################################################################
####    Stepwise feature selection for obesity complications    ####
####################################################################
#!/usr/bin/env Rscript
...
## define stepwise order with groups
## adaption of this enables data-domain specific outcome models
category_groups <- list(
  "Basic parameters" = c("Basic parameters", "Random") ,
  "+Diseases & Drugs" = c("Basic parameters", "Diseases and drug intake", "Random"),
  "+Biomarker" = c("Basic parameters", "Diseases and drug intake", "Blood markers", "Random"),
  "+Cardiopulmonary" = c("Basic parameters", "Diseases and drug intake", "Blood markers", "Cardiopulmonary parameters", "Random"),
  "+Body composition" = c("Basic parameters", "Diseases and drug intake", "Blood markers", "Cardiopulmonary parameters", "Body composition", "Random"),
  "+NMR" = c("Basic parameters", "Diseases and drug intake", "Blood markers", "Cardiopulmonary parameters","Body composition", "NMR","Random"),
  "+PGS" = c("Basic parameters", "Diseases and drug intake", "Blood markers", "Cardiopulmonary parameters","Body composition", "NMR","PGS","Random")
)
...
for (group_name in names(category_groups)) {
  ...
  print(paste(time_taken[3]/60, "minutes"))
  ...
  ## extract case numbers from each outcome
  case.num <- data.frame(n.total=nrow(ukb.phe), n.cases=sum(ukb.phe[, outc]),
                         n.train=length(t.idx), train.cases=sum(ukb.phe[ukb.phe$f.eid %in% t.idx, outc]),
                         n.opt=length(o.idx), opt.cases=sum(ukb.phe[ukb.phe$f.eid %in% o.idx, outc]),
                         n.val=length(v.idx), val.cases=sum(ukb.phe[ukb.phe$f.eid %in% v.idx, outc]))
  
  ## save case numbers from each outcome
  fwrite(case.num, paste0("<path>", outc, ".txt"), sep="\t", row.names=FALSE, na=NA)
}