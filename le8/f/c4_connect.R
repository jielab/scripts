...
suppressPackageStartupMessages({
  ...
  source(file.path(fdir, "c4_pathway_figure.R"))
  source(file.path(fdir, "c4_imaging.R"))
...
plot_supervised_atlas <- function(modules,sets,med,layer,outdir){
  ...
  top<-mem|>arrange(desc(YS_core),desc(module_stability),FDR_disc)|>slice_head(n=40)|>pull(feature)
