...
le8_plot_c1_temporal <- function(paired, heterogeneity, status, anchors, layer) {
  ...
  }else p1<-blank_plot("a. Paired measured and PGS associations",if(!length(anchors))"No configured anchor is present in this omics layer" else status$status[1])
  p2<-if(any(is.finite(heterogeneity$p_time_interaction)))heterogeneity|>ggplot(aes(-log10(pmax(p_time_interaction,1e-300)),feature))+
    geom_point()+labs(title="b. Does the baseline association vary across follow-up intervals?",x="-log10(time-interaction P)",y=NULL)+theme_5c(9) else
    blank_plot("b. Association across follow-up intervals",if(!length(anchors))"No configured anchor is present in this omics layer" else "No estimable time-interaction test; consult temporal_heterogeneity.csv")
