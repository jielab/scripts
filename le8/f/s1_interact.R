...
run_sex_layer<-function(layer=c("protein","metabolite")){
 ...
 if(cache_valid(sex_cache)){
   cache_message(paste0("S1/sex/",layer),sex_cache)
   return(data.table::fread(sex_cache,data.table=FALSE)|>as_tibble())
 }
...
run_le8_interactions <- function() {
  ...
  if(cache_valid(interaction_cache)){
    cache_message("S1/LE8 interactions",interaction_cache)
    return(readRDS(interaction_cache))
  }
