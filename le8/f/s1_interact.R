...
run_sex_layer<-function(layer=c("protein","metabolite")){
 ...
 sex_cache<-file.path(rawdir,"s1.sex_interaction.csv")
 if(LE8_REUSE_RESULTS || cache_valid(sex_cache)){
   res<-le8_figure_csv(rawdir,"s1.sex_interaction.csv")
   plot_s1_sex(res,layer,outdir);finalize_outputs(LE8_JOB,outdir);return(invisible(res))
 }
 ...
 sex_levels<-levels(d$sex_f);sex_name<-function(x)if(identical(sex_levels,c("0","1")))c(`0`="Female",`1`="Male")[[as.character(x)]] else paste("Sex category",x)
 ...
 res<-res|>mutate(FDR_interaction=p.adjust(p_interaction,"BH"),interaction_contrast=if(length(sex_levels)>=2)paste0(sex_name(sex_levels[2])," minus ",sex_name(sex_levels[1]))else NA_character_)|>arrange(p_interaction)
 write_raw_csv(res,"s1.sex_interaction.csv",rawdir)
 plot_s1_sex(res,layer,outdir);finalize_outputs(LE8_JOB,outdir);res
}
run_le8_interactions <- function() {
  ...
  interaction_cache <- file.path(rawdir,"s1.interactions.res.rds")
  if(LE8_REUSE_RESULTS || cache_valid(interaction_cache)){
    obj<-le8_figure_result(interaction_cache,c("results","surfaces"))
    plot_s1_interactions(obj$results,obj$surfaces,outdir);finalize_outputs(LE8_JOB,outdir);return(invisible(obj))
  }
  ...
  plot_s1_interactions(res,grid,outdir)
  saveRDS(list(results=res,surfaces=grid),interaction_cache,compress="xz");finalize_outputs(LE8_JOB,outdir)
}

plot_s1_sex <- function(res,layer,outdir) {
 sex_levels<-sub("^beta_sex","",grep("^beta_sex",names(res),value=TRUE))
 sex_name<-function(x)if(identical(sex_levels,c("0","1")))c(`0`="Female",`1`="Male")[[as.character(x)]] else paste("Sex category",x)
 bcols<-grep("^beta_sex",names(res),value=TRUE);if(length(bcols)>=2){x<-bcols[1];y<-bcols[2];res$label<-ifelse(min_rank(res$p_interaction)<=20,res$feature,NA_character_)
  xl<-sex_name(sub("beta_sex","",x));yl<-sex_name(sub("beta_sex","",y))
  pA<-ggplot(res,aes(.data[[x]],.data[[y]]))+geom_abline(slope=1,intercept=0,linetype=2,color="grey50")+geom_hline(yintercept=0,color="grey75")+geom_vline(xintercept=0,color="grey75")+geom_point(aes(color=FDR_interaction<.05),size=2,alpha=.8)+ggrepel::geom_text_repel(aes(label=label),size=2.7,seed=20,max.overlaps=25,na.rm=TRUE)+scale_color_manual(values=c(`TRUE`="#D95F02",`FALSE`="grey70"),guide="none")+labs(title=paste0("A. Sex-specific ",layer," associations"),subtitle="Orange: interaction FDR < 0.05",x=paste0("Log-HR per SD in ",xl),y=paste0("Log-HR per SD in ",yl))+theme_5c(11)
  ...
  contrast_subtitle<-unique(na.omit(res$interaction_contrast));contrast_subtitle<-if(length(contrast_subtitle))contrast_subtitle[[1]]else"Second category minus reference"
  pB<-ggplot(top,aes(beta_interaction,feature))+geom_vline(xintercept=0,color="grey60")+geom_errorbarh(aes(xmin=lo,xmax=hi),height=.12)+geom_point(aes(color=FDR_interaction<.05),size=2)+scale_color_manual(values=c(`TRUE`="#D95F02",`FALSE`="#4C78A8"),guide="none")+labs(title="B. Omic-by-sex interaction",subtitle=contrast_subtitle,x="Interaction log-hazard coefficient",y=NULL)+forest_theme(9)
 ...
 write_xlsx2(list(sex_interaction=res),"s1_int.Fig01.sex_interaction.out.xlsx");invisible(res)
}

plot_s1_interactions <- function(res,grid,outdir) {
