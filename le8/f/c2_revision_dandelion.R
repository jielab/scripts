...
.le8_dandelion_impl <- function(layer,assoc,ann,base,ygfile,rawdir,outdir,
  ...
  if(mode=="None"||layer!="protein")return(le8_result_update(empty,list(status=if(layer!="protein")"protein-only adaptation"else"RUN_Dandelion=None")))
  input<-find_dandelion_gene_file(ygfile)
  if(is.na(input$path))return(le8_result_update(empty,list(status="gene-level disease input unavailable",analysis_class=input$analysis_class)))
  ...
  if(nrow(ref)<5)return(le8_result_update(empty,list(status="fewer than five matched annotated targets",gene_level_file=input$path)))
  ...
  if(nrow(lead)<2)return(le8_result_update(empty,list(status="fewer than two eligible disease loci",gene_level_file=input$path)))
  ...
  # Truncating the target family affects the empirical DACT null mixture.
  # BY correction cannot repair selected inputs or dependent component P values.
  ...
  primary<-input$primary_eligible&&mode=="All"&&uncapped&&qtl_unfiltered&&ld_verified&&package_status=="ok"
  ...
  audit<-bind_rows(audit,tibble(metric=c("package_version","target_exposure_uncapped","QTL_unfiltered",
    "sample_overlap","native_selection","extension_selection"),value=c(
      if(requireNamespace("DANDELION",quietly=TRUE))as.character(utils::packageVersion("DANDELION"))else"unavailable",
      as.character(uncapped),as.character(qtl_unfiltered),Sys.getenv("C2_DANDELION_SAMPLE_OVERLAP",unset="unknown"),
      "package_selected: native per-exposure q-value rule",
      "selected: project-specific target aggregation and BH/BY; not the native DANDELION target rule")))
  ans<-le8_result_update(empty,list(status=if(package_status=="ok")"ok"else"DACT unavailable; see MaxP sensitivity",mode=mode,
...
run_dandelion_step <- function(layer,assoc,ann,base,ygfile,rawdir,outdir,
  ...
  tryCatch({
    ...
    if(file.exists(failure))unlink(failure)
    ans
  }, error=function(e) {
