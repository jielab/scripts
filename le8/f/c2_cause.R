...
suppressPackageStartupMessages({
  ...
  source(file.path(.c2_fdir,"c2_comparison_figures.R"))
  source(file.path(.c2_fdir,"c2_legacy_figures.R"))
  source(file.path(.c2_fdir,"c2_observational_comparison.R"))
})
...
find_disease_leads <- function(ygfile){
  ...
  if(length(cand)){
    for(f in cand){z<-read_lead_file(f,ygfile);if(nrow(z)>=2){z<-z|>arrange(P);if(DANDELION_MAX_SNPS>0)z<-head(z,DANDELION_MAX_SNPS);attr(z,"source")<-f;return(z)}}
  }
...
run_dandelion_step <- function(layer,assoc,ann,base,ygfile,rawdir,outdir,
  ...
  if(mode=="None")return(le8_result_update(empty,list(status="mode_none",analysis_class="not run by RUN_Dandelion=None")))
  if(layer!="protein")return(le8_result_update(empty,list(status="protein_only")))
  if(!requireNamespace("DANDELION",quietly=TRUE)){
    warning("DANDELION R package is not installed.",call.=FALSE)
    return(le8_result_update(empty,list(status="package_missing")))
  }
  ...
  if(is.na(gene_file)){
    ...
    warning("DANDELION: ",msg,call.=FALSE);return(le8_result_update(empty,list(status=msg,
  ...
  if(length(common)<20){
    ...
    warning("DANDELION: ",msg,call.=FALSE);return(le8_result_update(empty,list(status=msg,gene_level_file=gene_file,gene_evidence_type=gene_evidence_type)))
  }
  ...
  if(nrow(lead)<2){msg<-"Fewer than two independent disease-associated lead SNPs available.";warning("DANDELION: ",msg,call.=FALSE);return(le8_result_update(empty,list(status=msg,gene_level_file=gene_file,gene_evidence_type=gene_evidence_type,lead_snps=lead)))}
  gene2<-sort(common)
  ...
  if(length(gene2)<min_gene2){
    ...
    return(le8_result_update(empty,list(status=msg,gene_level_file=gene_file,gene_evidence_type=gene_evidence_type)))
  }
  ...
  if(nrow(ptrans)<min_gene2){msg<-"Too few selected proteins had trans-pQTL data at the disease SNPs.";warning("DANDELION: ",msg,call.=FALSE);return(le8_result_update(empty,list(status=msg,gene_level_file=gene_file,gene_evidence_type=gene_evidence_type,lead_snps=lead,qtl_audit=qtl_audit)))}
  ...
  if(inherits(med,"condition")){msg<-conditionMessage(med);warning("DANDELION failed: ",msg,call.=FALSE);return(le8_result_update(empty,list(status=msg,gene_level_file=gene_file,gene_evidence_type=gene_evidence_type,lead_snps=lead,snp_gene_map=uniq_snp,qtl_audit=qtl_audit)))}
...
run_c2_layer <- function(layer=c("protein","metabolite")){
  ...
  if(cache_valid(cache)){
    ...
    if(is.list(old)&&all(c("meta","MR","MR_reverse","MR_best")%in%names(old)) &&
       !grepl("^failed", old$DANDELION$status %||% "")){
