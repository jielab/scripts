...
plot_c1_fig12 <- function(layer, features_all, pgs_incident, pgs_prevalent,
  ...
  if (layer == "protein") {
    ...
    p_circle<-wrap_plots(list(p1a,p1c,p1b,p1d,p1e,p1f),ncol=2,guides='collect')+
      plot_annotation(caption=paste0('Category-wise radial associations. Each panel selects BH FDR < 0.05 from all metabolites tested in that analysis. ',
    ...
    circle_table<-map2_dfr(circ_data,circle_analyses,
...
run_c1_layer <- function(layer=c("protein","metabolite")) {
  ...
  if(!LE8_REPLACE&&!file.exists(scan_cache)){
    ...
    for(p in legacy){
      ...
      if(is.list(z)&&all(c("association_adj2","prevalent_adj2")%in%names(z))){scan_cache<-p;break}
    }
  }
