...
dir.Y <- Sys.getenv("LE8_GWAS_DIR", unset = file.path("/mnt/i", "gwas/main"))
dir.X <- Sys.getenv("LE8_PQTL_IV_DIR", unset = file.path("/mnt/i", "gwas/prot"))
dir.met.gwas <- Sys.getenv("LE8_MQTL_IV_DIR", unset = file.path("/mnt/i", "gwas/met"))
...
write_stage_cache <- function(data, path) {
  ...
  saveRDS(list(generated=format(Sys.time(),"%F %T %z"),data=data),tmp,compress="xz")
...
module_meta <- function(layer, module = LE8_JOB, extra = list()) {
  c(list(module = module, layer = layer, trait = Y, generated = format(Sys.time(), "%F %T %z"),
         seed = SEED, R_runtime = R.version.string, biom = BIOM, analysis_options=le8_analysis_options()), extra)
}
finalize_outputs <- function(module, outdir = getwd()) {
  ...
  index <- data.frame(module = module, trait = Y, directory = normalizePath(rawdir, winslash = "/", mustWork = FALSE),
...
blank_plot <- function(title, subtitle = "Required data were not available") {
  p<-ggplot() + theme_void(base_size = 13) +
...
# Category-wise metabolite radial plotting is defined in c1_met_circle.R below.
...
sys.source(file.path(.le8_patch_fdir,"le8_figures.R"), envir = .GlobalEnv)
sys.source(file.path(.le8_patch_fdir,"le8_budget_config.R"), envir = .GlobalEnv)
sys.source(file.path(.le8_patch_fdir,"figure_policy.R"), envir = .GlobalEnv)
