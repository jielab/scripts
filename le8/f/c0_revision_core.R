...
le8_revision_begin <- function(layer,module) {
  ...
  .le8_revision_state$deps<-character()
  # Output presence and integrity, not input/code fingerprints, control reuse.
  # Keep provenance for auditing only; never move completed or partial steps.
  ...
  .le8_revision_state$inputs<-if(is.list(old))old$dependencies else character()
...
le8_revision_end <- function() {
  ...
  base::saveRDS(list(cache_policy="complete-output",dependencies=inputs,
    version=LE8_REVISION),.le8_revision_state$manifest)
  invisible(NULL)
}

le8_load_revision <- function(layer,module) {
  le8_guard_stage_options(layer,module)
