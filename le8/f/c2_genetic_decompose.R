...
read_c2_heritability <- function(layer){
  ...
  if(is.na(fcol)||is.na(hcol))return(le8_result_update(empty,list(status=tibble(status="invalid",file=f,
...
run_individual_genetic_decomposition <- function(layer,outdir,candidates=tibble(),
  ...
  if(!length(score_cols))return(le8_result_update(empty,list(status=tibble(status="not run",
