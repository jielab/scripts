...
# LE8 input adapter. Keeping this beside the MR-link-2 implementation gives C2
# one Python entry point while preserving the upstream analysis CLI unchanged.
def _le8_open(path, mode="rt"):
...
def _le8_chrom(value):
    chrom = re.sub(r"^chr", "", str(value), flags=re.IGNORECASE).upper()
    chrom = re.sub(r"\.0$", "", chrom)
    chrom = {"X": "23", "Y": "24", "M": "25", "MT": "25"}.get(chrom, chrom)
    return chrom.lstrip("0") or "0"


def _le8_pick(header, candidates):
    normalized = {_le8_norm(name): name for name in header}
    for candidate in candidates:
        if _le8_norm(candidate) in normalized:
            return normalized[_le8_norm(candidate)]
    return None


def _le8_number(value, default=None):
    try:
...
def _le8_region(value):
    if not value:
        return None
    match = re.match(r"^(?:chr)?([^:]+):(\d+)-(\d+)$", value)
    if not match:
        raise SystemExit("Region must be formatted as chr:start-end, got: " + value)
    return match.group(1), int(match.group(2)), int(match.group(3))


def _le8_tabix_text(input_path, region):
    if region is None:
        return None
    tabix = shutil.which("tabix")
    if not tabix:
        return None
    indexes = [input_path + ".tbi", input_path + ".csi"]
    fresh = [x for x in indexes if os.path.isfile(x) and os.path.getsize(x) > 0 and
             os.path.getmtime(x) >= os.path.getmtime(input_path)]
    if not fresh:
        return None
    try:
        contigs = subprocess.check_output([tabix, "-l", input_path], text=True).splitlines()
        wanted = _le8_chrom(region[0])
        source_chrom = next((x for x in contigs if _le8_chrom(x) == wanted), None)
        if source_chrom is None:
            return ""
        query = f"{source_chrom}:{region[1]}-{region[2]}"
        result = subprocess.run([tabix, input_path, query], capture_output=True, text=True)
        return result.stdout if result.returncode == 0 else None
    except (OSError, subprocess.SubprocessError):
        return None


def le8_prepare_sumstats(input_path, output_path, default_n, region=None):
    region = _le8_region(region)
    with _le8_open(input_path) as header_source:
        sample = header_source.readline()
    if not sample:
        raise SystemExit("Empty input: " + input_path)
    delimiter = "\t" if sample.count("\t") >= sample.count(",") else ","
    indexed_text = _le8_tabix_text(input_path, region)
    if indexed_text is None:
        source = _le8_open(input_path)
    else:
        source = io.StringIO(sample + indexed_text)
    with source:
        reader = csv.DictReader(source, delimiter=delimiter)
        header = reader.fieldnames or []
        columns = {
            "snp": _le8_pick(header, ["pos_name", "SNP", "RSID", "RS_NUMBER", "VARIANT_ID", "ID", "MARKERNAME"]),
            "chr": _le8_pick(header, ["chromosome", "CHR", "CHROM", "CHROMOSOME"]),
            "pos": _le8_pick(header, ["position", "POS", "BP", "BASE_PAIR_LOCATION", "BASEPAIR"]),
            "ea": _le8_pick(header, ["effect_allele", "EA", "A1", "ALT", "ALLELE1", "EFFECTALLELE"]),
            "nea": _le8_pick(header, ["reference_allele", "NEA", "A2", "REF", "ALLELE0", "OTHERALLELE", "NONEFFECTALLELE"]),
            "beta": _le8_pick(header, ["beta", "BETA", "B", "EFFECT", "LOG_ODDS", "LOGOR", "bJ", "BJ"]),
            "se": _le8_pick(header, ["se", "SE", "SEBETA", "STDERR", "bJ_se", "BJ_SE"]),
            "p": _le8_pick(header, ["pval", "P", "PVALUE", "P_VALUE", "pJ", "PJ"]),
            "n": _le8_pick(header, ["n_iids", "N", "SAMPLESIZE", "N_TOTAL", "NEFF"]),
        }
        if any(columns[name] is None for name in ("chr", "pos", "ea", "nea", "beta", "se")):
            raise SystemExit("Missing required columns; need CHR POS EA NEA BETA SE in " + input_path)
        os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
        with gzip.open(output_path, "wt", newline="") as target:
            writer = csv.writer(target, delimiter="\t")
            # Use the shared LE8/GWAS column contract here so match_GRCH() can
            # map rsIDs to the build-specific PLINK reference before MR-link-2.
            writer.writerow(["SNP", "CHR", "POS", "EA", "NEA", "BETA", "SE", "Z", "P", "N"])
            written = 0
            for row in reader:
                chromosome = _le8_chrom(row.get(columns["chr"], ""))
                position = _le8_number(row.get(columns["pos"]))
                if region and (chromosome != _le8_chrom(region[0]) or position is None or not region[1] <= position <= region[2]):
                    continue
                effect = str(row.get(columns["ea"], "")).upper()
                other = str(row.get(columns["nea"], "")).upper()
                beta = _le8_number(row.get(columns["beta"]))
                se = _le8_number(row.get(columns["se"]))
                if position is None or beta is None or se is None or se <= 0 or not effect or not other:
                    continue
                snp = str(row.get(columns["snp"], "")).strip() if columns["snp"] else ""
                if not snp or snp == ".":
                    snp = f"chr{chromosome}_{int(position)}_{other}_{effect}"
                z_score = beta / se
                p_value = _le8_number(row.get(columns["p"])) if columns["p"] else None
                if p_value is None or not 0 < p_value <= 1:
                    p_value = max(math.erfc(abs(z_score) / math.sqrt(2.0)), 1e-300)
                sample_n = _le8_number(row.get(columns["n"]), default_n) if columns["n"] else default_n
                sample_n = int(sample_n) if abs(sample_n - round(sample_n)) < 1e-6 else sample_n
                writer.writerow([snp, chromosome, int(position), effect, other, beta, se, z_score, p_value, sample_n])
                written += 1
    if written == 0:
        raise SystemExit("No usable rows written from " + input_path)


def _run_le8_prepare_cli(argv):
    if len(argv) not in (4, 5):
        raise SystemExit("Usage: c2_mr_link2.py prepare <input> <output.tsv.gz> <default_n> [region]")
    le8_prepare_sumstats(argv[1], argv[2], float(argv[3]), argv[4] if len(argv) == 5 else None)
...
def mr_link2_loglik_reference_v0(th: np.ndarray, lam: np.ndarray,
    ...
    # Treat overflowed second terms as zero.
    Dyy = (1. / (n_y * lam + tY))
...
def warn_if_only_gws_snps(exp_pvals: np.ndarray, region,
                          gws_threshold: float = 5e-8,
                          fraction_threshold: float = 1.0) -> None:
    """Warn when an input looks like a selected-IV file rather than dense cis data."""
    fraction_gws = np.mean(exp_pvals < gws_threshold)
    if fraction_gws >= fraction_threshold:
        warnings.warn(
            f'Region {region}: {fraction_gws:.0%} of exposure SNPs are genome-wide significant '
            f'(p < {gws_threshold:.0e}). MR-link-2 requires all common SNPs in the cis region, '
            f'not only selected instruments. Use the cleaned *.cis.gz marginal summary statistics.',
            UserWarning,
            stacklevel=3,
        )
...
def mr_link2_on_region(region: StartEndRegion,
    ...
    def align_beta_to_reference(beta, allele1, allele2, ref1, ref2):
        allele1, allele2 = str(allele1).upper(), str(allele2).upper()
        ref1, ref2 = str(ref1).upper(), str(ref2).upper()
        if (allele1, allele2) == (ref1, ref2):
            return beta
        if (allele1, allele2) == (ref2, ref1):
            return -beta
        comp1, comp2 = allele1.translate(complement), allele2.translate(complement)
        if (comp1, comp2) == (ref1, ref2):
            return beta
        if (comp1, comp2) == (ref2, ref1):
            return -beta
        return None

    for rsid in ordered_snps:
        ...
        exp_beta = align_beta_to_reference(exp_beta, exp_a1, exp_a2, ref_a1, ref_a2)
        out_beta = align_beta_to_reference(out_beta, out_a1, out_a2, ref_a1, ref_a2)
        if exp_beta is not None and out_beta is not None:
            snp_to_beta_dict[rsid] = [exp_beta, out_beta]

    # Drop incompatible variants while retaining the harmonizable locus.
    harmonized_positions = [i for i, rsid in enumerate(ordered_snps) if rsid in snp_to_beta_dict]
    if not harmonized_positions:
        print('Could not make an MR estimate because no shared SNP alleles could be harmonized')
        return None
    removed_for_alleles = len(ordered_snps) - len(harmonized_positions)
    if removed_for_alleles:
        print(f'Removed {removed_for_alleles} SNPs with incompatible alleles; '
              f'{len(harmonized_positions)} harmonized SNPs remain')
    correlation_mat = correlation_mat[np.ix_(harmonized_positions, harmonized_positions)]
    ordered_snps = [ordered_snps[i] for i in harmonized_positions]
    ...
    warn_if_only_gws_snps(exp_pvals, region)
...
def read_regions_from_file(filename, region_or_regions):
    ...
    original_input_to_gwas_catalog_dict = {
        'pos_name': 'rsid', 'position': 'base_pair_location', 'reference_allele': 'other_allele',
        'se': 'standard_error', 'pval': 'p_value', 'n_iids': 'n',
        'SNP': 'rsid', 'CHR': 'chromosome', 'POS': 'base_pair_location',
        'EA': 'effect_allele', 'NEA': 'other_allele', 'BETA': 'beta',
        'SE': 'standard_error', 'P': 'p_value', 'N': 'n'
    }
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'prepare':
        _run_le8_prepare_cli(sys.argv[1:])
        raise SystemExit(0)
...
""",
    ...
    with tempfile.TemporaryDirectory() as tmp_prepend:
        ...
        # PLINK 1.9 aborts clumping when the summary-statistics file contains
        # duplicate variant IDs (exit status 3). QTL inputs can legitimately
        # contain repeated rsIDs, so retain the strongest association per rsID.
        duplicate_rsids = int(exposure_df['rsid'].duplicated(keep=False).sum())
        if duplicate_rsids:
            exposure_df = (exposure_df
                           .sort_values('p_value', na_position='last')
                           .drop_duplicates(subset='rsid', keep='first'))
            print(f'Removed {duplicate_rsids} rows with duplicate rsIDs before region selection; '
                  f'{exposure_df.shape[0]} unique variants remain')
