...
def le8_prepare_sumstats(input_path, output_path, default_n, region=None):
    ...
    with _le8_open(input_path) as source:
        ...
        with gzip.open(output_path, "wt", newline="") as target:
            ...
            writer.writerow(["pos_name", "chromosome", "position", "effect_allele", "reference_allele", "beta", "se", "z", "pval", "n_iids"])
            written = 0
            for row in reader:
                chromosome = str(row.get(columns["chr"], "")).replace("chr", "").replace("CHR", "")
                position = _le8_number(row.get(columns["pos"]))
                if region and (chromosome != str(region[0]) or position is None or not region[1] <= position <= region[2]):
                    continue
