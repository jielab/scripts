...
load_oof_score <- function(layer) {
  ...
  z <- readRDS(f); pr <- z$CV$pred %||% tibble(); best <- z$meta$best_model %||% "PRS_LE8_YSP"
  if (!all(c("model", "lp", "row_id") %in% names(pr))) return(empty_score())
  sc <- pr |> filter(model == best, is.finite(lp)) |> select(row_id, score = lp)
  if (!nrow(sc)) return(empty_score())
  all0 <- read_all(); prs <- find_prs_vars(all0, Y, 4)
  n0 <- unique(c("eid", "ethnic.c", vars.basic, vars.le8, prs, "birth_date", "date_attend", "date_lost", "date_death", paste0("fod_icd10_", Y)))
  d0 <- all0[, intersect(n0, names(all0)), drop = FALSE] |> filter_analysis_cohort() |> make_outcome(Y)
  core <- unique(c(tvar, evar, intersect(vars.basic, names(d0)), intersect(vars.le8, names(d0)), intersect(prs, names(d0))))
  d0 <- d0[complete.cases(d0[, core, drop = FALSE]) & d0[[tvar]] > 0, , drop = FALSE]
  sc <- sc |> filter(row_id >= 1, row_id <= nrow(d0)) |> mutate(eid = d0$eid[row_id]) |> select(eid, score)
...
p <- if (!nrow(curves)) blank_plot("Non-linear LE8 and omics-score risk chart", "No variable met the sample/event requirements") else
  ...
  labs(title = "Non-linear LE8 and omics-score risk chart", subtitle = "Natural-spline hazard ratios with contrast-valid confidence intervals; reference is the median", x = NULL, y = "Hazard ratio (log scale)") + theme_5c(10)
save_plot(p, "s2_nonlin.Fig01.spline_patterns.png", 12, 9, outdir = outdir)
write_xlsx2(list(nonlin_tests = res, spline_curves = curves), "s2_nonlin.Fig01.spline_patterns.out.xlsx")
saveRDS(list(tests=res,curves=curves),file.path(rawdir,"s2_nonlin.res.rds"),compress="xz")
...
saveRDS(list(performance=perf,selected=choose,inner=inner_perf,summary=summ,risk_HR=hr),file.path(rawdir,"s2.penalty.res.rds"),compress="xz");finalize_outputs(LE8_JOB,outdir)
