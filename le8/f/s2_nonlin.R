...
plot_s2_nonlin <- function(res,curves,nadir,outdir) {
...
plot_s2_penalty <- function(perf,choose,inner_perf,summ,hr,full,best_gamma,outdir) {
# Legacy caches included adjustment coefficients with an NA quintile label.
# Display only the four intended risk-group contrasts; retain the source cache.
if(nrow(hr))hr<-hr|>filter(str_detect(term,"^risk_groupRisk Q[2-5]$"),is.finite(HR),is.finite(lo),is.finite(hi),lo>0)|>
...
pB<-if(!nrow(choose))blank_plot("B. Penalty selected within each outer fold")else
  ggplot(choose,aes(chosen_gamma,factor(fold)))+geom_vline(xintercept=0,linetype=2,color="grey65")+
  geom_point(size=3,color="#D95F02")+
  scale_x_continuous(limits=range(c(0,inner_perf$gamma,choose$chosen_gamma),na.rm=TRUE))+
  labs(title=paste0("B. Inner-CV penalty choices; modal gamma = ",best_gamma),
    subtitle="Gamma = 0 retains the arithmetic mean without an additional penalty",x="Selected gamma",y="Outer validation fold")+theme_5c(10)
...
if(LE8_REUSE_RESULTS || (cache_valid(nonlin_cache) && cache_valid(penalty_cache))){
  ...
  best_gamma<-if(nrow(penalty$selected))as.numeric(names(sort(table(penalty$selected$chosen_gamma),decreasing=TRUE))[1])else 0
  full<-penalty$plot_scores
  # Selection decisions are already cached. A loess over a few repeated integer
  # failure counts was numerically unstable and did not evaluate the penalty.
  # Plot the actual nested-CV choices; no cohort reload is needed for this view.
  plot_s2_nonlin(nonlin$tests,nonlin$curves,nonlin$nadir,outdir)
  plot_s2_penalty(penalty$performance,penalty$selected,penalty$inner,penalty$summary,penalty$risk_HR,full,best_gamma,outdir)
...
plot_s2_nonlin(res,curves,nadir,outdir)
...
hr<-if(is.null(fit))tibble()else{sm<-coef(summary(fit));tibble(term=rownames(sm),HR=exp(sm[,"coef"]),lo=exp(sm[,"coef"]-1.96*sm[,"se(coef)"]),hi=exp(sm[,"coef"]+1.96*sm[,"se(coef)"]),p=sm[,"Pr(>|z|)"])|>filter(str_detect(term,"^risk_groupRisk Q[2-5]$"))|>mutate(group=str_extract(term,"[2-5]$"),group=factor(group,levels=2:5))}
plot_s2_penalty(perf,choose,inner_perf,summ,hr,full,best_gamma,outdir)
saveRDS(list(performance=perf,selected=choose,inner=inner_perf,summary=summ,risk_HR=hr,plot_scores=full[,c("n_fail","best_penalty")]),penalty_cache,compress="xz");finalize_outputs(LE8_JOB,outdir)
}
