...
if(Sys.getenv("LE8_C5_REFERENCE","")!="TRUE")stop("Run through c5_consolidate.R, which labels this per-layer reference separately from common-cohort joint validation")
...
C5_CODE_VERSION <- "2026-09-15.final-reference"
C5_MODEL_VERSION <- "2026-09-15.final-reference"
writeLines("Retained per-layer reference: training-only measured selection; different omic cohorts; not joint prot/met/PGS validation.",file.path(out.base,"c5.reference_scope.txt"))
...
c5_row4_yy <- function(score_rows,label){
  ...
  h<-d|>count(bin,name="n")|>transmute(bin,xmin=bin-C5_TEMPORAL_STEP/2,xmax=bin+C5_TEMPORAL_STEP/2,n)
...
attained_age_score_sensitivity <- function(score_rows,dat,methods,clinical,tvar,evar){
  empty<-tibble(method=character(),time_scale=character(),beta=numeric(),se=numeric(),
  ...
  map_dfr(eligible_methods,function(mm){
    ...
    fit_one<-function(kind){
      ...
      if(nrow(dd)<100||sum(dd[[evar]]==1)<20){
        message("C5: score time-scale sensitivity unavailable for ",mm," / ",kind,
      ...
      fit<-tryCatch(coxph(as.formula(paste0(surv," ~ ",paste(bt(rhs),collapse=" + "))),dd,ties="efron"),
        error=function(e){message("C5: score time-scale sensitivity fit failed for ",mm," / ",kind,": ",conditionMessage(e));NULL})
...
run_c5_layer <- function(layer=c("protein","metabolite")){
  if (LE8_REUSE_RESULTS) return(le8_restore_outputs(match.arg(layer), "c5_consolidate"))
  ...
  if(cache_valid(final_cache)){
    ...
    if(is.list(old)&&all(c("meta","scores","prediction","summary")%in%names(old))){
      cache_message(paste0("C5/",layer),final_cache);return(le8_restore_outputs(layer,"c5_consolidate"))
    }
    message("C5/",layer,": cache incomplete; recomputing score coverage and IPCW figures")
  }
  ...
  all0<-read_all();prs_vars<-character() # Disease PRS requires an explicit manifest in joint C5; no ambiguous name matching
  ...
  local_mrset<-if(nrow(m2)&&truthy(Sys.getenv("C5_GENETIC_EVIDENCE_INDEPENDENT",unset="FALSE")))m2|>filter(analysis==local_class,is.finite(FDR_all),FDR_all<.05)|>arrange(pval)|>pull(exposure)|>unique()|>intersect(biom_vars)else character()
  ...
  colocset<-if(nrow(co3)&&truthy(Sys.getenv("C5_GENETIC_EVIDENCE_INDEPENDENT",unset="FALSE")))co3|>filter(status=="ok",coalesce(PP.H4_robust_min,PP.H4)>=.7)|>pull(feature)|>unique()|>intersect(biom_vars)else character()
  ...
  out$mock<-le8_mock_c5(out,outdir)
