...
le8_c5_additions <- function(dat,biom_vars,ranked,training_screen,distalset,geneticset,
  ...
  values<-suppressWarnings(as.numeric(le8_csv_env("C5_ASSAY_BUDGETS",paste(LE8_ASSAY_BUDGETS,collapse=","))))
  if(!length(values)||any(!is.finite(values))||any(values!=floor(values))||any(values<1|values>500))stop("C5_ASSAY_BUDGETS must contain integers 1..500")
  ...
  le8_plot_c5_review(met, ca, dc, horizons, budgets, outdir)
  tables
}


le8_plot_c5_review <- function(met, ca, dc, horizons, budgets, outdir) {
  if(nrow(met)) {
