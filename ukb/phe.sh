...
Usage:
  ./phe.sh [options]
  ./phe.sh [START_STEP]
  ./phe.sh [START_STEP] [END_STEP]

Options:
  -h, --h, --help        Show this help message.

Environment control:
  START_STEP=data_qc ./phe.sh
  START_STEP=prep_raw END_STEP=process_linked RUN_R=0 ./phe.sh
  RUN_STEPS=prep_raw,extract_pheno ./phe.sh
  START_STEP=gp_prep END_STEP=gp ./phe.sh
  RUN_STEPS=gp_prep,gp ./phe.sh
...
R steps:
  data_qc vip_phe ses move death srd icd audit_i67 fod_ref gp_prep gp biom hla_pca gen_prs merge ckm audit_ckm audit_dates drug gwas

Association analyses (forest and PheWAS) have moved to ./assoc.sh.

Examples:
  DATS="vip" RUN_STEPS=extract_pheno ./phe.sh # 🏮
  SAVE_AOD_LIST=TRUE RUN_STEPS=icd,merge ./phe.sh # 🏮
...
Common variables:
  ...
  UKB_OUT=$DIR0/analysis/ukb/phe
  PHE_R=./f/phe.R
  RUN_R=1
  DATS="vip bbc img.ckm met death job"

ICD/AOD variables used by the R icd step:
  ICD_SOURCES=icd10,icd9,opcs4,srd
  SAVE_AOD_TEXT=TRUE      # recommended; creates aodtxt_* columns
  SAVE_AOD_LIST=FALSE     # keep FALSE unless list-column aod_* is required
  MAKE_PHEWAS=FALSE       # default; set TRUE only if PheWAS package is installed and PheWAS.rds is needed
EOF
}
...
START_STEP=${START_STEP:-${pos_args[0]:-data_qc}}
END_STEP=${END_STEP:-${pos_args[1]:-}}
RUN_STEPS=${RUN_STEPS:-}
RUN_R=${RUN_R:-1}
DATS=${DATS:-"vip bbc img.ckm met death job"}
ICD_SOURCES=${ICD_SOURCES:-icd10,icd9,opcs4,srd}
SAVE_AOD_TEXT=${SAVE_AOD_TEXT:-TRUE}
SAVE_AOD_LIST=${SAVE_AOD_LIST:-FALSE}
MAKE_PHEWAS=${MAKE_PHEWAS:-FALSE}
python_cmd=${PYTHON:-}
...
r_steps=(data_qc vip_phe ses move death srd icd audit_i67 fod_ref gp_prep gp biom hla_pca gen_prs merge ckm audit_ckm audit_dates drug gwas)
...
extract_pheno() {
    for dat in $DATS; do
        ...
        "$python_cmd" "$script_dir/0f/0join_file.py" -i "$dat.id.tmp1,TAB,1 $phedir/common/$dat.lst,TAB,0" -o "$dat.id.tmp2"
...
if [[ "$RUN_R" == "1" ]]; then
    ...
    if contains "$START_STEP" "${r_steps[@]}" || [[ "$START_STEP" == "all" ]]; then
        ...
        r_start=${R_START_STEP:-data_qc}
    fi
    r_end=${R_END_STEP:-${END_STEP:-}}
