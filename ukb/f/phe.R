...
step_order <- c(
	...
	"biom", "hla_pca", "gen_prs", "merge", "audit_dates",
	"qc", "drug", "gwas"
)
...
# UKB baseline urine albumin/creatinine ratio.
# Albumin: mg/L; urine creatinine: micromole/L.
# UACR mg/g = albumin * 8840 / creatinine.
# For Field 30505 '<6.7', retain interval bounds rather than treating as NA.
derive_ukb_uacr <- function(dat,
                            alb_col = "bb_UALB",
                            alb_flag_col = "bb_UALB_flag",
                            cr_col = "bb_UCRE",
                            cr_flag_col = "bb_UCRE_flag") {
  ...
  alb_flag <- trimws(as.character(get_col(alb_flag_col, NA_character_)))
  cr <- ckm_num(get_col(cr_col))
  cr_flag <- trimws(as.character(get_col(cr_flag_col, NA_character_)))

  # Field 30515 is extremely rare. A '<88' creatinine cannot support a stable
  # denominator, so these records remain unassessed.
  cr_below_lod <- grepl("<\\s*88", cr_flag, ignore.case = TRUE)
  cr_valid <- is.finite(cr) & cr > 0 & !cr_below_lod

  exact <- is.finite(alb) & alb >= 0 & cr_valid
  alb_below_lod <- !is.finite(alb) &
    grepl("<\\s*6([.]7)?", alb_flag, ignore.case = TRUE) & cr_valid

  exact_uacr <- rep(NA_real_, n)
  exact_uacr[exact] <- alb[exact] * 8840 / cr[exact]

  lo <- hi <- rep(NA_real_, n)
  lo[exact] <- hi[exact] <- exact_uacr[exact]
  lo[alb_below_lod] <- 0
  hi[alb_below_lod] <- 6.7 * 8840 / cr[alb_below_lod]

  # Conservative staging value: exact values, plus below-LOD records whose
  # upper bound is still unequivocally A1 (<30 mg/g).
  definite_a1 <- alb_below_lod & is.finite(hi) & hi < 30
  stage_uacr <- exact_uacr
  stage_uacr[definite_a1] <- hi[definite_a1]

  category <- dplyr::case_when(
    is.finite(stage_uacr) & stage_uacr < 30 ~ "A1",
    is.finite(stage_uacr) & stage_uacr < 300 ~ "A2",
    is.finite(stage_uacr) ~ "A3",
    alb_below_lod ~ "interval_uncertain",
    TRUE ~ NA_character_
  )

  # Midpoint-on-log-scale sensitivity value for PREVENT add-on analyses only.
  prevent_sens <- exact_uacr
  prevent_sens[alb_below_lod] <- (6.7 / sqrt(2)) * 8840 / cr[alb_below_lod]

  data.frame(
    eid = as.character(dat$eid),
    bb_UACR = stage_uacr,
    bb_UACR_exact = exact_uacr,
    bb_UACR_lo = lo,
    bb_UACR_hi = hi,
    bb_UACR_category = category,
    bb_UACR_assessed = exact | definite_a1,
    bb_UALB_below_lod = alb_below_lod,
    bb_UCRE_below_lod = cr_below_lod,
    bb_UACR_interval_uncertain = alb_below_lod & !definite_a1,
    bb_UACR_prevent_sens = prevent_sens,
...
# Build baseline/pre-baseline GP components from gp_clinical.full.fst.
# Active list format (one row per component):
# codes_csv, component, kind, plausible_min, plausible_max,
# positive_op, positive_threshold, primary_use.
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Fast ICD mode scans each source once, then builds all fod/lod/cnt/aodtxt columns.
# This fully replaces the old per-trait get_icd() loop, which repeatedly scanned the full N x slot matrix.
...
# Do not pass urine result flags/dates through clean_biom(): it coerces all
# columns to numeric and would erase '<6.7' flags. Keep urine variables outside
# the general biomarker QC and merge them back afterward.
ckm_bbc_cols <- intersect(c(
  "bb_UALB", "bb_UALB_flag", "bb_UCRE", "bb_UCRE_flag",
  "date_bb_UALB", "date_bb_UCRE"
), names(bbc_raw))
bbc_main <- bbc_raw[, c("eid", setdiff(names(bbc_raw), c("eid", ckm_bbc_cols))), drop = FALSE]
bbc_main <- clean_biom(bbc_main, miss_col = 0.2)
bbc_ckm0 <- bbc_raw[, c("eid", ckm_bbc_cols), drop = FALSE]
uacr <- derive_ukb_uacr(bbc_raw)
bbc <- Reduce(function(x, y) merge(x, y, by = "eid", all = TRUE, sort = FALSE),
              list(bbc_main, bbc_ckm0, uacr))
saveRDS(bbc, paste0(indir, "/Rdata/bbc.rds"))
cat("UACR assessed:", sum(bbc$bb_UACR_assessed %in% TRUE, na.rm = TRUE),
    "; exact:", sum(is.finite(bbc$bb_UACR_exact)),
    "; below LOD:", sum(bbc$bb_UALB_below_lod %in% TRUE, na.rm = TRUE), "\n")
...
prs <- fread(file.path(dir0, "data/ukb/prs/all.prs.gz"), header = TRUE) %>% as.data.frame()
saveRDS(prs, paste0(indir, "/Rdata/prs.rds"))
...
lst <- c("phe", "bbc", "diet", "move", "ses", "death", "fod.icd10", "fod.icd9", "fod.opcs4", "fod.srd", "fod.gp", "gp.ckm.med", "img.ckm", "fod.ref", "pca", "gen", "prs")
...
# Every merge input is expected to contain at most one row per participant.  A
# repeated base::merge() copies and sorts the ever-growing (roughly 500k x
# 1,300) result once per input, which can exhaust memory even when the final
# data set itself fits.  Build the full outer join column-wise instead.
...
dat0 <- dat0 %>% mutate(
	...
	cad.prs = cad.6m.score_sum, cad.prs.3c = f3c(cad.prs), # 🏮
...
# 🚩💊drug⭕️
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ppp <- fread(paste0(dir0, '/data.BIG/gwas/ppp/map_3k_v1.tsv'), sep = '\t', select = c(1,3,5), header = TRUE, fill = TRUE) %>% as.data.frame()
	drug <- fread(paste0(dir0, '/files/prot-drug.txt'), sep = '\t', header = TRUE, fill = TRUE) %>% as.data.frame()
	tmp <- merge_check(ppp = ppp, drug = drug, by.x = 'UniProt', by.y = 'UNIPROT_ACCESSION')
	drug <- merge(tmp$ppp, tmp$drug, by.x = 'UniProt', by.y = 'UNIPROT_ACCESSION') %>% 
