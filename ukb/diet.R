...
#🌠 4 digits (such as p1289), Touchscreen
#🌠 5 digits (such as p20090), 24-hour Recall
#🌠 5 digits (p26xxx), computed nutrients
#🌠 6 digits (such as p100010), Oxford WebQ refined
...
# 🚩 🍟MAHA
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# diet0 <- readRDS(paste0(indir, "/Rdata/diet0.rds"))
# field <- read_excel(paste0(dir0, "/files/foods.xlsx"), sheet = "Sheet2")
# field.w <- read_excel(paste0(dir0, "/files/foods.xlsx"), sheet = "Sheet3")
# dat.food <- readRDS(paste0(indir, "/Rdata/diet.fudan.rds")) 
score_up10 <- function(x, target) {x <- as.numeric(x); pmin(pmax(x / target, 0), 1) * 10}
score_down10 <- function(x, limit_good, limit_bad) {x <- as.numeric(x); pmin(pmax((limit_bad - x) / (limit_bad - limit_good), 0), 1) * 10}
score_mid10 <- function(x, lo, hi, hi_bad = Inf) {
	x <- as.numeric(x)
...
score_min10 <- function(x, target) {x <- as.numeric(x); pmin(pmax(x / target, 0), 1) * 10}
...
maha_primary <- base_maha %>% mutate(
	protein.maha = score_min10(protein_gkg, 1.2),
	dairy.maha = score_up10(dairy_srv, 3),
	veg.maha = score_up10(veg_srv, 3),
	fruit.maha = score_up10(fruit_srv, 2),
	wholegrain.maha = score_up10(wholegrain_srv, 2),
	fat.maha = score_up10(fat_srv, 1),
	upf.maha = score_down10(upf_srv, 0, 4),
	alcohol.maha = score_down10(alcohol_srv, 0, 2.5),
	sodium.maha = score_down10(sodium_mg, 2300, 5000)
...
maha_bal <- base_maha %>% mutate(
	protein.maha_bal = score_mid10(protein_gkg, 1.2, 1.6, 2.0),
	dairy.maha_bal = score_mid10(dairy_srv, 1, 2, 4),
	veg.maha_bal = score_up10(veg_srv, 3),
	fruit.maha_bal = score_up10(fruit_srv, 2),
	wholegrain.maha_bal = score_up10(wholegrain_srv, 2),
	fat.maha_bal = score_mid10(fat_srv, 0.5, 1.0, 2.0),
	upf.maha_bal = score_down10(upf_srv, 0, 3),
	alcohol.maha_bal = score_down10(alcohol_srv, 0, 1.5),
	sodium.maha_bal = score_down10(sodium_mg, 2000, 4000)
...
maha_strict <- base_maha %>% mutate(
	protein.maha_strict = score_mid10(protein_gkg, 0.8, 1.2, 1.8),
	dairy.maha_strict = score_mid10(dairy_srv, 0.5, 1.5, 3),
	veg.maha_strict = score_up10(veg_srv, 4),
	fruit.maha_strict = score_up10(fruit_srv, 2.5),
	wholegrain.maha_strict = score_up10(wholegrain_srv, 3),
	fat.maha_strict = score_mid10(fat_srv, 0.5, 1.0, 1.5),
	upf.maha_strict = score_down10(upf_srv, 0, 2.5),
	alcohol.maha_strict = score_down10(alcohol_srv, 0, 1),
	sodium.maha_strict = score_down10(sodium_mg, 1500, 3000)
...
maha_nodairy <- base_maha %>% mutate(
	protein.maha_nodairy = score_min10(protein_gkg, 1.2),
	veg.maha_nodairy = score_up10(veg_srv, 3),
	fruit.maha_nodairy = score_up10(fruit_srv, 2),
	wholegrain.maha_nodairy = score_up10(wholegrain_srv, 2),
	fat.maha_nodairy = score_up10(fat_srv, 1),
	upf.maha_nodairy = score_down10(upf_srv, 0, 4),
	alcohol.maha_nodairy = score_down10(alcohol_srv, 0, 2.5),
	sodium.maha_nodairy = score_down10(sodium_mg, 2300, 5000)
...
maha_noprotein <- base_maha %>% mutate(
	dairy.maha_noprotein = score_up10(dairy_srv, 3),
	veg.maha_noprotein = score_up10(veg_srv, 3),
	fruit.maha_noprotein = score_up10(fruit_srv, 2),
	wholegrain.maha_noprotein = score_up10(wholegrain_srv, 2),
	fat.maha_noprotein = score_up10(fat_srv, 1),
	upf.maha_noprotein = score_down10(upf_srv, 0, 4),
	alcohol.maha_noprotein = score_down10(alcohol_srv, 0, 2.5),
	sodium.maha_noprotein = score_down10(sodium_mg, 2300, 5000)
