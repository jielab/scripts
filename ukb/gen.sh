...
usage(){ cat <<'HELP'
cd /mnt/d/scripts/ukb

# GRCh37 -> GRCh38 imputed pfiles; then score against GRCh38 inputs.
./gen.sh liftGen --dir-in /mnt/i/ukbGen/37/imp --dir-out /mnt/i/ukbGen/38/imp \
  --chain-file /mnt/d/files/liftOver/hg19ToHg38.over.chain.gz
./gen.sh pgs --input /mnt/i/data/gwas/bmi.ref --label bmi \
  --pgs-pfile-dir /mnt/i/ukbGen/38/imp --dir-out /mnt/i/analysis/pgs/bmi

# Independent SNP extraction from typed data; BED coordinates must match its build.
./gen.sh snp --dir-gen /mnt/i/ukbGen/37 --data typ --snp-file /mnt/d/files/Wang_MY.bed \
  --snp-file-type bed --dir-out /mnt/d/analysis/ukb --label Wang
HELP
}
...
liftgen_output_is_complete() {
    ...
    # marker, dependencies, links, and PVAR rows before resuming past it.
    ...
    awk -v expected="$(awk -F '\t' '$1=="total_variants" {print $2}' "$output_log")" '
        /^##/ {next}
        /^#CHROM[\t ]/ {header=1; next}
        !/^#/ {if (NF<5) bad=1; n++}
        END {exit !(header && !bad && n==expected)}
    ' "$output_pvar"
}
...
run_liftgen() {
    if [[ -n "${bim_file:-}" ]]; then
        python3 "$SCRIPT_DIR/f/lift_bim.py" "$(to_wsl_path "$bim_file")" \
            ...
            "$(to_wsl_path "${ref_fasta:-/mnt/i/refGen/fasta/GRCH38.fasta}")"
        return
    fi
    ...
    ref_fasta=$(to_wsl_path "${ref_fasta:-/mnt/i/refGen/fasta/GRCH38.fasta}")
    ...
    local -a pvars
    mapfile -t pvars < <(find "$dir_in" -maxdepth 1 -type f -name 'chr*.pvar' -printf '%p\n' | sort -V)
    (( ${#pvars[@]} > 0 )) || { echo "ERROR: no chr*.pvar files found in $dir_in" >&2; exit 1; }
    ...
    for pvar in "${pvars[@]}"; do
        ...
        ' > "$out_tmp"
        [[ $(cat -- "$out_tmp" | awk '!/^#/{n++} END{print n+0}') -eq "$n_total" ]] || {
...
run_pgs() {
    ...
    pgs_pfile_dir=$(to_wsl_path "${pgs_pfile_dir:-/mnt/i/ukbGen/38/imp}")
...
scratch=\$(mktemp -d "$work_dir/run.XXXXXX")
...
if [[ \$(wc -l < merge-list.txt) -eq 1 ]]; then
    merged=\$(head -1 merge-list.txt)
else
    merged=merged
    plink2 --pmerge-list merge-list.txt --make-pgen --out "\$merged"
    while read -r part; do rm -f "\$part.pgen" "\$part.pvar" "\$part.psam"; done < merge-list.txt
fi
python3 "$SCRIPT_DIR/f/plink_raw_export.py" --output "$dir_out/$label.raw.gz" -- plink2 --pfile "\$merged" --maj-ref --export A
EOF2
    } > "$cmd_file"
    chmod +x "$cmd_file"
    ...
    if [[ "${dry_run:-N}" == Y ]]; then
        ...
        bash "$cmd_file"
        echo "OK: SNP data written to: $dir_out/$label.raw.gz"
    fi
}
